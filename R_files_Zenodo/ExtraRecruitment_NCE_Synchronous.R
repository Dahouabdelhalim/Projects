...
tnce = 10
ncetimes<-1:tnce
out2=as.data.frame(lsoda(y=newstate, times=ncetimes, func=threestage, parms=parms))  #run again, no fishing, new inits

#Now institute delayed NCEs
...
fishedvol<-(maxfished-y[210,8])/y[200,8] #0.119   #with delayed NCE: 0.318
maxcomm<-max(y$CommDens) #
commvol<-(maxcomm - y[210,7])/y[200,7] #0.418    #with NCE: 0.657 with delayed NCE: 0.657
