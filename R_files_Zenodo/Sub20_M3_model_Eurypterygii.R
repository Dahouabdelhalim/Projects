...
#MLE
print(paste("Start MLE MuSSE of neot M3 CM",chrmax, "..."))
timepoint<-c(proc.time()[[3]])
fit.musse.neot<-find.mle(mussefunc,p)
timepoint<-c(timepoint,proc.time()[[3]])

## Save fit.musse.neot
filename<-paste("fit_musse_neot_M3_CM",chrmax,"_samplingf_XXXXXX.Robj",sep="")
save(fit.musse.neot,file=filename)

## Result of the 7 para Estimation

coef_res<-c(coef(fit.musse.neot)[c(7,6,5,2,4,1,3)]) #2020 definition
