...
plot(FishedDens~time, data=y, type="l",ylim=c(0,20),bty="L",las=1, lwd=3)
lines(FishedDens~time, data=y, type="l",ylim=c(0,20),bty="L",las=1, lwd=3, col="red")

#RETURN TIMES###
#Calculate manually by looking at y dataframe, add up duration of first and 2nd transient periods until within 10% of equilibria

##Plot Fig 1b- stylized time series example
plot(FishedDens~time, data=save.y, type="l", ylim=c(0,20), bty="L", las=1, lwd=3, col="forestgreen", 
     ylab="Fished community biomass (kg)", xlab="Years since fishery closure",cex.lab=1.5, cex.axis=1.25)
lines(FishedDens~time, data=y, type="l", lwd=3, lty=2, col="purple")
