...
dat <- read.csv ("Lateralwidth.csv", header=T, row.names = 1)
...
# 1. Open pdf file
pdf("Lateralwidth.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
dtt_Lateralwidth <- dtt(phy, dat, index= "avg.manhattan",
    mdi.range=c(0,1), nsim=0, CI=0.95, plot=TRUE, calculateMDIp=F)
plot(dtt_Lateralwidth)
# 3. Close the file
dev.off()
