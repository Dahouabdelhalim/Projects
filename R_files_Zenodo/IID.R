...
# Generate non-phylogenetic model with unequal rates file for JAGS
modelstring = "
model {
...
# Unequal phenotipic variance
  TAU <- inverse(gamma1*A1 + gamma2*A2)
}
"
...
# Create matrices with ones in the diagonal according to the categorical variable
Numb<-1:nSubj
# G1 for the plesiomorphic condition: 0
G1 <- matrix(0, nrow=nSubj, ncol=nSubj)
Ancestral<-Numb[z==0]
for (i in 1:length(Ancestral)) {
  G1[Ancestral[i],Ancestral[i]]<-1
}
# G2 for the apomorphic condition: 1
G2 <- matrix(0, nrow=nSubj, ncol=nSubj)
Derived<-Numb[z==1]
for (i in 1:length(Derived)) {
  G2[Derived[i],Derived[i]]<-1
}
...
# Set up chains
parameters = c("beta0" , "beta1" , "beta2"  , "beta3"  , "sigma1" , "sigma2")  # Saved parameters
...
# Set up chains
parameters = c("beta0" , "beta1" , "beta2"  , "beta3"  , "sigma")  # Saved parameters
adaptSteps = 1000             # Tunning steps
burnInSteps = 10000           # Burn-in steps
nChains = 3                   # Number of chains
numSavedSteps=100000          # Total steps
