...
# subset out host alternating species,
phyl<-drop.tip(phyl, data[host.alt!="hetero"]$species)
...
if(FALSE){
  ...
  Sys.time()
  out
  saveRDS(out, file="musse/SecSSE/monoecious/mono-cr.RDS")
}
...
if(FALSE){
  ...
  saveRDS(out, file="musse/SecSSE/monoecious/mono-ctd.RDS")
}
...
if(TRUE){
  ...
  saveRDS(out, file="musse/SecSSE/monoecious/mono-etd.RDS")
...
if(FALSE){
  cr<-readRDS("musse/SecSSE/monoecious/mono-cr.RDS")
  ctd<-readRDS("musse/SecSSE/monoecious/mono-ctd.RDS")
  etd<-readRDS("musse/SecSSE/monoecious/mono-etd.RDS")
  
  mods<-list(cr,ctd,etd) 
  m<-c("cr","ctd","etd") # random min/max
  logs<-setNames(c(cr$ML,ctd$ML,etd$ML),m)
