...
##GSA using Community Volatility as the model outcome: Prey First RECOVERY
...
##Set up data for GSA
preyGSA<-matrix(NA,sims,25)   #dummy matrix to hold state variables and sampled parameter values
preyGSA[,1]<-t(N)          #input state variable values from simulations
preyGSA[,2:25]<-t(parameters) #input randomly chosen parameter values
preyGSA<-as.data.frame(preyGSA)
colnames(preyGSA)<-c("Volatility", "r","Ka","Kul","sigma",    #define parameter labels
...
preyvol.rf<-randomForest(preyGSA[,1] ~ .,
                     data       = preyGSA[,2:25],
                     importance = TRUE)
print(preyvol.rf)
round(importance(preyvol.rf),2)
preyvol.imp = pmax (importance (preyvol.rf,scale = FALSE) [, 1], 0)
preyvol.imp = preyvol.imp/sum(preyvol.imp)

#Bar plot of normalized importance values
#pdf("ImportancePrey.pdf")
...
#hi resolution plot
#tiff("testPreyFirst.tif", width=3, height=3, units="in", res=600, pointsize=4)
...
Prey_importance<-as.data.frame(preyvol.imp)
preyvol.imp<-sort(preyvol.imp, decreasing=F)
barplot(preyvol.imp, horiz=T, xlim=c(0,0.5), las=1, xlab="Importance value") #sorted plot without greek symbols
#updated plot with sorted labels
tiff("PreySorted_HiRes.tif", width=6, height=6, units="in", res=800, pointsize=6, compression="lzw")
...
#pdf("HistogramPreyFirst.pdf")
par(mai=c(1.02,1.22,0.862,0.62))
hist(preyGSA[,1], breaks=seq(0,4,by=0.15), las=1, col="grey", main="", xlab="Volatility", labels=T)
#dev.off()
...
cartpreyvol<-rpart(Volatility~r+Ka+Kul+sigma+deltaUs+deltaUm+deltaUl+
                 aM+aL+b+gammaS+gammaM+deltaLs+deltaLm+
                 deltaLl+MUs+MUm+MUl+FU+taus+taum+taul+
                 ML+FL, data=preyGSA, control=rpart.control(xval=10,minsplit=20)) #, method="pois",  #use Poisson dist.
print(cartpreyvol)
plot(cartpreyvol)
text(cartpreyvol, use.n=T, cex=0.75)
printcp(cartpreyvol)
plotcp(cartpreyvol)#look at cost-complexity parameter plot to set threshold cp for pruned tree
prunedpreyvol<-prune(cartpreyvol,cp=0.012) #use cp value from cp plot to prune tree

#CART plot
#tiff("Pred_GSA_Cart.tif", width=5, height=5, units="in", res=800, pointsize=4.8, compression="lzw")
par(mai=c(1.32,1.42,0.42,0.62), cex.axis=1.5, cex.lab=1.5)
prp(prunedpreyvol, extra=1, yesno=T, cex=1.5)
#dev.off()
