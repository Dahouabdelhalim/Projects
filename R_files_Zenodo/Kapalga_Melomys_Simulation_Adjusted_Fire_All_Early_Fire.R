...
system.time(
  melomys_AF_AEF_pop_data <- foreach (i = 1:25, .combine = rbind) %do% {
    set.seed(333)
    Melomys4 <- simulation(landscape = melomys_landscape,
    ...
    pop_data <- return_pop_data(Melomys4)
    rm(Melomys4)
    gc()
    pop_data
  }
...
save(melomys_AF_AEF_pop_data, file = "melomys_AF_AEF_pop_data.RData")

# Plot possum population trajectory of each life-stage
plot(Melomys)

# Plot total possum population trajectory
plot(Melomys4, stage = 0, ylim = c(0,15000))
