...
for (code in dataVessel$X3A_CODE){
  ...
  if (counter%%10==0){
    end_time <- Sys.time()
    cat("status: ",status,"elapsed",(end_time-start_time),"s;","total",(end_time-start_time0),"s","\\n")
    start_time<-end_time
  }
  request = gsub("#CODE#", code, GRSFRequest)
  requestFile<-paste0("response_",UUIDgenerate(),".txt")
  got<-system(paste0("wget -q -O ",requestFile," \\"",request,"\\""),ignore.stdout = T,ignore.stderr=T,intern = T)
