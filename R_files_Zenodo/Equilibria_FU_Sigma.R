##Robert Dunn
##Three stage urchin model,   recruitment facilitation and urchin F
...
sigma= 0.5      #recruitment facilitation- h
...
###############
##Bifurcation for recruitment facilitation at 6 values of urchin F, keep final equilibrium pop value 
###############

## sigma=0.0 ##
FUseq<-seq(0.0,1.0,length=100)   #vector of lobster fishing mortality rates to run model over
N1 <-matrix(NA,length(FUseq),5)  #create vector with NA's to store equilibrium state var. population size
...
## sigma=0.2 ##
FUseq<-seq(0.0,1,length=100)   #vector of lobster fishing mortality rates to run model over
N2 <-matrix(NA,length(FUseq),5)  #create vector with NA's to store equilibrium state var. population size
sigma=0.2                        #set level of recruitment facilitation
for(i in 1:length(FUseq)){
  FU=FUseq[i]
  parms=c(r=r, Ka=Ka, Kul=Kul, sigma=sigma,deltaUs=deltaUs, deltaUm=deltaUm, 
          deltaUl=deltaUl,aM=aM, aL=aL, b=b, gammaS=gammaS, gammaM=gammaM, 
          ...
          MUs=MUs, FU=FU, taus=taus, taum=taum, taul=taul, ML=ML, FL=FL)
  out=lsoda(y=state, times=times, func=threestage, parms=parms)
  N2[i,1]=out[200,2]   #kelp pop vector
  N2[i,2]=out[200,3]   #Us pop vector
  N2[i,3]=out[200,4]   #Um pop vector
  N2[i,4]=out[200,5]   #Ul pop vector
  N2[i,5]=out[200,6]   #lobster pop vector
}

## sigma=0.4 ##
FUseq<-seq(0.0,1,length=100)   #vector of lobster fishing mortality rates to run model over
N3 <-matrix(NA,length(FUseq),5)  #create vector with NA's to store equilibrium state var. population size
sigma=0.4                        #set level of recruitment facilitation
for(i in 1:length(FUseq)){
  FU=FUseq[i]
  parms=c(r=r, Ka=Ka, Kul=Kul, sigma=sigma,deltaUs=deltaUs, deltaUm=deltaUm, 
          deltaUl=deltaUl,aM=aM, aL=aL, b=b, gammaS=gammaS, gammaM=gammaM, 
          ...
          MUs=MUs, FU=FU, taus=taus, taum=taum, taul=taul, ML=ML, FL=FL)
  out=lsoda(y=state, times=times, func=threestage, parms=parms)
  N3[i,1]=out[200,2]   #kelp pop vector
  N3[i,2]=out[200,3]   #Us pop vector
  N3[i,3]=out[200,4]   #Um pop vector
  N3[i,4]=out[200,5]   #Ul pop vector
  N3[i,5]=out[200,6]   #lobster pop vector
}

## sigma=0.6 ##
FUseq<-seq(0.0,1,length=100)   #vector of lobster fishing mortality rates to run model over
N4 <-matrix(NA,length(FUseq),5)  #create vector with NA's to store equilibrium state var. population size
sigma=0.6                        #set level of recruitment facilitation
for(i in 1:length(FUseq)){
  FU=FUseq[i]
  parms=c(r=r, Ka=Ka, Kul=Kul, sigma=sigma,deltaUs=deltaUs, deltaUm=deltaUm, 
          deltaUl=deltaUl,aM=aM, aL=aL, b=b, gammaS=gammaS, gammaM=gammaM, 
          ...
          MUs=MUs, FU=FU, taus=taus, taum=taum, taul=taul, ML=ML, FL=FL)
  out=lsoda(y=state, times=times, func=threestage, parms=parms)
  N4[i,1]=out[200,2]   #kelp pop vector
  N4[i,2]=out[200,3]   #Us pop vector
  N4[i,3]=out[200,4]   #Um pop vector
  N4[i,4]=out[200,5]   #Ul pop vector
  N4[i,5]=out[200,6]   #lobster pop vector
}

## sigma=0.8 ##
FUseq<-seq(0.0,1,length=100)   #vector of lobster fishing mortality rates to run model over
N5 <-matrix(NA,length(FUseq),5)  #create vector with NA's to store equilibrium state var. population size
sigma=0.8                        #set level of recruitment facilitation
for(i in 1:length(FUseq)){
  FU=FUseq[i]
  parms=c(r=r, Ka=Ka, Kul=Kul, sigma=sigma,deltaUs=deltaUs, deltaUm=deltaUm, 
          deltaUl=deltaUl,aM=aM, aL=aL, b=b, gammaS=gammaS, gammaM=gammaM, 
          ...
          MUs=MUs, FU=FU, taus=taus, taum=taum, taul=taul, ML=ML, FL=FL)
  out=lsoda(y=state, times=times, func=threestage, parms=parms)
  N5[i,1]=out[200,2]   #kelp pop vector
  N5[i,2]=out[200,3]   #Us pop vector
  N5[i,3]=out[200,4]   #Um pop vector
  N5[i,4]=out[200,5]   #Ul pop vector
  N5[i,5]=out[200,6]   #lobster pop vector
}

## sigma=0.9 ##
FUseq<-seq(0.0,1,length=100)   #vector of lobster fishing mortality rates to run model over
N6 <-matrix(NA,length(FUseq),5)  #create vector with NA's to store equilibrium state var. population size
sigma=0.9                        #set level of recruitment facilitation
for(i in 1:length(FUseq)){
  FU=FUseq[i]
  parms=c(r=r, Ka=Ka, Kul=Kul, sigma=sigma,deltaUs=deltaUs, deltaUm=deltaUm, 
          deltaUl=deltaUl,aM=aM, aL=aL, b=b, gammaS=gammaS, gammaM=gammaM, 
          ...
          MUs=MUs, FU=FU, taus=taus, taum=taum, taul=taul, ML=ML, FL=FL)
  out=lsoda(y=state, times=times, func=threestage, parms=parms)
  N6[i,1]=out[200,2]   #kelp pop vector
  N6[i,2]=out[200,3]   #Us pop vector
  N6[i,3]=out[200,4]   #Um pop vector
  N6[i,4]=out[200,5]   #Ul pop vector
  N6[i,5]=out[200,6]   #lobster pop vector
}

############################
#plot all 6 (for each state variable separately)
############################

pdf("Equilibria_FU_Sigma.pdf")
par(mfrow=c(3,2))
plot(FUseq,N1[,1], type="p", cex=0.5, pch="o", col="black", lwd=2, xlab="Urchin F",
     ylab="Kelp (kg)",ylim=c(0,2500), bty="L", las=1)
lines(FUseq,N2[,1], type="p", cex=0.5, pch="o", col="blue4", lwd=2)
lines(FUseq,N3[,1], type="p", cex=0.5, pch="o", col="purple", lwd=2)
lines(FUseq,N4[,1], type="p", cex=0.5, pch="o", col="red", lwd=2)
lines(FUseq,N5[,1], type="p", cex=0.5, pch="o", col="orange", lwd=2)
lines(FUseq,N6[,1], type="p", cex=0.5, pch="o", col="yellow", lwd=2)

plot(FUseq,N1[,2], type="p", cex=0.5, pch="o", col="black", lwd=2, ylim=c(0,40),xlab="Urchin F",
     ylab="Small Urchins (kg)", bty="L", las=1)
lines(FUseq,N2[,2], type="p", cex=0.5, pch="o", col="blue4", lwd=2)
lines(FUseq,N3[,2], type="p", cex=0.5, pch="o", col="purple", lwd=2)
lines(FUseq,N4[,2], type="p", cex=0.5, pch="o", col="red", lwd=2)
lines(FUseq,N5[,2], type="p", cex=0.5, pch="o", col="orange", lwd=2)
lines(FUseq,N6[,2], type="p", cex=0.5, pch="o", col="yellow", lwd=2)

plot(FUseq,N1[,3], type="p", cex=0.5, pch="o", col="black", lwd=2, ylim=c(0,40),xlab="Urchin F",
     ylab="Medium Urchins (kg)", bty="L", las=1)
lines(FUseq,N2[,3], type="p", cex=0.5, pch="o", col="blue4", lwd=2)
lines(FUseq,N3[,3], type="p", cex=0.5, pch="o", col="purple", lwd=2)
lines(FUseq,N4[,3], type="p", cex=0.5, pch="o", col="red", lwd=2)
lines(FUseq,N5[,3], type="p", cex=0.5, pch="o", col="orange", lwd=2)
lines(FUseq,N6[,3], type="p", cex=0.5, pch="o", col="yellow", lwd=2)
legend(0.15,41.5,xjust=0.5,title="sigma",c("0.0","0.2","0.4","0.6","0.8","0.9"), 
       bty="n", c("black", "blue4","purple", "red", "orange", "yellow"))

plot(FUseq,N1[,4], type="p", cex=0.5, pch="o", col="black", lwd=2, ylim=c(0,40),xlab="Urchin F",
     ylab="Large Urchins (kg)", bty="L", las=1)
lines(FUseq,N2[,4], type="p", cex=0.5, pch="o", col="blue4", lwd=2)
lines(FUseq,N3[,4], type="p", cex=0.5, pch="o", col="purple", lwd=2)
lines(FUseq,N4[,4], type="p", cex=0.5, pch="o", col="red", lwd=2)
lines(FUseq,N5[,4], type="p", cex=0.5, pch="o", col="orange", lwd=2)
lines(FUseq,N6[,4], type="p", cex=0.5, pch="o", col="yellow", lwd=2)

plot(FUseq,N1[,5], type="p", cex=0.5, pch="o", col="black", lwd=2, xlab="Urchin F",
     ylab="Lobsters (kg)", ylim=c(0,15), bty="L", las=1)
lines(FUseq,N2[,5], type="p", cex=0.5, pch="o", col="blue4", lwd=2)
lines(FUseq,N3[,5], type="p", cex=0.5, pch="o", col="purple", lwd=2)
lines(FUseq,N4[,5], type="p", cex=0.5, pch="o", col="red", lwd=2)
lines(FUseq,N5[,5], type="p", cex=0.5, pch="o", col="orange", lwd=2)
lines(FUseq,N6[,5], type="p", cex=0.5, pch="o", col="yellow", lwd=2)
