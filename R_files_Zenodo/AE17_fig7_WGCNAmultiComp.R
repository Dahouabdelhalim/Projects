...
### Meta data ###
model_original<-read.csv("data/Phenotype/AE17_EPFpHComplete.csv",stringsAsFactors = FALSE)
sampleList<-readRDS("data/meta/AE17_RNAmetaData.RData")
model <- sampleList[sampleList$ID != "17005",]
m_final <- model[model$ID != "17099",]
mo <- model_original[model_original$ID %in% m_final$ID,]
m_final$EPF_pH <- mo$pHTotal
m_final$diff_pH <- m_final$EPF_pH-mo$pH_Total_2W
