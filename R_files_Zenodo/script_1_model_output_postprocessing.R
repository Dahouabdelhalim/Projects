...
res_path  = "./model_ouput/"  # Folder path where CLM 5 results are store in ".nc" format.
par_path = "./parameters/"  # Folder path where the experiment parameters are store in ".nc" format.
out_path = "./results/"          # Folder path to store the output files.
expr = "kbhr" # Experiment ID use to identify the model results.
srfc ='std' # Name used for the surface data set to select from the parameter file.
folder = 'kbhr' # Folder name to save the output data.

#-Select ID of the SapFluxNet site --------------------------------------------#
stid <- 'ES-Alt' # Site id of each experimental site
pftn=6 # pft order use by R. 6 = BET, 8 = BDT
...
# Parameters Setup -------------------------------------------------------------
def_exp = c("std") # Experiments using only the default parameter file of CLM 5.0
...
if (expr %in% def_exp) {
  ...
  nc <- nc_open(paste(par_path,"clm5_params.c171117_",stid,"_",expr,".nc",sep=""))
  ck <- ncvar_get(nc,"ck");ck<-ck[pftn]
  kmax <- ncvar_get(nc,"kmax"); kmax <- kmax[pftn,]
  p50 <- ncvar_get(nc,"psi50");p50 <- p50[pftn,]
  rt_cond <- ncvar_get(nc,"krmax");rt_cond<-rt_cond[pftn]
...
lst <- list.files(paste(res_path,stid,"/",folder,"/",sep=""))
lst <- lst[-length(lst)]
setwd(paste(res_path,stid,"/",folder,"/",sep=""))
...
for (i in unique(lst)) {
  ...
  shalai <- ncvar_get(nc,varid = "LAISHA",raw_datavals = F)
  sunlai <- ncvar_get(nc,varid = "LAISUN",raw_datavals = F)
  ...
  bsha <- ncvar_get(nc,varid = "BSHA",raw_datavals = F)
  bsun <- ncvar_get(nc,varid = "BSUN",raw_datavals = F)
  ...
  SHV <- c(SHV,shv)
  LAI <- c(LAI,lai)
  SHALAI <- c(SHALAI,shalai)
  SUNLAI <- c(SUNLAI,sunlai)
...
SHV <- na.omit(SHV)
LAI <- na.omit(LAI)
SUNLAI <- na.omit(SUNLAI)
SHALAI <- na.omit(SHALAI)
TSOI <- na.omit(TSOI)
VSWC <- na.omit(VSWC); VSWC <- data.frame(VSWC)

mdr <- data.frame(TIME,YEAR,ET,EI,ES,TA,RH,RF,SWR,LWR,WND,VEGWP,smp,SHG,SHV,VSWC,FPSN,ZWT,BSHA,BSUN,GSSUN,GSSHA,QDRAI,QOVER,QCHARGE,FSDS,FIRA,P,GHF,LAI,SUNLAI,SHALAI)
...
dval <- mdr[c("doy","time","yr","FPSN","RF","ET","EI","ES","ZWT","root","xyl","sun","shade","BSHA","BSUN","GSSHA","GSSUN","vpd","QDRAI","QOVER","QCHARGE","FSDS","FIRA","WND","TA","RH","P","SWR","GHF","sl1","sl2","sl3","sl4","sl5","sl6","sl7","LAI","SUNLAI","SHALAI")]
dval$hr <- round(24*(dval$time-dval$doy))
...
dval$NR <- 0.0864*((dval$FSDS*(1-0.23))-dval$FIRA) #net radiation
...
# Path to folder ---------------------------------------------------------------
dir.create(paste(out_path,stid,sep=""))
write.csv(df,file = paste(out_path,stid,"/",stid,"_",folder,"_hr.csv",sep=""))
