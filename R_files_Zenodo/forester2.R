forester2<-function (left_side_data, estimate, ci_low, ci_high, ci_sep = " to ",
                     right_side_data = NULL, estimate_precision = 1, ggplot_width = 30,
                     null_line_at = 0, null_line_at1 = 0, null_line_at2 = 0, file_path = file.path(tempdir(), paste0("forester_plot.",
                                                                                                                     render_as)), dpi = 600, display = TRUE, font_family = "mono",
                     estimate_col_name = "Estimate", stripe_colour = "#eff3f2",
                     background_colour = "white", x_scale_linear = TRUE, xlim = NULL,
                     xbreaks = NULL, nudge_y = 0, nudge_x = 1, nudge_height = 0,
                     nudge_width = 0, justify = 0, arrows = FALSE, arrow_labels = c("Lower",
                                                                                    "Higher"), add_plot = NULL, add_plot_width = 1, add_plot_gap = FALSE,
                     point_sizes = 3, point_shapes = 16, center_ggplot = NULL,
                     lower_header_row = FALSE, render_as = "png")
{
  if (!length(justify) == 1) {
  ...
  center <- ggplot2::ggplot() + ggplot2::geom_point(data = gdata,
                                                    ggplot2::aes(y = row_num, x = estimate, size = sizes,
                                                                 shape = shape), na.rm = TRUE) + ggplot2::geom_errorbarh(data = gdata,
    ...
    ggplot2::geom_vline(xintercept = null_line_at1, linetype = "dashed", colour="darkred", size=0.4) +
    ggplot2::geom_vline(xintercept = null_line_at2, linetype = "dashed", colour="darkred", size=.4) +
