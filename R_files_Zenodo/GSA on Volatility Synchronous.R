...
##GSA using Community Volatility as the model outcome: SYNCHRONOUS RECOVERY
...
parameters <- data.frame(rbind(runif(sims,6,12),    #r
                               ...
                               runif(sims,0.0,0.75), # sigma #orig. was 0.975 but that gave some odd volatility values
...
for(j in 1:ncol(parameters)){
  ...
  fishedvol<-(maxfished-FishedDens[200])/FishedDens[200]  #calculate fished community volatility
  
  N[1,j]=fishedvol   #Volatility value for a given simulation
}

##Set up data for GSA
synchGSA<-matrix(NA,sims,25)   #dummy matrix to hold state variables and sampled parameter values
synchGSA[,1]<-t(N)          #input state variable values from simulations
synchGSA[,2:25]<-t(parameters) #input randomly chosen parameter values
synchGSA<-as.data.frame(synchGSA)
colnames(synchGSA)<-c("Volatility", "r","Ka","Kul","sigma",    #define parameter labels
                     "deltaUs","deltaUm", "deltaUl",
                     "aM","aL", "b", "gammaS", "gammaM", "deltaLs", "deltaLm",
                     "deltaLl","MUs","MUm","MUl", "FU","taus","taum","taul",
                     "ML", "FL")

#Run Synchronous GSA- random forest and then CART
##Random forest on volatility
synchvol.rf<-randomForest(synchGSA[,1] ~ .,
                     data       = synchGSA[,2:25],
                     importance = TRUE)
print(synchvol.rf)
round(importance(synchvol.rf),2)
synchvol.imp = pmax (importance (synchvol.rf,scale = FALSE) [, 1], 0)
synchvol.imp = synchvol.imp/sum(synchvol.imp)

#Bar plot of normalized importance values
#pdf("ImportanceSynch.pdf")
par(mai=c(1.02,1.02,0.62,0.42))
barplot (synchvol.imp,
         horiz = T,
...
#hi resolution plot
#tiff("testSynch.tif", width=3, height=3, units="in", res=600, pointsize=4)
...
Synch_importance<-as.data.frame(synchvol.imp)
synchvol.imp<-sort(synchvol.imp, decreasing=F)
barplot(synchvol.imp, horiz=T, xlim=c(0,0.5), las=1, xlab="Importance value") #sorted plot without greek symbols
#updated plot with sorted labels
tiff("SynchSorted_HiRes.tif", width=6, height=6, units="in", res=800, pointsize=6, compression="lzw")
par(mai=c(1.32,1.42,0.42,0.62), cex.axis=1.5, cex.lab=1.5)
barplot(synchvol.imp, 
        names.arg=c(expression(paste(delta[italic(UL)])), 
                    expression(paste(alpha[italic(L)])),
                    expression(italic(M[Um])),
                    expression(paste(tau[italic(S)])), 
                    expression(paste(tau[italic(M)])),
                    expression(italic(K[UL])),
...
hist(synchGSA[,1], breaks=20, las=1, col="grey", main="")#, labels=T)
#dev.off()
...
cartsynchvol<-rpart(Volatility~r+Ka+Kul+sigma+deltaUs+deltaUm+deltaUl+
                 aM+aL+b+gammaS+gammaM+deltaLs+deltaLm+
                 deltaLl+MUs+MUm+MUl+FU+taus+taum+taul+
                 ML+FL, data=synchGSA, control=rpart.control(xval=10,minsplit=20))
#, method="pois",  #use Poisson dist.
print(cartsynchvol)
plot(cartsynchvol)
text(cartsynchvol, use.n=T, cex=0.75)
printcp(cartsynchvol)
plotcp(cartsynchvol)#look at cost-complexity parameter plot to set threshold cp for pruned tree
prunedvol<-prune(cartsynchvol,cp=0.01) #use cp value from cp plot to prune tree

#CART plot
#tiff("Synch_GSA_Cart.tif", width=5, height=5, units="in", res=800, pointsize=4.8, compression="lzw")
par(mai=c(1.32,1.42,0.42,0.62), cex.axis=1.5, cex.lab=1.5)
prp(prunedvol, extra=1, yesno=T, cex=1.5)
#dev.off()
