...
Neu.new <- full_join(Neu, d10.density.data, by = "Tank")
...
EPA.glm.neu.cont <- glm(as.numeric(C20.5)/100 ~ Treatment + density - 1, data = Neu.uni, family = quasibinomial(link = "logit"))

library(multcomp)
Cont1 <- matrix(c(-2, 1, 1, 0), 1) #Control vs. the 2 PUFAs
cont.test1 <- glht(EPA.glm.neu.cont, linfct = Cont1)
summary(cont.test1)
Cont2 <- matrix(c(0, 1, -1, 0), 1)  #PUFA vs. PUFA + a-t
cont.test2 <- glht(EPA.glm.neu.cont, linfct = Cont2)
summary(cont.test2)
