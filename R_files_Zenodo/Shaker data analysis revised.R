...
# Random slope models: included on recommendation of Stage 2 reviewer 07232019

# first 2 minutes, active bins
lmer(active_bins ~ age * Day + (1|subj_id), data = activity8) %>% anova() # original analysis: random intercept
lmer(active_bins ~ age * Day + (Day-1|subj_id), data = activity8) %>% anova() # random slope

# first 2 minutes, total activity
lmer(total ~ age * Day + (1|subj_id), data = activity8) %>% anova() # original analysis: random intercept
lmer(total ~ age * Day + (Day-1|subj_id), data = activity8) %>% anova() # random slope

# all 20 minutes, active bins
lmer(active_bins ~ age * Day + (1|subj_id), data = activity80) %>% anova() # original analysis: random intercept
lmer(active_bins ~ age * Day + (Day-1|subj_id), data = activity80) %>% anova() # random slope

# all 20 minutes, total activity
lmer(total ~ age * Day + (1|subj_id), data = activity80) %>% anova() # original analysis: random intercept
lmer(total ~ age * Day + (Day-1|subj_id), data = activity80) %>% anova() # random slope

# amount of reward retrieved 
lmer(loss ~ age * day + (1|subj_id), data = veggie_relish) %>% anova() # original analysis: random intercept
lmer(loss ~ age * day + (day-1|subj_id), data = veggie_relish) %>% anova() # random slope

# efficiency measure
lmer(efficiency ~ age * Day + (1|subj_id), data = efficient) %>% anova() # original analysis: random intercept
lmer(efficiency ~ age * Day + (Day-1|subj_id), data = efficient) %>% anova() # random slope
