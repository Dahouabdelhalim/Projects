...
time.plot.mult <- function(nrow, ncol=1, plot.height=6.22, plot.width=7, time.height=1.25, bg="white", 
	...
	bottom.mar = 3.25, left.mar = 3.5, top.mar = 0.75, right.mar = 1, mgpX=c(2, 0.75, 0), mgpY=c(2, 0.75, 0), las = 0, xaxs="i", 
...
#	print(height.prop)
	...
	par(mar=c(bottom.mar, left.mar, 0, right.mar), mgp=mgpY, xaxs=xaxs, yaxs="i", cex=cex, cex.axis=cex.axis, cex.lab=cex.lab)
	
	#plot time scale
	for(i in 1:ncol) {
		if(is.null(all.cols) | is.element(i, all.cols)) {		
			plot(periods, my.y, xlim=rev(range(periods)), xlab="", ylab="", type="n", axes=FALSE, frame.plot=TRUE)	
			axis(side=1, at=pretty(range(periods)), lwd.ticks=1, lwd=0)
			title(xlab="Geologic time (Ma)", mgp=mgpX)
	...
	par(mar=c(0, left.mar, top.mar, right.mar), mgp=mgpY, yaxs="r", cex=cex, cex.axis=cex.axis, cex.lab=cex.lab, xaxs=xaxs)	
}