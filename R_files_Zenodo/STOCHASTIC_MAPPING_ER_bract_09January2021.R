...
require(phytools)
## read data
X<-read.csv("bract.csv",header=TRUE)
X
bract <- X$bract
names(bract) <- X$Species
class(bract)

bract <- as.factor(bract)
class(bract)
## read tree
...
fitER<-ace(bract,tree,model="ER",type="discrete")
fitER
...
# 1. Open pdf file
pdf("bract_ER_1.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
plotTree(tree,type="fan",fsize=0.7,ftype="i",lwd=1)
nodelabels(node=1:tree$Nnode+Ntip(tree),
           pie=fitER$lik.anc,piecol=colr,cex=0.4)
tiplabels(pie=to.matrix(bract[tree$tip.label],
                        levels(bract)),piecol=colr,cex=0.3)
...
mtreeER<-make.simmap(tree,bract,model="ER")

# 1. Open pdf file
pdf("bract_ER_2.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
...
mtreesER<-make.simmap(tree,bract,model="ER",nsim=1000)

# 1. Open pdf file
pdf("bract_ER_3.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
...
# 1. Open pdf file
pdf("bract_ER_4.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
par(mfrow=c(1,1))
...
# 1. Open pdf file
pdf("bract_ER_5.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
...
# 1. Open pdf file
pdf("bract_ER_6.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
objER<-densityMap(mtreesER,states=levels(bract)[2:1],plot=FALSE)
...
# 1. Open pdf file
pdf("bract_ER_7.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
dotTree(tree,bract,colors=colr,fsize=0.7,ftype="i",
        legend=FALSE)
...
# 1. Open pdf file
pdf("bract_ER_8.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
par(mar=c(4,4,3,3))
