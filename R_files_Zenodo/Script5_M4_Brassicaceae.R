...
## Load the data
phy.bras<-read.tree(file="Brassica_tree_YK2020.tre")
...
###### MLE wih MuSSE M4 model ######

## Make MuSSE object
mussefunc<-make.musse(phy.bras,phy.bras$tip.state,snum,strict=F)

## Determine free.i for 5 parameters of karyotype evolution with polyploidization
free.i<-c(tcal(snum,1,2),tcal(snum,1,3),tcal(snum,2,1),tcal(snum,2,3),tcal(snum,3,2))

## Determine free.i for 2 parameters of speciation and extinction rates
free.i<-c(1,snum+1,((snum*2)+free.i))

## Get constraint formulae and target.i for MuSSE M4 model
cons_and_tar<-get_cons_and_target_musse_polyp_ec(chrmax)

## Make constraints
mussefunc<-constrain.kt(mussefunc,cons_and_tar[[1]],free.i,cons_and_tar[[2]])

## Get priors
int.p<-starting.point.musse(phy.bras,k=snum)
p<-int.p[free.i] #prior
names(p)<-c()

## Start MLE of M4 model
print(paste("Start MLE of M4 bras CM",chrmax, "..."))
timepoint<-c(proc.time()[[3]])
fit.musse.bras<-find.mle(mussefunc,p)
timepoint<-c(timepoint,proc.time()[[3]])

## Save the resultant R object fit.musse.bras
filename<-paste("fit_musse_bras_polyp_CM",chrmax,"_unisample_XXXXXX.Robj",sep="")
save(fit.musse.bras,file=filename)

## Text output of the result
coef_res<-c(coef(fit.musse.bras)[c(7,6,3,5,4,1,2)]) #2020 definition
names(coef_res)<-c("k1","k2","k3","k4","k5","lambda","mu")
coef_res #coef
fit.musse.bras[2] #lnLik
paste("Chromosome limit,",chrmax, ": Process time,", round(timepoint[2]-timepoint[1],3))

###### ASR wih MuSSE M4 model ######
## Ansestral State Reconstruction

print(paste("Start ASR of M4 bras CM",chrmax, "..."))
timepoint<-c(proc.time()[[3]])
st.nodes.bras<-asr.marginal(mussefunc,coef(fit.musse.bras))
timepoint<-c(timepoint,proc.time()[[3]])

## Save the resultant R objecct, st.nodes.neot
filename<-paste("st_nodes_bras_M4_CM",chrmax,"_unisample_XXXXXX.Robj",sep="")
save(st.nodes.bras,file=filename)
