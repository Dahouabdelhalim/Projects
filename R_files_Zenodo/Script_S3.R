...
# Weighted Gene Co-expression Analysis (WGCNA) of T. americanus
...
caste=matrix(c("Follower", "Leader", "Scout", "Scout", "Leader","Follower", "Leader", "Scout", "Follower", "Leader", "Scout", "Follower", "Leader", "Follower", "Scout"), ncol=1)
rownames(caste)=rownames(datExpr0)
colnames(caste)=c("Behavior")

# Cluster with information on behavioral groups
sampleTree2 = hclust(dist(datExpr0), method = "average")
traitColors = labels2colors(caste, colorSeq=c( "mediumturquoise","mediumvioletred","midnightblue")) 
...
bwnet = blockwiseModules(datExpr0, maxBlockSize = 20000,
                         power = 10, TOMType = "unsigned", minModuleSize = 50,
                         reassignThreshold = 0, mergeCutHeight = 0.3,
                         numericLabels = TRUE,
                         saveTOMs = TRUE,
                         saveTOMFileBase = "Tam_unfilt_TOM-blockwise",
                         verbose = 3)
...
group<-as.factor(c("Follower", "Leader", "Scout","Scout", "Leader", "Follower","Leader", "Scout", "Follower","Leader", "Scout", "Follower","Leader", "Follower", "Scout"))

# Create "empty" dataframe
results_all_tam<-data.frame(module=c(rep(3,length(MEs))), pvalue=c(rep(3,length(MEs))), p_fo_le=c(rep(3,length(MEs))), p_fo_sc=c(rep(3,length(MEs))), p_le_sc=c(rep(3,length(MEs))))
...
for (i in 1:length(MEs)){
  ...
  results_all_tam$pvalue[i]<-y$p.value
  
  Wilc = pairwise.wilcox.test(x$moduleeigengene,x$group,p.adjust.method="BH")
  results_all_tam$p_fo_le[i]<-Wilc$p.value[1]
  results_all_tam$p_fo_sc[i]<-Wilc$p.value[2]
  results_all_tam$p_le_sc[i]<-Wilc$p.value[4]
  
  # Create according boxplot of module eigengene
  z<-ggboxplot(x, x = "group", y = "moduleeigengene", 
               color = "group", palette = c( "mediumturquoise","mediumvioletred", "midnightblue"),
  ...
  write.table(w, paste("ModuleContigList_",names(MEs[i]),"_Tam.txt", sep=""), sep="\\t",  row.names = FALSE, quote = FALSE)
  
}

# Write the test results in a separate file
write.table(results_all_tam, file="Results_KruskalWallis_WGCNA_Tam.txt", sep="\\t", row.names = FALSE, quote = FALSE)
