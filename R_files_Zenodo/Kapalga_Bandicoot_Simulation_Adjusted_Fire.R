...
system.time(
  bandicoot_AF_pop_data <- foreach (i = 1:25, .combine = rbind) %do% {
    set.seed(333)
    Coot2 <- simulation(landscape = coot_landscape,
    ...
    pop_data <- return_pop_data(Coot2)
    rm(Coot2)
    gc()
    pop_data
  }
...
save(bandicoot_AF_pop_data, file = "bandicoot_AF_pop_data.RData")

# Plot bandicoot population trajectory of each life-stage
plot(Coot2)

# Plot total bandicoot population trajectory
plot(Coot2, stage = 0, ylim = c(0,35000))

# Plot bandicoot abundance by timestep
plot(Coot2[1], type = "raster", stage = 0, timesteps = 1:126, animate = TRUE)
