...
#MLE
print(paste("Start MLE of otop null2 CM",chrmax, "args..."))
timepoint<-c(proc.time()[[3]])
fit.musse.otop<-find.mle(mussefunc,p)
timepoint<-c(timepoint,proc.time()[[3]])

## Save fit.musse.otop
filename<-paste("fit_musse_otop_M0_prime_CM",chrmax,"_samplingf_XXXXXX.Robj",sep="")
save(fit.musse.otop,file=filename)

## Result of the 6 para Estimation

coef_res<-c(coef(fit.musse.otop)[c(6,5,3,4,1,2)]) #2020 definition
