...
#these are the resolved data (species identification); the unresolved network maintains "genus" only identification possible in the field
wf<-read.csv("visitation_2015R.csv")
...
#now merge with site level dataset (includes pathogen prevalence, agriculture, etc., for each of the 11 sites)

##snsR<-merge(sns,site_indices, by="site")
##write.csv(snsR,file='snsR.csv') 

#merge with pathogen dataset (includes all bees collected and successfully screened for pathogens)
##netpath<-merge(snsR, pathogens, by="site")

##write.csv(netpath,file='netpathR.csv') 

#number of interactions over the summer
WFsum[ WFsum > 0] <- 1
colSums(as.data.frame(colSums(WFsum))) #126 interacting pairs
net<- wf[c(-1,-2,-3)]
colSums(as.data.frame(colSums(net))) #2928
