###########################################################
#######   EnrichmentAnalysisIAAvsGADAxTEDDY.R     #########
###########################################################
...
###########################################################
# Functions:
source ("/media/data/leobalzano/ScriptsForTEDDY/Scripts/loadTEDDYtools.R") # Location of TEDDYtools
...
# Check point 1:
# KEGG results
load("/media/data/leobalzano/ScriptsForTEDDY/Data/Piano/KEGG_GSA_ALL.ro") # This is the result of the gene set enrichment analysis without using a list of genes
load("/media/data/leobalzano/ScriptsForTEDDY/Data/Piano/KEGG_GSA_ALLEFaab.ro")

# KEGG Pathways Database:
load("/media/data/leobalzano/ScriptsForTEDDY/Data/Piano/KEGG_REST_Annotation.ro") # This was downloaded by KEGG database
#KEGG_REST_sets_ <- read.delim("/media/data/leobalzano/ScriptsForTEDDY/Data/Piano/GlobalData/KEGG_REST_sets.txt")


# Genes selected by VIP-NPLSDA
NPLSDA_genes = read.delim("/media/data/leobalzano/ScriptsForTEDDY/Data/Piano/GlobalData/NPLSDA_gene_selection.txt",
                          stringsAsFactors = F,header = F)[,1]

# The KEGG geneset
load("/media/data/leobalzano/ScriptsForTEDDY/Data/Piano/GlobalData/TEDDY_geneSets.ro")


# Check point 2:
load("/media/data/leobalzano/ScriptsForTEDDY/Data/Piano/KEGGGSAFAAbIAAvsGADA.RData")
load("/media/data/leobalzano/ScriptsForTEDDY/Data/Piano/GSC_FaabIAAvsGADA.RData")
...
###########################################################
# First recover all the available Gene Expression data  
# Get Expression Data
LM_globalTargets = loadTargets_v2(SELdisease = "IA",time_point = c(12,9,6,3,0),complete_case = "flexible",targets = GEXtargets) #Uses all samples
RAW_EXPMAT = log2(RAW_EXPMATv2)
LM_globalEXPMAT = getExprmat_v3(RAWEXPRMAT = RAW_EXPMAT,STARGETS = LM_globalTargets, ciNORM = TRUE, gNORM = FALSE,time = unique(LM_globalTargets$time),annotation_column = "SYMBOL") 
...
###########################################################
#After Calculating KEGG Gene Set Enrichment Analysis through Piano
# Which requires time and is computationally demanding.
# We performed the KEGG Analysis: This is the Gene set enrichment analysis, without including a genelist
# For Faab 1 &2
# Time Specific Differential Expression Analysis

ALL_limma_results = getSignatures(cMAT = LM_globalEXPMAT,targetMAT = LM_globalTargets,Features = NULL,time = c(0,3,6,9,12),g.pval = 0.05,s.pval=0.05,model_type = "limma")
...
KEGG_GSA_consensus = plot_PIANO_consensus(GlobalResultsFolder = "/media/data/leobalzano/EnrichmentFaab/",GSA_ALL = KEGG_GSA_ALL,med_Pval = 1)
...
plotGSET_dynamics(GSA_ALL_consensus = KEGG_GSA_consensus,med_Pval = 1,output_pdf = "/media/data/leobalzano/EnrichmentFaab/TESTGSets_dynamics.pdf")
...
summary(KEGG_GSA_ALLEFaab$ALL$g_time_0)
KEGG_GSA_ALLEFaab$ALL$g_time_0$mean$info
KEGG_GSA_ALLEFaab$ALL$g_time_0$mean$nGenesUp
...
KEGGsmall$`Arachidonic acid metabolism`
KEGGsmall$`Arachidonic acid metabolism`<-c("ALOX12", "ALOX5", "ALOX15B", "ALOX12B", "PTGS1")

vecallgenesofPathstep1<-data.frame(unlist(KEGGsmall))
vecallgenesofPath<-vecallgenesofPathstep1$unlist.KEGGsmall.
length(unique(vecallgenesofPath)) # Just 5229 genes are present in the 160 pathways
...
# Subset the data to the mentioned size 5229 genes
vecallgenesofPath2<-as.vector(unique(vecallgenesofPath))  # 5229
...
###################################################
###################################################
# 1) Subsetting the globalTargets Matrix
table(LM_globalTargetsSmallerSubset$FirstAAb)
LM_globalTargetsSmallerSubsetIAAvsGADA<-LM_globalTargetsSmallerSubset[LM_globalTargetsSmallerSubset$outcome==1 & 
                                                                        LM_globalTargetsSmallerSubset$FirstAAb!= 4 &
                                                                        LM_globalTargetsSmallerSubset$FirstAAb!= 5 &
                                                                        LM_globalTargetsSmallerSubset$FirstAAb!= 7,]

dim(LM_globalTargetsSmallerSubsetIAAvsGADA)
LM_globalTargetsSmallerSubsetIAAvsGADA[1:10,]
table(LM_globalTargetsSmallerSubsetIAAvsGADA$outcome)
table(LM_globalTargetsSmallerSubsetIAAvsGADA$FirstAAb)
# 2) Changing name outcome and FirstAAb
colnames(LM_globalTargetsSmallerSubsetIAAvsGADA)[6]
colnames(LM_globalTargetsSmallerSubsetIAAvsGADA)[6]<-"old.outcome"
colnames(LM_globalTargetsSmallerSubsetIAAvsGADA)

colnames(LM_globalTargetsSmallerSubsetIAAvsGADA)[20]
colnames(LM_globalTargetsSmallerSubsetIAAvsGADA)[20]<-"old.FirstAAb"
colnames(LM_globalTargetsSmallerSubsetIAAvsGADA)

# 3) Create column "outcome as "old.FirstAAb" -1 so IAA-first becomes 1 and GADA becomes 0
LM_globalTargetsSmallerSubsetIAAvsGADA$outcome<-LM_globalTargetsSmallerSubsetIAAvsGADA$old.FirstAAb -1
LM_globalTargetsSmallerSubsetIAAvsGADA[1:10,]

# 4) Create column "FirstAAb" by just adding a 1 to all rows
LM_globalTargetsSmallerSubsetIAAvsGADA$FirstAAb<-rep(1,nrow(LM_globalTargetsSmallerSubsetIAAvsGADA))
LM_globalTargetsSmallerSubsetIAAvsGADA[1:10,]

# 5) Subset the EXPMAT
EXPMATIAAvsGADA<-SmallerSubsetEXPMAT[,colnames(SmallerSubsetEXPMAT) %in% LM_globalTargetsSmallerSubsetIAAvsGADA$sample_mask_id]
dim(SmallerSubsetEXPMAT)
dim(EXPMATIAAvsGADA)
dim(LM_globalTargetsSmallerSubsetIAAvsGADA)
...
###################################################
###########     GSEA test IAA vs GADA     #########
###################################################

FAAB_signatures_limma_IAAvsGADA = getSignatures(cMAT = EXPMATIAAvsGADA,targetMAT = LM_globalTargetsSmallerSubsetIAAvsGADA,Features = FAABFeatures,time = c(0,3,6,9,12),g.pval = 0.05,s.pval=0.05,model_type = "limma")
FAAB_signatures = FAAB_signatures_limma_IAAvsGADA$RESULTS
subgroups = names(FAAB_signatures)
...
GSC_FaabIAAvsGADA = data.frame(GSC,stringsAsFactors = F)


#save(GSC_FaabIAAvsGADA,file= "/media/data/leobalzano/EnrichmentFaab/GSC_FaabIAAvsGADA.RData")
summary(GSC_FaabIAAvsGADA)
...
GSA_ALL = MyGLS_ALL2
GSC_FaabIAAvsGADA = loadGSC(GSC_FaabIAAvsGADA)
for(tag in names(MyGLS_ALL2)){
  ...
  for(sGLS in names(TAG_master)){
    print(sGLS)
    GLS = TAG_master[[sGLS]]
    GSA_ALL[tag][[sGLS]] = runMultiGSA(GSC = GSC_FaabIAAvsGADA,GLS = GLS)
  }
}

KEGGGSAFAAbIAAvsGADA = GSA_ALL
#save(KEGGGSAFAAbIAAvsGADA,file= "/media/data/leobalzano/EnrichmentFaab/KEGGGSAFAAbIAAvsGADA.RData")
...
summary(KEGGGSAFAAbIAAvsGADA)


KEGGGSAFAAbIAAvsGADALIST<-list(FirstAAb_1.time_0=KEGGGSAFAAbIAAvsGADA$FirstAAb_1.time_0,
                 FirstAAb_1.time_3=KEGGGSAFAAbIAAvsGADA$FirstAAb_1.time_3,
                 FirstAAb_1.time_6=KEGGGSAFAAbIAAvsGADA$FirstAAb_1.time_6,
                 FirstAAb_1.time_9=KEGGGSAFAAbIAAvsGADA$FirstAAb_1.time_9,
                 FirstAAb_1.time_12=KEGGGSAFAAbIAAvsGADA$FirstAAb_1.time_12)
summary(KEGGGSAFAAbIAAvsGADALIST)

FirstAAb_2<-list(FirstAAb_2.time_0=KEGGGSAFAAbIAAvsGADA$FirstAAb_1.time_0,
                 FirstAAb_2.time_3=KEGGGSAFAAbIAAvsGADA$FirstAAb_1.time_3,
                 FirstAAb_2.time_6=KEGGGSAFAAbIAAvsGADA$FirstAAb_1.time_6,
                 FirstAAb_2.time_9=KEGGGSAFAAbIAAvsGADA$FirstAAb_1.time_9,
                 FirstAAb_2.time_12=KEGGGSAFAAbIAAvsGADA$FirstAAb_1.time_12)
summary(FirstAAb_2)


MyKEGGGSAFAAbIAAvsGADALIST<-list(KEGGGSAFAAbIAAvsGADALIST=KEGGGSAFAAbIAAvsGADALIST,FirstAAb_2=FirstAAb_2)

summary(MyKEGGGSAFAAbIAAvsGADALIST)




# Step 1 
KEGG_GSA_consensus = plot_PIANO_consensus(GlobalResultsFolder = "/media/data/leobalzano/ScriptsForTEDDY/Data/Piano/Results/GADAIAA/",GSA_ALL = MyKEGGGSAFAAbIAAvsGADALIST,med_Pval = 1)

# Generate Consensus analysis
GSC_FaabIAAvsGADA
KEGG_REST_sets<-GSC_FaabIAAvsGADA$gsc
...
summary(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_12)


plotGSET_dynamics(GSA_ALL_consensus = KEGG_GSA_consensus2,med_Pval = 0.1,output_pdf = "/media/data/leobalzano/ScriptsForTEDDY/Data/Piano/Results/GADAIAA/GSets_dynamicsIAAvsGADA_p01.pdf")
...
# Creating Piano Data Frame hacking script so we can play with it at will
dim(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_12$pMat) # 156 Pathways
dim(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_9$pMat) # 90 Pathways
dim(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_6$pMat) # 156 Pathways
dim(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_3$pMat) # 127 Pathways
dim(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_0$pMat) # 156 Pathways


length(unique(c(rownames(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_12$pMat),
                rownames(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_9$pMat),
                rownames(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_6$pMat),
                rownames(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_3$pMat),
                rownames(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_0$pMat))))



AG="KEGGGSAFAAbIAAvsGADALIST"
...
allpaths<-data.frame(unique(c(rownames(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_12$pMat),
                              rownames(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_9$pMat),
                              rownames(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_6$pMat),
                              rownames(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_3$pMat),
                              rownames(KEGG_GSA_consensus2$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_0$pMat))))
rownames(allpaths)<-allpaths$unique.c.rownames.KEGG_GSA_consensus2.KEGGGSAFAAbIAAvsGADALIST.FirstAAb_1.time_12.pMat...
n <- max(dim(GSA_ALL_consensus$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_0$pMat)[1],
         dim(GSA_ALL_consensus$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_3$pMat)[1],
         dim(GSA_ALL_consensus$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_6$pMat)[1],
         dim(GSA_ALL_consensus$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_9$pMat)[1],
         dim(GSA_ALL_consensus$KEGGGSAFAAbIAAvsGADALIST$FirstAAb_1.time_12$pMat)[1])
...
class(pianoDFGADAfirstSelection$TIME)
...
pianoDFGADAfirst
summary(pianoDFGADAfirst)
head(pianoDFGADAfirst)
#save(pianoDFGADAfirst, file="/media/data/leobalzano/EnrichmentFaab/pianoDFGADAfirst.RData")


####

Whicharethepathways (data=pianoDFGADAfirst, threshold = 0.3, Do_you_want_to_know="yes")

thrs<-c(0.01,0.05,0.1)
Whicharethepathways (data=pianoDFGADAfirst, threshold = thrs, Do_you_want_to_know="yes")




Unsupervisedselection (data=pianoDFGADAfirst, threshold=0.1,Heatmaptitle="GADA_First")

################################################################
# Now we are going to plot a set of figures

pdf (file="/media/data/leobalzano/ScriptsForTEDDY/Data/Piano/Results/GADAIAA/testingIAAvsGADA.pdf",height = 11,width = 9)
#### GADA 0.01
plot.new()
...
Unsupervisedselection (data=pianoDFGADAfirst, threshold=0.01,Heatmaptitle="IAAvsGADA alpha=0.01")
plot.new()
...
Unsupervisedselection (data=pianoDFGADAfirst, threshold=0.05,Heatmaptitle="IAAvsGADA alpha=0.05")
plot.new()
#### GADA 0.1
plot.new()
...
Unsupervisedselection (data=pianoDFGADAfirst, threshold=0.1,Heatmaptitle="IAAvsGADA alpha=0.1")
plot.new()
...
Unsupervisedselection (data=pianoDFGADAfirst, threshold=0.2,Heatmaptitle="IAAvsGADA alpha=0.2")
plot.new()

####GADA 0.3
plot.new()
tulo<-Whicharethepathways2 (data=pianoDFGADAfirst, threshold = 0.3, Do_you_want_to_know="yes")
tulo2<-as.data.frame(tulo$Pathways)
mtext(tulo$Sigs)
for (i in 1:nrow(tulo2)) {
  mtext(paste(as.vector(tulo2[i, ]), collapse = "       "), line = -(i +1), adj = 0)
} 
Unsupervisedselection (data=pianoDFGADAfirst, threshold=0.3,Heatmaptitle="IAAvsGADA alpha=0.3")
plot.new()
...
###########################################################
###########################################################
# We decided to select the pathways at FDR=0.3
Whicharethepathways2 (data=pianoDFGADAfirst, threshold = 0.1, Do_you_want_to_know="yes")

unique(pianoDFGADAfirst$GSET)
...
pianoDFALLSmallersubsetselection<-pianoDFGADAfirst[pianoDFGADAfirst$GSET %in% selection,]
# Heatmap #
pselection = ggplot(pianoDFALLSmallersubsetselection, aes(TIME, GSET)) + 
  geom_tile(aes(fill = PVALresc), colour = "transparent") + 
  #geom_tile(aes(fill = PVALresc), colour = ifelse(pianoDFALLselection$PVAL<0.3,"black","transparent"), size=1) +  
  scale_fill_gradientn(colours = colorends, values = rescale(gradientends)) + 
  scale_x_discrete("", expand = c(0, 0)) + 
  scale_y_discrete("", expand = c(0, 0)) + 
  #geom_text(aes(label = round(PVAL, 2))) +   #Turned off for noPval figure
  geom_text(aes(label = ifelse(PVAL<0.3,"*","")), size=9) + # To put asterisk in significant
  #theme_grey(base_size = 18) + 
  theme(legend.position = "none",
        axis.ticks = element_blank(), 
        axis.text.x = element_text(angle = 330, hjust = 0))  + ggtitle(AG)

#pdf (file="pianoDFALLselectionALL109withasterisksbyANA.pdf",height = 11,width = 9)
print(pselection)
#dev.off()
# PaperFormat
# Selection sorted by 
...
# This sorting i liked it 
pianoDFALLSmallersubsetselectionSorted <- pianoDFALLSmallersubsetselection[order(pianoDFALLSmallersubsetselection$directionnumbers, pianoDFALLSmallersubsetselection$PVAL),]


pselectionSorted = ggplot(pianoDFALLSmallersubsetselectionSorted, aes(TIME, GSET)) + 
  geom_tile(aes(fill = PVALresc), colour = "white") + 
  scale_fill_gradientn(colours = colorends, values = rescale(gradientends)) + 
  scale_x_discrete("", expand = c(0, 0)) + 
  scale_y_discrete("", expand = c(0, 0)) + 
...
#pdf (file="/media/data/leobalzano/ScriptsForTEDDY/Data/Piano/Results/GADAIAA/IAAvsGADAfinalHeatmap.pdf",height = 11,width = 9)
print(pselectionSorted)
#dev.off()
