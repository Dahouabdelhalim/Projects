...
phy.o.neot.groupA<-keep.tip(phy.o.neot,14:355)
phy.o.neot.groupA$tip.state<-phy.o.neot$tip.state[14:355]
...
val_chrinfo$OrderSuborder<-paste(val_chrinfo$Order,val_chrinfo$Suborder)
chrindex<-match(phy.o.neot.groupA$tip.label,val_chrinfo$Species_name)
val_chrinfo.groupA<-val_chrinfo[chrindex,]
groupAtaxonID<-unique(val_chrinfo.groupA$OrderSuborder)
val_chrinfo<-val_chrinfo[val_chrinfo$OrderSuborder %in% groupAtaxonID,]

val_chrinfo$State<-factor(val_chrinfo$State,levels=1:snum)
val_chrinfo<-val_chrinfo[!(is.na(val_chrinfo$State)),]
table(val_chrinfo$State)

tipstate<-factor(phy.o.neot.groupA$tip.state,levels=1:snum)
...
###### MLE of MuSSE M2 model ######

mussefunc<-make.musse(phy.o.neot.groupA,phy.o.neot.groupA$tip.state,snum,sampling.f=sampling.f,strict=F)
free.i<-c(tcal(snum,1,2),tcal(snum,2,1),tcal(snum,2,3),tcal(snum,3,2))
free.i<-c(1,sp_state,snum+1,snum+sp_state,((snum*2)+free.i))
cons_and_tar<-get_cons_and_target_musse_ec(chrmax,sp_state)
mussefunc<-constrain.kt(mussefunc,cons_and_tar[[1]],free.i,cons_and_tar[[2]])
int.p<-starting.point.musse(phy.o.neot.groupA,k=snum)
p<-int.p[free.i] #prior
names(p)<-c()

#MLE
print(paste("Start MLE of neot groupA CM",chrmax, " test args...",sep=""))
timepoint<-c(proc.time()[[3]])
fit.musse.neot.groupA<-find.mle(mussefunc,p)
timepoint<-c(timepoint,proc.time()[[3]])

## Save fit.musse.neot.groupA
filename=paste("fit_musse_neot_groupA_M2_CM",chrmax,"_samplingf_XXXXXX.Robj",sep="")
save(fit.musse.neot.groupA,file=filename)

## Result of the 8 para Estimation
coef_res<-c(coef(fit.musse.neot.groupA)[c(8,7,5,6,2,4,1,3)]) #2020 definition
names(coef_res)<-c("k1","k2","k3","k4","lambdaT","muT","lambdaC","muC")
coef_res #coef
fit.musse.neot.groupA[2] #lnLik
