...
setwd("~/Dropbox/Rosvall_Postdoc/Heat stress/WGCNA/Heat Stress PTR/PTR cold vs control")
...
#Read in ZEFI PTR gene counts - can use raw or normalized counts
PTR = read.csv("ZF_PTR_normalizedCounts_noheat.csv")
...
use = rowSums(PTR_normcounts > 10) >=14 # 90% (87.5%) must have at least a normalized count of 10
countMatrixFiltered = PTR_normcounts[ use, ]
dim(countMatrixFiltered) # 12395     16
...
dim(Filtered) # Left with 9296  genes out of 16015
write.csv(Filtered,file="ZF_PTR_normCounts_coldvscontrol_logreduced25.csv")
...
########################################### Read in trait data ##########################################
traitData = read.csv("ZF_heat_traits_WGCNA_noheat.csv");
dim(traitData) # 16 31
names(traitData)
...
datExpr<-read.csv("ZF_PTR_normCounts_coldvscontrol_logreduced25.csv")
dim(datExpr) # 9296   17
...
# Unsure if we should merge modules
#save(MEs, moduleLabels, moduleColors, geneTree, file = "ZF_PTR_coldvscontrol_normcounts_nocold_unmerged_modules.RData")
...
save(MEs, moduleLabels, moduleColors, geneTree, file = "ZF_PTR_coldvscontrol_normcounts_modules.RData")
...
table(moduleColors)
# moduleColors
# black        blue greenyellow        grey     magenta      purple         red      salmon 
# 125         765          78        7335          94          88         154          41 
# tan   turquoise      yellow 
# 46         386         184 
write.csv(MEs, file = "ZF_PTR_coldvscontrol_normcounts_eigengenes_modules.csv")
...
#GS and MM are highly correlated (cor = 0.32, p = 0.0017), illustrating that genes highly significantly
...
write.csv(geneInfo, file = "ZF_PTR_cold_genename_piloerect.csv")
...
chooseTopHubInEachModule(datExpr0,colorh = "turquoise") # can't get this to work... out put is 8168 for both
# 1193
