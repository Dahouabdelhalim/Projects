################################################################################
#Fig 8
#Hourly production and consumption profiles for 3 scenarios
################################################################################
            ...
            for (scen in scenariosq){
              tryCatch({ 
                if (load=="2007" & ev!="half"){
                  ...
                  #ymax <- 3000
                  ifelse(cat=="rps",renew <- "100% Renewable", ifelse(cat=="free", renew <- "Unconstrained", renew <- cat))
                  ifelse(ev=="2016",evl <- "0.5%", ifelse(ev=="half", evl <- "50%", evl <- "100%"))
                  #ifelse(substr(thetaval, 1,1)=="0",thet <- paste("0.",substr(thetaval, 2,2),sep=""), thet <- thetaval)
                  ...
                  pdf(paste("plot/Fig8_base3theta=",thetaval,"load",load,"ev",ev,cat,cost,flatdyn,"scen",scen,"b.pdf",sep=""), height = 6, width = 6)
                  print(paste(base,"3theta=",thetaval,"load",load,"ev",ev,cat,cost,flatdyn,"scen",scen,".pdf",sep=""))
                  par(mfrow=(c(2,1)))
                  ...
                  p1 <- ggplot(dfpm[order(dfpm$source),], aes(monthhour, value, fill = source, order = rev(source))) +
                    ...
                    scale_fill_manual(name="Source",
                                      values=c(LSFO="#CCCCCC", Biodiesel="#99CC00", Diesel="#999999", Coal="#000000",
                                               ...
                                               Wind="#3399FF", SolarPV="#FFFF00", DischargeBattery="#990000",FuelCell="#CCFFCC",
                    ...
                    theme_classic()+
                    theme(axis.text.x=element_blank(), axis.ticks=element_blank(), legend.text = element_text(size=8),
                          plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5),legend.key.size = unit(0.5, 'cm'),
                  ...
                  dfp <- transform(dfp, monthhour=seq(1,nrow(dfp)))
                  dfpm <- melt(dfp, id=c("month","day","hour","monthhour"))
                  dfpm <- transform(dfpm, labelbase=ifelse(hour==5 & month==1 & variable=="base_price","Base price",""))
