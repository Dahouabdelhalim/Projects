...
#MLE
print(paste("Start MLE of neot MuSSE M1 CM",chrmax, "..."))
timepoint<-c(proc.time()[[3]])
fit.musse.neot<-find.mle(mussefunc,p)
timepoint<-c(timepoint,proc.time()[[3]])

## Save fit.musse.neot
filename<-paste("fit_musse_neot_M1_CM",chrmax,"_unisample_XXXXXX.Robj",sep="")
save(fit.musse.neot,file=filename)

## Result of the 5 para Estimation

coef_res<-c(coef(fit.musse.neot)[c(5,4,3,1,2)]) #2020 definition
