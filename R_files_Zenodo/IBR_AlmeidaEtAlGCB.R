#'@title Individual based rarefaction analysis for Almeida et al. GCB
...
#'Conduct an individual based rarefaction
#'
#'@param comm a site by species matrix for an individual site (i.e. one row)
#'@param samplesize an integer representing subsample size
#'@return a species x simulation matrix 
#'
#'@description a site with 50 spp and 200 individuals rarefied to sample size 75
#' will return a matrix with 50 rows and 1000 columns, with each column
#' a rarefaction simulation in which the original community has had 125 individuals
#' randomly removed
individual_based_rarefaction <- function(comm, samplesize){
  
  if (sum(comm) < samplesize){
    samplesize <- sum(comm)
  }
  
  #Get abundance of bag
  Precomm_abundance  <- as.numeric(comm)
  
  #Translate abundance into list of individuals denoted by species number
  Precomm_dist  <- rep(1:length(Precomm_abundance), times = Precomm_abundance)
  
  #Create empty matrix to hold 1000 random samples 
  comm_sim <- matrix(nrow = samplesize, ncol = 1000)
  
  #randomly sample individuals at post-disturbance abundance level 1000 times
  #each individual presented by species num (e.g. [1, 1, 1, 1, 2, 2, 3, 4, 5, 5])
  for (i in 1:1000){
    comm_random  <- sample(x = Precomm_dist, size = samplesize, replace = F, prob = NULL)
    comm_sim[,i] <- cbind(comm_random) }
  
  #create matrix to hold all iterations with abundance values
  comm_simFull  <- matrix(nrow = ncol(comm), ncol = ncol(comm_sim)+1)
  
  #make first column species numbers
  comm_simFull[,1] <- 1:ncol(comm)
  
  #create matrix with random samples in format spp x iteration
  for (j in 1:(ncol(comm_sim))) {
    comm_sp  <- count(df = comm_sim[,j])
    for (i in 1:nrow(comm_simFull)){
      if (comm_simFull[i,1] %in% comm_sp[,1]){
        pos  <- which(comm_sp == comm_simFull[i,1], arr.ind = T)
        comm_simFull[i,j+1]  <- comm_sp[pos[1],2] }
      else {
        comm_simFull[i, j+1]  <- 0}
      i = 0}}
  
  comm_simFull  <- comm_simFull[,-1]
  rownames(comm_simFull) <- colnames(comm)
  return(comm_simFull)
}

#'Determine expected local richness based on individual based rarefaction
#'
#'@param IBR_comm a rarefied community matrix,
#'i.e. the output of individual_based_rarefaction function
#'@return a matrix containing 1000 rarefied richness values
#'
#'@discription mean of output matrix is expected richness following
#'rarefaction
IBR_richness <- function(IBR_comm){
  IBR_comm_total_sp_richness <- 0
  IBR_comm_rarefaction_matrix <- matrix(nrow = 1, ncol = 1000)
  
  for (i in 1:1000){
    for (j in 1:nrow(IBR_comm)){
      if (IBR_comm[j, i] > 0){
        IBR_comm_total_sp_richness  <- IBR_comm_total_sp_richness + 1}
      IBR_comm_rarefaction_matrix[,i] <- IBR_comm_total_sp_richness}
    IBR_comm_total_sp_richness <- 0
    j <- 0}
  
  return(IBR_comm_rarefaction_matrix)
}

#'Determine expected regional richness based on individual based rarefaction
#'
#'@param IBR_region a list containing the rarefied community matrix for
#'each site in a region (i.e. the output of individual_based_rarefaction for
#'each site)
#'@return a matrix containing 1000 rarefied richness values
#'
#'@discription mean of output matrix is expected richness following
#'rarefaction
IBR_Regional_richness <- function(region){
  RegSp <- 0
  IBR_RegionalRichness_matrix <- matrix(nrow = 1, ncol = 1000)
  sp_counter <- 0
  ...
  for (j in 1:1000){
    for (i in 1:nrow(region[[1]])){
      ...
      sp_counter <- 0}
    IBR_RegionalRichness_matrix[,j] <- RegSp
    RegSp <- 0}
  
  
  
  return(IBR_RegionalRichness_matrix)
}

#' Determine local Pe values based on individual based rarefaction
#' 
#' @param IBR_comm a rarefied community matrix,
#'i.e. the output of individual_based_rarefaction function
...
IBR_LocalPe <- function(IBR_comm){
  IBR_locEx <- matrix(ncol = nrow(IBR_comm), nrow = 1)
  ex_num  <- 0
  
  for (i in 1:ncol(IBR_locEx)){
    for (j in 1:1000){
      if (IBR_comm[i,j] == 0){
        ex_num <- ex_num + 1}}
    IBR_locEx[1,i] <- ex_num/1000
    ex_num <- 0}
  
  colnames(IBR_locEx) <- rownames(IBR_comm)
  return(IBR_locEx)
}

#'Determine regional Pe values based on individual based rarefaction
#'
#'@param IBR_region a list containing the rarefied community matrices for
#'each site in a region (i.e. the output of individual_based_rarefaction for
#'each site)
...
#'@description Pe values are the proportion of times a species was
#'simulated to go extinct across 1000 rarefaction simulations
IBR_RegionalPe <- function(IBR_region){
  IBR_regEx <- matrix(nrow = 1, ncol = nrow(IBR_region[[1]]))
  ex_num  <- 0
  ex_p_value <- 0
  
  
  for (i in 1:ncol(IBR_regEx)){
    for (j in 1:1000){
      for (x in IBR_region){
        if (x[i, j] == 0){
          ex_num <- ex_num + 1}}
      if (ex_num == length(IBR_region)){
        ex_p_value <- ex_p_value + 1
        ex_num <- 0}
      else{
        ex_num <- 0}}
    IBR_regEx[1,i] <- ex_p_value/1000
    ex_p_value <- 0}
  
  colnames(IBR_regEx) <- rownames(IBR_region[[1]])
  return(IBR_regEx)
}
...
library(plyr) #plyr needed for individual based rarefaction, NOT dplyr
#Create list where each element will be a rarefied site
Treatment_Rarefactions_Region <- vector("list", nTreatments)

colnames(Pre_Treatment_Region) <- NULL

#Conduct rarefaction simulation
for (i in 1:nTreatments){ #for each site
  Treatment_Rarefactions_Region[[i]] <- individual_based_rarefaction(t(Pre_Treatment_Region[,i]), sum(Post1_Treatment_Region_NoCol[,i])) #rarefy each site to post-disturbance abundance
}
...
for (i in 1:nTreatments){
  NullTreatmentPost1_Richnesses$richness[i] <- mean(IBR_richness(Treatment_Rarefactions_Region[[i]]))
}
...
NullPost1RegionalRichness$richness <- mean(IBR_Regional_richness(Treatment_Rarefactions_Region))
...
NullPost2RegionalRichness$richness <- mean(IBR_Regional_richness(Treatment_Rarefactions_Region))
...
for (i in 1:length(Treatment_Rarefactions_Region)){
  TreatmentPostLocalPe[,i] <- t(IBR_LocalPe(Treatment_Rarefactions_Region[[i]]))
}
...
### Regional Pe ----
TreatmentRegionalPe <- t(IBR_RegionalPe(Treatment_Rarefactions_Region))
...
for (i in 1:nTreatments){ # for each treatment site
  ...
  vals <- IBR_richness(site) #extract null richness estimates for each site
...
RegionalPost1Richness <- IBR_Regional_richness(Treatment_Rarefactions_Region) #calclate regional rarefied richness estimates
...
write.csv(TotalRichnesses, "IBR_TotalRichness_AlmeidaEtAlGCB.csv")
write.csv(TotalExtinctions, "IBR_TotalExtinction_AlmeidaEtAlGCB.csv")
write.csv(TotalPe, "IBR_TotalPe_AlmeidaEtAlGCB.csv")
write.csv(TotalNull95CI, "IBR_TotalNull95CI_AlmeidaEtAlGCB.csv")
