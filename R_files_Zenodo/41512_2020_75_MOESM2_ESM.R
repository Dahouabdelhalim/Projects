...
#name the data files
data_train_file = "GADModelV2.dta"
data_validate_file = "YDXValidationGADModelData.dta"

#load Stata datasets
data_train = read.dta13(data_train_file)
updatedDare <- read_excel(dataFile3Bis)
data_train <-  data_train %>% left_join(updatedDare, by = c("ID" = "Patient ID")) 


dataset_val =  read.dta13(data_validate_file)
...
#divide each variable importance scores by max to scale
# Yourcovariate1 = c(1.00,0.99,0.43,0.97)
# Yourcovariate2 = c(0.73, 0.53, 0.65, 1.00)
# Yourcovariate3 = c(0.99,1.00,1.00, 0.91)
# varImpDF = data.frame(Model,Yourcovariate1,Yourcovariate2,Yourcovariate3)
# 
# #build the plots of the variable importance ranks
# plotVarImp1 = ggplot(data = varImpDF, aes(x = Model, y = varImpDF$Yourcovariate1))+geom_bar(stat="identity",width=0.06)+ coord_flip()+ ylab("Scaled variable importance score")+
#   xlab("") + ggtitle("Your covariate 1") + scale_y_continuous(expand = c(0, 0)) +theme(axis.text.y = element_blank(),axis.ticks.y = element_blank() )
# 
# plotVarImp2 = ggplot(data = varImpDF, aes(x = Model, y = varImpDF$Yourcovariate2))+geom_bar(stat="identity",width=0.06)+ coord_flip()+ ylab("Scaled variable importance score")+
#   xlab("") + ggtitle("Your covariate 2") + scale_y_continuous(expand = c(0, 0)) +theme(axis.text.y = element_blank(),axis.ticks.y = element_blank() )
# 
# plotVarImp3 = ggplot(data = varImpDF, aes(x = Model, y = varImpDF$Yourcovariate3))+geom_bar(stat="identity",width=0.06)+ coord_flip()+ ylab("Scaled variable importance score")+
#   xlab("") + ggtitle("Your covariate 3") + scale_y_continuous(expand = c(0, 0)) +theme(axis.text.y = element_blank(),axis.ticks.y = element_blank() )
# 
# plotVarImp3 = ggplot(data = varImpDF, aes(x = Model, y = varImpDF$Yourcovariate3))+geom_bar(stat="identity",width=0.06)+ coord_flip()+ ylab("Scaled variable importance score")+
#   xlab("") + ggtitle("Your covariate 3") + scale_y_continuous(expand = c(0, 0)) +theme(axis.text.y = element_blank(),axis.ticks.y = element_blank() )
# 
