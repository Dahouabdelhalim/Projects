...
# only Rencell to be analyzed here, and exclude house-keeping gene (this is just for normalization)
qPCR_data_Rencell = qPCR_data[qPCR_data$Cell == "Rencell" & qPCR_data$Gene != "GAPDH",]

aov_for_overall_variance=aov(log_expression~paste(Substrate,Gene),qPCR_data_Rencell[ qPCR_data_Rencell$Substrate != "TCP",])
...
qPCR_data_Rencell$Substrate = factor(qPCR_data_Rencell$Substrate,levels=c("TCP","PDMS","imprinted"))

qPCR_data_Rencell_agg=aggregate(Normalized_to_TCP_control ~ Substrate+Gene,qPCR_data_Rencell,FUN=mean)
qPCR_data_Rencell_agg_sd=aggregate(Normalized_to_TCP_control ~ Substrate+Gene,qPCR_data_Rencell,FUN=sd)

for_barplot=matrix(nrow=length(unique(qPCR_data_Rencell_agg$Gene)),ncol=length(unique(qPCR_data_Rencell_agg$Substrate)))

rownames(for_barplot)=c("Nestin","Tuj1","MAP2","Synapsin")
colnames(for_barplot)=unique(qPCR_data_Rencell_agg$Substrate)
...
for(theSubstrate in colnames(for_barplot))
{
    ...
    {
        for_barplot[theGene,theSubstrate]=qPCR_data_Rencell_agg$Normalized_to_TCP_control[qPCR_data_Rencell_agg$Gene==theGene & qPCR_data_Rencell_agg$Substrate==theSubstrate]
        for_barplot_sd[theGene,theSubstrate]=qPCR_data_Rencell_agg_sd$Normalized_to_TCP_control[qPCR_data_Rencell_agg_sd$Gene==theGene & qPCR_data_Rencell_agg_sd$Substrate==theSubstrate]
        if(theSubstrate=="TCP")
        {
...
barplot_with_errorbars(t(for_barplot),sd_height=t(for_barplot_sd),beside=TRUE,col=grey(c(1,0.5,0.2)),sig_codes=t(significance_labels(for_barplot_pval)),sig.cex=3,xlab="Gene",ylab="Gene expression normalized to GAPDH, relative to TCP",ylim=c(0,3))
...
genes_for_efficiency_assessment_PDMS = unique(gsub("imprinted ","",gsub("PDMS ","",names(pvals)))[pvals<=0.05])

qPCR_efficiency_PDMS = qPCR_data_Rencell[qPCR_data_Rencell$Gene %in% genes_for_efficiency_assessment_PDMS,]
...
summary(glht(a1,linfct=mcp(Substrate="Tukey")))
