##################################################################################################
#######################     New Functions for Multiway Package     ###############################
##################################################################################################
# Author: Leandro Balzano-Nogueira
# These are the functions created for Multiway package
...
tucker3mod2<-function (X, COMP, conver = 1e-07, max.iter = 10000, verbose = FALSE) 
{
  ...
  for (it in 1:max.iter) {
    ...
    #tdfmalo<-sum(Adf,Bdf,Cdf) # tdf= Total degrees of Freedom
...
plsreg<-function (predictors, responses, comps = 2, crosval = TRUE) 
{
  ...
  #Br = diag(1/ricky) %*% Bs %*% diag(apply(Y, 2, sd))
...
NPLSDAmod<- function (XN, YN, outcome.Y=NULL, factors=3, COMP= c(2,2,2), conver = 1e-16, max.iteration = 10000, centering=2) 
{
  ...
  if (Ydim[2]>1){   # This part performs the centering in matrix X and Y
    ...
    #####
    # Evaluation of the model Vars
    outcome.YY<-cbind(outcome.Y[,,1],outcome.Y[,,1])
    ...
    PLSmodelX<-try(plsreg(predictors = X, responses=outcome.YY, comps = factors, crosval = TRUE) )
    if (class(PLSmodelX) == "try-error") { 
      Xfinal <-X[, colSums(X) != 0 ]
      PLSmodelX<-plsreg(predictors = Xfinal, responses=outcome.YY, comps = factors, crosval = TRUE)
    } else{
        print("NPLSDA regression of Block X cannot be calculated")
      }
    
    YpredX<-PLSmodelX$y.pred
    Explvar2DX<-PLSmodelX$expvar
    VIP2DX<-PLSmodelX$VIP 
    tucorrX<-PLSmodelX$cor.tu  
    residualsX<-PLSmodelX$resid
    Q22DX<-PLSmodelX$Q2
    Q2cum2DX<-PLSmodelX$Q2cum
    
    
    PLSmodelY<-try(plsreg(predictors = Y, responses=outcome.YY, comps = factors, crosval = TRUE) )
    if (class(PLSmodelY) == "try-error") { 
      Yfinal <-Y[, colSums(Y) != 0 ]
      PLSmodelY<-plsreg(predictors = Yfinal, responses=outcome.YY, comps = factors, crosval = TRUE)
    } else{
      print("NPLSDA regression of Block Y cannot be calculated")
    }
    
    
    YpredY<-PLSmodelY$y.pred
    Explvar2DY<-PLSmodelY$expvar
    VIP2DY<-PLSmodelY$VIP 
    tucorrY<-PLSmodelY$cor.tu  
    residualsY<-PLSmodelY$resid
    Q22DY<-PLSmodelY$Q2
    Q2cum2DY<-PLSmodelY$Q2cum
    ...
    PLSmodelXvsR<-try (plsreg(predictors = A3D[,,i], responses=outcome.YY, comps = factors, crosval = TRUE) )
    if (class(PLSmodelXvsR) == "try-error") { 
      Xfinal <-A3D[,,i][, colSums(A3D[,,i]) != 0 ]
      PLSmodelXvsR<-plsreg(predictors = Xfinal, responses=outcome.YY, comps = factors, crosval = TRUE)
    } else{
      print("NPLSDA regression of Block X by time point cannot be calculated")
    }
    explvar3dtitaX<-PLSmodelXvsR$expvar
    nameexplvar <- paste('item:',i,sep='')
    explvar3dsasX<-list(explvar3dtitaX)
    explvar3dX[[nameexplvar]]<-explvar3dsasX
    #
    VIP3DtitaX<-PLSmodelXvsR$VIP 
    namevip <- paste('item:',i,sep='')
    vip3dsasX<-list(VIP3DtitaX)
    vip3dX[[namevip]]<-vip3dsasX
    #
    Q23DtitaX<-PLSmodelXvsR$Q2
    nameQ23D <- paste('item:',i,sep='')
    Q23DsasX<-list(Q23DtitaX)
    Q23DX[[nameQ23D]]<-Q23DsasX
    #
    Q23DcumtitaX<-PLSmodelXvsR$Q2cum
    nameQ23Dcum <- paste('item:',i,sep='')
    Q23DcumsasX<-list(Q23DcumtitaX)
    Q23DcumX[[nameQ23Dcum]]<-Q23DcumsasX
    #### Y
    #Yfinal <-B3D[,,i][, colSums(B3D[,,i]) != 0 ]
    
    PLSmodelYvsR<-try (plsreg(predictors = B3D[,,i], responses=outcome.YY, comps = factors, crosval = TRUE) )
    if (class(PLSmodelYvsR) == "try-error") { 
      Yfinal <-B3D[,,i][, colSums(B3D[,,i]) != 0 ]
      PLSmodelYvsR<-plsreg(predictors = Yfinal, responses=outcome.YY, comps = factors, crosval = TRUE)
    } else{
      print("NPLSDA regression of Block Y by time point cannot be calculated")
    }
    explvar3dtitaY<-PLSmodelYvsR$expvar
    nameexplvar <- paste('item:',i,sep='')
    explvar3dsasY<-list(explvar3dtitaY)
    explvar3dY[[nameexplvar]]<-explvar3dsasY
    
    #
    VIP3DtitaY<-PLSmodelYvsR$VIP 
    namevip <- paste('item:',i,sep='')
    vip3dsasY<-list(VIP3DtitaY)
    vip3dY[[namevip]]<-vip3dsasY
    
    for (ttt in 1: length(vip3dY)){
      additionalVars<-setdiff (colnames(B3D[,,ttt]),rownames(vip3dY[[ttt]][[1]]) )
      addition<-matrix(data=0,nrow = length(additionalVars),ncol = ncol(vip3dY[[ttt]][[1]])  )
      rownames(addition)<-additionalVars
      
      vip3dY[[ttt]][[1]]<-rbind(vip3dY[[ttt]][[1]],addition)
      
    }
    #
    Q23DtitaY<-PLSmodelYvsR$Q2
    nameQ23D <- paste('item:',i,sep='')
    Q23DsasY<-list(Q23DtitaY)
    Q23DY[[nameQ23D]]<-Q23DsasY
    #
    Q23DcumtitaY<-PLSmodelYvsR$Q2cum
    nameQ23Dcum <- paste('item:',i,sep='')
    Q23DcumsasY<-list(Q23DcumtitaY)
    Q23DcumY[[nameQ23Dcum]]<-Q23DcumsasY
    
    }
  
    R2XmeanX<-NULL
    R2RespmeanX<-NULL
    for (i in 1:length(explvar3dX)) {
      a=as.matrix(explvar3dX[i][[1]][[1]][,1])
      R2XmeanX<-cbind(R2XmeanX,a)
      b=as.matrix(explvar3dX[i][[1]][[1]][,3])
      R2RespmeanX<-cbind(R2RespmeanX,b)
    }
    R2Xmean3DX<-as.matrix(rowMeans(R2XmeanX)); colnames(R2Xmean3DX)<-"R2Xmean3DX"
    R2RespmeanX<-as.matrix(rowMeans(R2RespmeanX)); colnames(R2RespmeanX)<-"R2RespmeanX"
    Explvar3DX<-cbind(R2Xmean3DX,R2RespmeanX)
    ## Y VS R
    R2XmeanY<-NULL
    R2RespmeanY<-NULL
    for (i in 1:length(explvar3dY)) {
      a=as.matrix(explvar3dY[i][[1]][[1]][,1])
      R2XmeanY<-cbind(R2XmeanY,a)
      b=as.matrix(explvar3dX[i][[1]][[1]][,3])
      R2RespmeanY<-cbind(R2RespmeanY,b)
    }
    R2Xmean3DY<-as.matrix(rowMeans(R2XmeanY)); colnames(R2Xmean3DY)<-"R2Xmean3DY"
    R2RespmeanY<-as.matrix(rowMeans(R2RespmeanY)); colnames(R2RespmeanY)<-"R2RespmeanY"
    Explvar3DY<-cbind(R2Xmean3DY,R2RespmeanY)
    
    ###############################################
    # Calculating Q2mean3D
    Q2meanX<-NULL
    for (i in 1:length(Q23DX)) {
      a=as.matrix(Q23DX[i][[1]][[1]][,1])
      Q2meanX<-cbind(Q2meanX,a)
    }
    Q2mean3DX<-as.matrix(rowMeans(Q2meanX)); colnames(Q2mean3DX)<-"Q2mean3DX"
    # Y vs Response
    Q2meanY<-NULL
    for (i in 1:length(Q23DY)) {
      a=as.matrix(Q23DY[i][[1]][[1]][,1])
      Q2meanY<-cbind(Q2meanY,a)
    }
    Q2mean3DY<-as.matrix(rowMeans(Q2meanY)); colnames(Q2mean3DY)<-"Q2mean3DY"
    ###############################################
    # Calculating Q2cummean3D
    Q2cummeanX<-NULL
    for (i in 1:length(Q23DcumX)) {
      a=as.matrix(Q23DcumX[i][[1]][[1]][,1])
      Q2cummeanX<-cbind(Q2cummeanX,a)
    }
    Q2cummean3DX<-as.matrix(rowMeans(Q2cummeanX)); colnames(Q2cummean3DX)<-"Q2cummean3DX"
    ## Y VS Resp
    Q2cummeanY<-NULL
    for (i in 1:length(Q23DcumY)) {
      a=as.matrix(Q23DcumY[i][[1]][[1]][,1])
      Q2cummeanY<-cbind(Q2cummeanY,a)
    }
    Q2cummean3DY<-as.matrix(rowMeans(Q2cummeanY)); colnames(Q2cummean3DY)<-"Q2cummean3DY"
    
    ####################################################################################
    
    t1meanX<-t2meanX<-t3meanX<-t4meanX<-NULL
    t1meanY<-t2meanY<-t3meanY<-t4meanY<-NULL
   if(factors==2) {
      for (i in 1:length(vip3dX)) {
      t1X=as.matrix(vip3dX[[i]][[1]][,1])
      t1meanX<-cbind(t1meanX,t1X)
      t2X=as.matrix(vip3dX[[i]][[1]][,2])
      t2meanX<-cbind(t2meanX,t2X)
      }
     t1mean3DX<-as.matrix(rowMeans(t1meanX)); colnames(t1mean3DX)<-"t1mean3DX"
     t2mean3DX<-as.matrix(rowMeans(t2meanX)); colnames(t2mean3DX)<-"t2mean3DX"
     VIP3Dmodel1X<-cbind(t1mean3DX,t2mean3DX)
     
     # Y
     for (i in 1:length(vip3dY)) {
     t1Y=as.matrix(vip3dY[[i]][[1]][,1])
     t1meanY<-cbind(t1meanY,t1Y)
     t2Y=as.matrix(vip3dY[[i]][[1]][,2])
     t2meanY<-cbind(t2meanY,t2Y)
     }
    t1mean3DY<-as.matrix(rowMeans(t1meanY)); colnames(t1mean3DY)<-"t1mean3DY"
    t2mean3DY<-as.matrix(rowMeans(t2meanY)); colnames(t2mean3DY)<-"t2mean3DY"
    VIP3Dmodel1Y<-cbind(t1mean3DY,t2mean3DY)
    }
    ...
    if(factors==3) {
      for (i in 1:length(vip3dX)) {
        ...
        t2X=as.matrix(vip3dX[[i]][[1]][,2])
        t2meanX<-cbind(t2meanX,t2X)
        t3X=as.matrix(vip3dX[[i]][[1]][,3])
        t3meanX<-cbind(t3meanX,t3X)
      }
      t1mean3DX<-as.matrix(rowMeans(t1meanX)); colnames(t1mean3DX)<-"t1mean3DX"
      t2mean3DX<-as.matrix(rowMeans(t2meanX)); colnames(t2mean3DX)<-"t2mean3DX"
      t3mean3DX<-as.matrix(rowMeans(t3meanX)); colnames(t3mean3DX)<-"t3mean3DX"
      VIP3Dmodel1X<-cbind(t1mean3DX,t2mean3DX,t3mean3DX)
      ...
      for (i in 1:length(vip3dY)) {
        ...
        t2Y=as.matrix(vip3dY[[i]][[1]][,2])
        t2meanY<-cbind(t2meanY,t2Y)
        t3Y=as.matrix(vip3dY[[i]][[1]][,3])
        t3meanY<-cbind(t3meanY,t3Y)
      }
      t1mean3DY<-as.matrix(rowMeans(t1meanY)); colnames(t1mean3DY)<-"t1mean3DY"
      t2mean3DY<-as.matrix(rowMeans(t2meanY)); colnames(t2mean3DY)<-"t2mean3DY"
      t3mean3DY<-as.matrix(rowMeans(t3meanY)); colnames(t3mean3DY)<-"t3mean3DY"
      VIP3Dmodel1Y<-cbind(t1mean3DY,t2mean3DY,t3mean3DY)
    } 
    if(factors==4) {
      for (i in 1:length(vip3dX)) {
        t1X=as.matrix(vip3dX[[i]][[1]][,1])
        ...
        t3X=as.matrix(vip3dX[[i]][[1]][,3])
        t3meanX<-cbind(t3meanX,t3X)
        t4X=as.matrix(vip3dX[[i]][[1]][,4])
        t4meanX<-cbind(t4meanX,t4X)
      }
      t1mean3DX<-as.matrix(rowMeans(t1meanX)); colnames(t1mean3DX)<-"t1mean3DX"
      t2mean3DX<-as.matrix(rowMeans(t2meanX)); colnames(t2mean3DX)<-"t2mean3DX"
      t3mean3DX<-as.matrix(rowMeans(t3meanX)); colnames(t3mean3DX)<-"t3mean3DX"
      t4mean3DX<-as.matrix(rowMeans(t4meanX)); colnames(t4mean3DX)<-"t4mean3DX"
      VIP3Dmodel1X<-cbind(t1mean3DX,t2mean3DX,t3mean3DX,t4mean3DX)
      ...
      for (i in 1:length(vip3dY)) {
        ...
        t3Y=as.matrix(vip3dY[[i]][[1]][,3])
        t3meanY<-cbind(t3meanY,t3Y)
        t4Y=as.matrix(vip3dY[[i]][[1]][,4])
        t4meanY<-cbind(t4meanY,t4Y)
      }
      t1mean3DY<-as.matrix(rowMeans(t1meanY)); colnames(t1mean3DY)<-"t1mean3DY"
      t2mean3DY<-as.matrix(rowMeans(t2meanY)); colnames(t2mean3DY)<-"t2mean3DY"
      t3mean3DY<-as.matrix(rowMeans(t3meanY)); colnames(t3mean3DY)<-"t3mean3DY"
      t4mean3DY<-as.matrix(rowMeans(t4meanY)); colnames(t4mean3DY)<-"t4mean3DY"
      VIP3Dmodel1Y<-cbind(t1mean3DY,t2mean3DY,t3mean3DY,t4mean3DY)
    } 
    ##########################################################################################################
    ...
    timetableX<-NULL
    for (i in 1:length(vip3dX)) {
      timeX<- as.matrix(rowSums(vip3dX[i][[1]][[1]]))
      timetableX<-cbind(timetableX,timeX)
    }
    VIP3Dmodel2X<-timetableX
    
    # Y
    timeY<-NULL
    timetableY<-NULL
    for (i in 1:length(vip3dY)) {
      timeY<- as.matrix(rowSums(vip3dY[i][[1]][[1]]))
      timetableY<-cbind(timetableY,timeY)
    }
    VIP3Dmodel2Y<-timetableY
    
    #########
    #RearrangementENV<-cbind(RearrangementENV,piece)
    xdimA3D<-dim(A3D)
    #dim(RearrangementGEN)
    #dim(A)
    A<-matrix(A3D, xdimA3D[1], xdimA3D[2] * xdimA3D[3])
    xdimB3D<-dim(B3D)
    B<-matrix(B3D, xdimB3D[1], xdimB3D[2] * xdimB3D[3])
    plsdaforplotting<-block.plsda (X=list(Block.X=A, Block.Y=B), Y = outcome.Y[,,1], ncomp=factors)
    ...
    ##########
    #plotIndiv(plsdaforplotting,ind.names = F)
    ...
    for (i in 1:dim(A3D)[3]) {
      plsdaforplotting<-block.plsda (X=list(Block.X=A3D[,,i], Block.Y=B3D[,,i]), Y = outcome.Y[,,1], ncomp=factors)
    ...
    Block.XConsensus<-cbind(comp1mean3D,comp2mean3D)
    ...
    Block.YConsensus<-cbind(Ycomp1mean3D,Ycomp2mean3D)
    ...
    Block.XConsensus<-cbind(comp1meanloadings3D,comp2meanloadings3D)
    ...
    Block.YConsensus<-cbind(Ycomp1meanloadings3D,Ycomp2meanloadings3D)
    ...
    result <- list(FactorsX = FactorsX, FactorsY = FactorsY, 
                   T = Tt, WsupraJ = WsupraJ, WsupraK = WsupraK, U = U, QsupraJ = QsupraJ, 
                   QsupraK = QsupraK, B = B, Gu = Gu, G = G,YpredX =YpredX, YpredY =YpredY,
                   Explvar2DX=Explvar2DX, Explvar2DY=Explvar2DY,
                   VIP2DX=VIP2DX, VIP2DY=VIP2DY,
                   Explvar3DX=Explvar3DX, Explvar3DY=Explvar3DY,
                   VIP3Dmodel1X=VIP3Dmodel1X,VIP3Dmodel1Y=VIP3Dmodel1Y,
                   VIP3Dmodel2X=VIP3Dmodel2X, VIP3Dmodel2Y=VIP3Dmodel2Y,
                   tucorrelationX=tucorrX, tucorrelationY=tucorrY,
                   ResidualsX=residualsX, ResidualsY=residualsY,
                   ...
                   NPLSDAQ22DX=Q22DX, NPLSDAQ22DY=Q22DY,
                   NPLSDAQ2cum2DX=Q2cum2DX, NPLSDAQ2cum2DY=Q2cum2DY,
                   NPLSDAQ2mean3DX=Q2mean3DX,NPLSDAQ2mean3DY=Q2mean3DY,
                   NPLSDAQ2cummean3DX=Q2cummean3DX,NPLSDAQ2cummean3DY=Q2cummean3DY,
  ...
  if (Ydim[2]==1){   # Classic NPLSDA
    ...
    Gu <- vector("list", factors)
    X <- matrix(A3D, xdims[1], xdims[2] * xdims[3])
    rownames(X)<-rownames(XN)
    colnames(X)<-rep(colnames(XN), xdims[3] )
    ordX <- length(dim(A3D))
    Y <- matrix(B3D, ydims[1], ydims[2] * ydims[3])
    Y2 <- matrix(B3D2, ydims[1], ydims[2] * ydims[3])
    ordY <- length(dim(B3D))
...

{
  ...
  if (is.null(X$NPLSDAvariates$X)){   # BLOCK.NPLSDA
    plot(X$NPLSDAvariates$Block.X[,pc1],X$NPLSDAvariates$Block.X[,pc2], type = "p", col = "black", pch=21,
         ...
         main = paste("Block X Mode 1", sep= " "), cex.main=1.2,
         cex.lab= 1.2,
    ...
    plot(X$NPLSDAvariates$Block.Y[,pc1],X$NPLSDAvariates$Block.Y[,pc2], type = "p", col = "black", pch=21,
         ...
         main = paste("Block Y Mode 1", sep= " "), cex.main=1.2,
         cex.lab= 1.2,
   ...
   # text (X$NPLSDAvariates$Block.Y[,pc1],X$NPLSDAvariates$Block.Y[,pc2], labels=rownames(FactorsY$Mode1), 
    ...
    for(i in 1:length(X$NPLSDAvariatesperMode3)){
      plot(X$NPLSDAvariatesperMode3[[i]][[1]]$Block.X[,pc1],X$NPLSDAvariatesperMode3[[i]][[1]]$Block.X[,pc2], 
         ...
         xlab = paste("Component ", pc1,sep = ""), 
         ylab = paste("Component ", pc2,sep = ""), las=1,
         main = paste("Block X Time",i,  sep= " "), cex.main=1.2,
         cex.lab= 1.2,
    ...
    plot(X$NPLSDAConsensusvariates$Block.XConsensus[,pc1],X$NPLSDAConsensusvariates$Block.XConsensus[,pc2], 
         ...
         xlab = paste("Component ", pc1,": ",round(X$NPLSDAexplVar[pc1,1]*100,digits=2)," %",sep = ""), 
         ylab = paste("Component ", pc2, ": ",round(X$NPLSDAexplVar[pc2,1]*100,digits=2)," %", sep = ""), las=1,
         main = paste("Consensus", sep= " "), cex.main=1.2,
         cex.lab= 1.2,
    ...
    for(i in 1:length(X$NPLSDAvariatesperMode3)){
      plot(X$NPLSDAvariatesperMode3[[i]][[1]]$Block.Y[,pc1],X$NPLSDAvariatesperMode3[[i]][[1]]$Block.Y[,pc2], 
           ...
           xlab = paste("Component ", pc1,sep = ""), 
           ylab = paste("Component ", pc2,sep = ""), las=1,
           main = paste("Block Y Time",i,  sep= " "), cex.main=1.2,
           cex.lab= 1.2,
    ...
    #     ylab = paste("Component ", pc1, ": ",round(X$NPLSDAexplVar[pc1,2]*100,digits=2)," % of Block Y", sep = ""), las=1,
     #    main = paste("Consensus", sep= " "), cex.main=2,
    #     cex.lab= 1.2,
    #     xlim = c(min(X$NPLSDAvariates$Block.X[,pc1]), max(X$NPLSDAvariates$Block.X[,pc1])),
     #    ylim = c(min(X$NPLSDAvariates$Block.Y[,pc1]), max(X$NPLSDAvariates$Block.Y[,pc1])))
    #abline(v=0,h=0)
    #text (X$NPLSDAvariates$Block.X[,pc1], X$NPLSDAvariates$Block.Y[,pc1], labels=rownames(X$FactorsX$Mode1), 
    #      col=DVars$colourOutcome,cex=1, pos=3)
    ...
    plot(X$NPLSDAloadings$Block.X[,pc1]*penalty,X$NPLSDAloadings$Block.X[,pc2]*penalty, type = "p", col = "black", pch=21,
         ...
         xlab = paste("Component", pc1, sep = " "), 
         ylab = paste("Component", pc2, sep = " "), 
         main = paste("Biplot Block X Loadings", sep= " "), 
         xlim = c(-1,1),
         ylim = c(-1,1))
    abline(v=0,h=0)
    ...
    for(i in 1:length(X$NPLSDAloadingsperMode3)){
      plot(X$NPLSDAloadingsperMode3[[i]][[1]]$Block.X[,pc1]*penalty,X$NPLSDAloadingsperMode3[[i]][[1]]$Block.X[,pc2]*penalty, 
           ...
           xlab = paste("Component ", pc1,sep = ""), 
           ylab = paste("Component ", pc2,sep = ""), las=1,
           main = paste("Biplot Block X Time",i,  sep= " "), cex.main=1.2,
           cex.lab= 1.2,
           xlim = c(-1,1),
           ylim = c(-1,1))
           #xlim = c(min(X$NPLSDAloadingsperMode3[[i]][[1]]$Block.X[,pc1]), max(X$NPLSDAloadingsperMode3[[i]][[1]]$Block.X[,pc1])),
   ...
   ##########################################################################################################
    ...
    plot(X$NPLSDAloadings$Block.Y[,pc1]*penalty,X$NPLSDAloadings$Block.Y[,pc2]*penalty, type = "p", col = "black", pch=21,
         ...
         xlab = paste("Component", pc1, sep = " "), 
         ylab = paste("Component", pc2, sep = " "), 
         main = paste("Biplot Block Y Loadings", sep= " "), 
         xlim = c(-1,1),
         ylim = c(-1,1))
    abline(v=0,h=0)
    arrows (0,0,X$NPLSDAloadings$Y[,pc1],X$NPLSDAloadings$Y[,pc2], col = background, pch=21, bg = background)
    ...
    for(i in 1:length(X$NPLSDAvariatesperMode3)){
      plot(X$NPLSDAloadingsperMode3[[i]][[1]]$Block.Y[,pc1]*penalty,X$NPLSDAloadingsperMode3[[i]][[1]]$Block.Y[,pc2]*penalty, 
           ...
           xlab = paste("Component ", pc1,sep = ""), 
           ylab = paste("Component ", pc2,sep = ""), las=1,
           main = paste("Biplot Block Y Time",i,  sep= " "), cex.main=1.2,
           cex.lab= 1.2,
           xlim = c(-1,1),
           ylim = c(-1,1))
    ...
    plot(x3, y3, type = "p", col = "black", pch=21,bg="dodgerblue2", xlab = paste("Component", pc1, sep = " "), 
         ylab = paste("Component", pc2, sep = " "), 
         main = paste("Block X Mode 3"), 
         #xlim = c((min(x3)), (max(x3))),
         #ylim = c((min(y3)), (max(y3))))
         xlim = c((min(x3)), (max(x3)+ abs(0.2*max(x3)))),
         ylim = c((min(y3)), (max(y3)+ abs(0.2*max(y3)))))
    abline(v=0,h=0)
    text (x3,y3, labels=rownames(FactorsX$Mode3), col="dodgerblue2",cex=2, pos=3)
    
    ######################################## 
    plot(x3Y, y3Y, type = "p", col = "black", pch=21,bg="dodgerblue2", xlab = paste("Component", pc1, sep = " "), 
         ylab = paste("Component", pc2, sep = " "), 
         main = paste("Block Y Mode 3"), 
         xlim = c((min(x3Y)), (max(x3Y)+ abs(0.2*max(x3Y)))),
         ylim = c((min(y3Y)), (max(y3Y)+ abs(0.2*max(y3Y)))))
    abline(v=0,h=0)
    text (x3Y,y3Y, labels=rownames(FactorsY$Mode3), col="dodgerblue2",cex=2, pos=3)
    ...
    ex <- as.matrix(explcore(X$G, factors, n=5))
    plot.new()
    mtext(paste("Expl.var of X: ", round(X$Explvar2DX[factors,2]*100,digits=2),"%") )
    mtext(paste("Expl.var of Response by X: ", round(X$Explvar2DX[factors,4]*100,digits=2),"%"),line=-1 )
    mtext(paste("Q2 of X: ", round(X$NPLSDAQ2cum2DX[factors,ncol(X$NPLSDAQ2cum2DX)],digits = 3)),line= -2)
    
    mtext(paste("Expl.var of Y: ", round(X$Explvar2DY[factors,2]*100,digits=2),"%"), line=-4 )
    mtext(paste("Expl.var of Response by Y: ", round(X$Explvar2DY[factors,4]*100,digits=2),"%"),line=-5 )
    mtext(paste("Q2 of Y: ", round(X$NPLSDAQ2cum2DY[factors,ncol(X$NPLSDAQ2cum2DY)],digits = 3)),line= -6)
    
    mtext(paste(colnames(ex), collapse = "                "), line = -8)
    par(cex = 0.8)
    for (i in 1:nrow(ex)) {
      mtext(paste(as.vector(ex[i, ]), collapse = "       "), line = -(i + 4) * 2, adj = 0)
    } 
  } else {
    ##################################################### Block X Mode 1
...
################################################################ Transformed Biplot
  ...
  mtext(paste("Q2: ", round(X$NPLSDAQ22D[1,ncol(X$NPLSDAQ22D)],digits = 3)),line= -4)
  #X$NPLSDAQ22D
  #mtext(paste("model expl.var: ", sum(as.numeric(ex[c(1:2),4]))))
  #mtext(paste("model expl.var: ", formatC(X$expl.var)))
  mtext(paste(colnames(ex), collapse = "                                   "), line = -6)
  par(cex = 0.8)
  for (i in 1:nrow(ex)) {
    mtext(paste(as.vector(ex[i, ]), collapse = "                            "), line = -(i + 3) * 2, adj = 0)
  } 
}
}
