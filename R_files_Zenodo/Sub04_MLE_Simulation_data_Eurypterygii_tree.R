...
int.p<-starting.point.musse(phy.sim,k=snum)
p<-int.p[free.i] #prior
names(p)<-c()

#MLE
print(paste("Start MLE of trial",trial," of neot815 CM35 ..."))
timepoint<-c(proc.time()[[3]])
fit.musse.sim<-find.mle(mussefunc,p)
timepoint<-c(timepoint,proc.time()[[3]])

## Save fit.musse.neot
filename<-paste("fit_musse_sim_neot_CM35_trial",trial,".Robj",sep="")
save(fit.musse.sim,file=filename)

## Result of the 8 para Estimation
coef_res<-c(coef(fit.musse.sim)[c(6,5,3,4,1,2)]) #2020 definition
