...
# Data: 
# Gene Expression
load("/media/data/leobalzano/ScriptsForTEDDY/Data/Ful1larrayGEMARCH306.RData")
load("/media/data/leobalzano/ScriptsForTEDDY/Data/FullarrayGEMARCH136.RData")
load("/media/data/leobalzano/ScriptsForTEDDY/Data/FullarraytwooutofthreeGeneList306.RData")
load("/media/data/leobalzano/ScriptsForTEDDY/Data/FullarrayGE862x136.RData")

# Metabolomics
load("/home/leobalzano/Documents/TEDDY data/MP149/Diet/NPLS/NPLSDAmetabolomicsDecember/FullarrayMetabolomics136.RData")  # 136 x 1321 x 5
load("/home/leobalzano/Documents/TEDDY data/MP149/Diet/NPLS/NPLSDAmetabolomicsDecember/FullarrayMetabolomics306x245.RData")  # 306 x 245 x 5
load("/home/leobalzano/Documents/TEDDY data/MP149/Diet/NPLS/NPLSDAmetabolomicsDecember/Allmetabolomics136.RData")  # 136 x 245 x 5

# Response Variable
load("/media/data/leobalzano/ScriptsForTEDDY/Data/outcomedummyarray136.RData")
load("/media/data/leobalzano/ScriptsForTEDDY/Data/outcomedummyarray306.RData")
...
for (s in 1:2){
  ...
  for (i in 1:dim(muestraarray)[3]){
    TEDDYGEplsda <- opls(muestraarray[,,i], outcomedummyarray136[,,i],predI = 1, permI = 10,printL=FALSE,
...
for (i in 1:dim(FullarrayGE862x136)[3]){
  #for (i in 1:2){
  TEDDYGEplsda <- opls(FullarrayGE862x136[,,i], outcomedummyarray136[,,i],predI = 1, permI = 100,printL=FALSE,
                       plotL=FALSE) # For PLSDA
  pvalstita<-TEDDYGEplsda$summaryDF
  namepvals<-paste("time:",times[i],sep = "")
  pvals<-list(pvalstita)
...
for (s in 1:10){
  ...
  for (i in 1:dim(muestraarray)[3]){
    #for (i in 1:2){
    TEDDYGEplsda <- opls(muestraarray[,,i], outcomedummyarray136[,,i],predI = 1, permI = 100,printL=FALSE,
                         plotL=FALSE) # For PLSDA
    pvalstita<-TEDDYGEplsda$summaryDF
    namepvals<-paste("time:",times[i],sep = "")
    pvals<-list(pvalstita)
...
for (i in 1:dim(Allmetabolomics136)[3]){
  #for (i in 1:2){
  TEDDYGEplsda <- opls(Allmetabolomics136[,,i], outcomedummyarray136[,,i],predI = 1, permI = 100,printL=FALSE,
                       plotL=FALSE) # For PLSDA
  pvalstita<-TEDDYGEplsda$summaryDF
  namepvals<-paste("time:",times[i],sep = "")
  pvals<-list(pvalstita)
