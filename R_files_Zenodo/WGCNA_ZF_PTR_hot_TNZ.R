...
setwd("~/Dropbox/Rosvall_Postdoc/Heat stress/WGCNA/Heat Stress PTR/PTR hot vs control")
...
PTR = read.csv("ZF_PTR_normalizedCounts_nocold.csv")
...
PTR_normcounts <- data.frame(PTR[,-1], row.names=PTR[,1])
PTR_normcounts = PTR_normcounts[-1]
dim(PTR_normcounts) # 16015    17
...
use = rowSums(PTR_normcounts > 10) >=14 # 90% must have at least a normalized count of 10
countMatrixFiltered = PTR_normcounts[ use, ]
dim(countMatrixFiltered) # 12384    16
...
dim(Filtered) # Left with 9288  genes out of 17051
write.csv(Filtered,file="ZF_PTR_normCounts_logreduced25.csv")

# Read gene annotation file - so that we can match gene names with Ensembl IDs
annot = read.csv("ZF_PTR_annGeneNames.csv")


########################################### Read in trait data ##########################################
traitData = read.csv("ZF_heat_traits_WGCNA_PTR_nocold.csv");
dim(traitData) # 16 32
names(traitData)
...
datExpr<-read.csv("ZF_PTR_normCounts_logreduced25.csv")
dim(datExpr) # 9288   17
...
# 153 is an outlier - potentially bc of missing data from small tissue input
...
# 6268  245  235  220  218  174  172  168  165  133  119  118  117  114  113  111   95   79 
# 18   19   20   21   22   23   24 
# 69   65   63   61   60   59   47 
...
# 168           235           220           113            60            47 
# darkred darkturquoise         green   greenyellow          grey        grey60 
# 61            59           174           118          6268            79 
# lightcyan    lightgreen   lightyellow       magenta  midnightblue          pink 
# 95            69            65           133           111           165 
# purple           red     royalblue        salmon           tan     turquoise 
# 119           172            63           114           117           245 
# yellow 
# 218 
...
save(MEs, moduleLabels, moduleColors, geneTree, file = "ZF_PTR_hotvscontrol_normcounts_nocold_modules.RData")
...
labeledHeatmap(Matrix = moduleTraitCor,
               ...
               ySymbols = names(MEs),
               colorLabels = FALSE,
               colors = colorRampPalette(brewer.pal(8, "OrRd"))(50),
...
table(moduleColors)
# blue         cyan    darkgreen      darkred        green  greenyellow         grey 
# 235          113           60          407          174          118         6268 
# lightcyan  lightyellow      magenta midnightblue       purple          red    royalblue 
# 95          824          133          393          119          172           63 
# salmon 
# 114 
#write.csv(MEs, file = "ZF_PTR_hotvscold_normcounts_nocold_eigengenes_modules.csv")

moduleColors

#focuse on one trait
# Define variable weight containing the weight column of datTrait
Pant = as.data.frame(datTraits$Pant);
names(Pant) = "Pant"
...
geneTraitSignificance = as.data.frame(bicor(datExpr0, Pant, use = "p",robustY = FALSE,maxPOutliers =0.1));
GSPvalue = as.data.frame(corPvalueStudent(as.matrix(geneTraitSignificance), nSamples));
names(geneTraitSignificance) = paste("GS.", names(Pant), sep="");
names(GSPvalue) = paste("p.GS.", names(Pant), sep="");
...
verboseScatterplot(abs(geneModuleMembership[moduleGenes, column]),
                   ...
                   ylab = "Gene significance for Panting",
...
write.csv(geneInfo, file = "ZF_PTR_genename_pant.csv")
