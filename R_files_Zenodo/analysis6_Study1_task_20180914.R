source("analysis6_Study1_task_20180914.R", encoding="Shift_JIS")library(ggplot2) # きれいなグラフをプロットするためのライブラリを読み込み
...
load.and.split <- function(filename) {
    ...
    return(rawdata)
                                        ...
                                        #files <- list('ID015.csv')
#files <- list('ID015.csv', 'ID016.csv')
                                        #files <- list('ID015.csv', 'ID016.csv', 'ID017.csv', 'ID018.csv', 'ID019.csv', 'ID020.csv')
...
for (thecontents in list(list("raw", contents), list("filtered", filtered.contents))) { # 以下の内容をxcontents=contentsとxcontents=filtered.contentsでそれぞれ実行
        ...
        for (name2 in name.of.contents) { # 以下の内容をfilesの中のすべてのファイルについて実行
            ...
            if (name1!=name2) {
                ...
                if (is.self.simul(name1, name2)) {
                    if (crosscorr.data.frame.self==0) {
                        crosscorr.data.frame.self <- newrow
                    } else {
                        crosscorr.data.frame.self <- data.frame(crosscorr.data.frame.self, newrow)
                    }
    ...
    for (llabel in lls) {
        ...
        simultaneous <- subset(df, (ch==i) & (stype=="Inter-brain"), c(correlation))$correlation
            ...
            print(sprintf("%s ch%s, Cliff's delta=%e", llabel, i, 2*wresult$statistic/(length(control)*length(simultaneous))-1), quote=FALSE) # 正の値ならcontrolがsimultaneousより大きく、負の値ならsimultaneousがcontrolより大きい
            ...
            g <- ggplot(df2, aes(x=stype, y=correlation, fill=pairs)) + geom_bar(position="dodge", stat="identity", show.legend=FALSE)+theme_grey(base_size=18)+theme(axis.text=element_text(size=24))+labs(title=sprintf("%s ch%s, p=%e", llabel, i, wresult$p.value))+xlab("")+ylab("Correlation coefficient")
        plot(g)
        addpagenum()
    }
    }
