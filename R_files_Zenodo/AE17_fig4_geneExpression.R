...
model<-readRDS("data/meta/AE17_RNAmetaData.RData")
levels(model$SFV) <- c("E09","C09","E80","C80")
model$SFV <-as.factor(as.character(model$SFV))
#model_dnam<-model[model$ID != "17099",]

#### Gene Matrix ####
gc_qualityweights<-readRDS("results/RNA/RNA_gene_postVoomAndNormalization_DGEListObj.RData")
...
P3 <- ggplot(temp_df,aes(x=x,y=y,colour=condition)) + 
  geom_point(size=4) + 
  theme_cowplot() + 
  #scale_colour_manual(values = col_perm[c(3,1,4,2)],
  #                    labels=c('OA 2800\\n     D9','Ambient\\n     D9','OA 2800\\n     D80', 'Ambient\\n     D9')) + 
...
early_time_meta <- model[model$Time == 9,]
...
late_time_counts <- gc_comb[,model$Time == 80]
late_time_meta <- model[model$Time == 80,]
