...
Event_All_Count_A1<-Event_All_Count[which(Event_All_Count$Aviary=="2"),]
...
# Load the data
load("2018_A2_prebreedingMtx.RData") #prebreedmtx
...
for(i in 1:nrow(A1_individuals)){
  ...
  AdultsinA1<-dataParentsID[dataParentsID$aviary=="2",]
...
dataChickID_A1<-dataChickID[which(dataChickID$fosteraviary=="2"),]
...
A1AllMales<-dataParentsID$qr_code[which(dataParentsID$sex=="m"&dataParentsID$aviary=="2")]
A1AllFemales<-dataParentsID$qr_code[which(dataParentsID$sex=="f"&dataParentsID$aviary=="2")]
...
### plot 
pdf("A2ProvNet_MotherRec_network.pdf")
...
maleAdults<-dataParentsID$qr_code[which(dataParentsID$sex=="m"&dataParentsID$aviary=="2")]
...
femaleAdults<-dataParentsID$qr_code[which(dataParentsID$sex=="f"&dataParentsID$aviary=="2")]
...
pdf("Aviary2_PrebreedingSocialNetwork_RecipientMother.pdf")
...
### plot 
pdf("A2ProvNet_FatherRec_network.pdf")
...
pdf("Aviary2_PrebreedingSocialNetwork_RecipientFather.pdf")
coords <- layout_with_fr(net)
plot(net,layout=coords,edge.width=(200*E(net)$weight^1.2))
dev.off()
...
library(vegan)
mantel(prov, soc, na.rm = T) #r=-0.1251, Significance 0.801
...
pdf("Paper_Aviary2_ProvisioningNetwork.pdf")
...
pdf("Paper_Aviary2_PrebreedingSocialNetwork.pdf")
#coords <- layout_with_fr(net)
plot(net,layout=coords, edge.width=(200*E(net)$weight^1.2))
dev.off()
