...
res <- lme(log.short~z.temp+I(z.temp^2)+sex*(z.age+I(z.age^2))+treatment*z.dose, random=~1|id, data=xdata)
...
## Colinearity
library(car)
res.lm <- lm(log.short~z.dose+sex+z.age+temperature+treatment, data=xdata)
vif(res.lm)# Ok, <4
