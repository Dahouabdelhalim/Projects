...
#MLE
print(paste("Start MLE of neot groupA M0 prime CM",chrmax, " args...",sep=""))
timepoint<-c(proc.time()[[3]])
fit.musse.neot.null.groupA<-find.mle(mussefunc,p)
timepoint<-c(timepoint,proc.time()[[3]])

## Save fit.musse.neot.null.groupA
filename=paste("fit_musse_neot_null_groupA_M0_prime_CM",chrmax,"_sampling.f_XXXXXX.Robj",sep="")
save(fit.musse.neot.null.groupA,file=filename)

## Result of the 6 para Estimation
coef_res<-c(coef(fit.musse.neot.null.groupA)[c(6,5,3,4,1,2)]) #2020 definition
names(coef_res)<-c("k1","k2","k3","k4","lambda","mu")
coef_res #coef
fit.musse.neot.null.groupA[2] #lnLik
