...
pd <- population_dynamics(change = growth(possum_trans_mat,
                                          transition_function = modified_transition_custom(fire_stack_early = "Early_fire",
                                                                                           ...
                                                                                           early_surv_mult = 0.83,
                                                                                           late_surv_mult = 0.7,
                                                                                           early_tran_mult = 0.83,
                                                                                           late_tran_mult = 0.7,
                                                                                           early_recr_mult = 0.46,
                                                                                           late_recr_mult = 0.02),
...
system.time(
  possum_pop_data <- foreach (i = 1:25, .combine = rbind) %do% {
    set.seed(333)
    Poss <- simulation(landscape = poss_landscape,
...
save(possum_pop_data, file = "possum_pop_data.RData")

max_pop <- ceiling(max(possum_pop_data))

plot_pop(possum_pop_data, upper_limit = max_pop)
