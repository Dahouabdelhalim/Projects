...
###########################################################
# Functions:
source ("/media/data/leobalzano/ScriptsForTEDDY/Scripts/TEDDYtools2.R") # These are the functions to reformat the data
source ("/media/data/leobalzano/Dropbox (UFL)/ScriptsForTEDDY/Scripts/NPLSDAfunctionsApr11.R") # These are the functions created to perform the NPLSDA

###########################################################
# Data:
# Gene Expression
load('/media/data/leobalzano/ScriptsForTEDDY/Data/TEDDY_Gene_Expression_Data_inversevst_MP149_Log2Xwithin.ro')
load("/media/data/leobalzano/ScriptsForTEDDY/Data/arrayGENEXMarch136.RData")
load("/media/data/leobalzano/ScriptsForTEDDY/Data/FullarrayGEMARCH136.RData")
load("/media/data/leobalzano/ScriptsForTEDDY/Data/NPLSDAFullarrayGEMARCH136.RData")
load("/media/data/leobalzano/ScriptsForTEDDY/Data/selectedgenesbydifVIPsMarch.RData")
...
LM_globalTargets = loadTargets(SELdisease = "IA",time_point =  c(12,9,6,3,0),complete_case = "flexible",targets = targets) #Uses all samples
head(LM_globalTargets)
dim(LM_globalTargets)
rownames(LM_globalTargets)<-LM_globalTargets[,1]

Genexpression<-merge (LM_globalTargets,tGEXdata,by=0)
dim(tGEXdata);dim(LM_globalTargets); dim (Genexpression)
Genexpression[1:10,1:10]
#GenexpressionCompleteTable<-Genexpression
#save(GenexpressionCompleteTable, file="/media/data/leobalzano/Milestone9/GenexpressionCompleteTable.RData")
getwd()

###########################################################
# Subsetting the data to the 136 pairs with values reported in 3 0ut 0f 5 time points
GeneExpression136<- Genexpression[is.element(Genexpression$mask_id, patients3tps[,1]),,]
dim(GeneExpression136)
length(unique(GeneExpression136$mask_id))  # 136 PERFECT

###########################################################
# GE data in 3D structure
dim(GeneExpression136)   # 476 x 21305
colnames(GeneExpression136[1:23])
GENEX<-GeneExpression136[,c(3,21:21305)]       # Just Mask.Id and the variables
GENEX12<-GENEX[GeneExpression136$time == "12",]; dim(GENEX12);dim(GENEX)
GENEX9<-GENEX[GeneExpression136$time == "9",];dim(GENEX9)
GENEX6<-GENEX[GeneExpression136$time == "6",];dim(GENEX6)
GENEX3<-GENEX[GeneExpression136$time == "3",];dim(GENEX3)
GENEX0<-GENEX[GeneExpression136$time == "0",];dim(GENEX0)
...
colnames(patients3tps2)<- "mask_id"
GENEX12total<-merge(patients3tps2,GENEX12, by="mask_id", all.x = T);dim(GENEX12total);dim(patients3tps2);dim(GENEX12)
GENEX12total[1:5,1:5]
rownames(GENEX12total)<- GENEX12total[,1]
GENEX12total<- as.matrix(GENEX12total[,c(-1)])
dim(GENEX12total)

GENEX9total<-merge(patients3tps2,GENEX9, by="mask_id", all.x = T);dim(GENEX9total);dim(patients3tps2);dim(GENEX9)
GENEX9total[1:5,1:5]
rownames(GENEX9total)<- GENEX9total[,1]
GENEX9total<- as.matrix(GENEX9total[,c(-1)])
dim(GENEX9total)

GENEX6total<-merge(patients3tps2,GENEX6, by="mask_id", all.x = T);dim(GENEX6total);dim(patients3tps2);dim(GENEX6total)
GENEX6total[1:5,1:5]
rownames(GENEX6total)<- GENEX6total[,1]
GENEX6total<- as.matrix(GENEX6total[,c(-1)])
dim(GENEX6total)

GENEX3total<-merge(patients3tps2,GENEX3, by="mask_id", all.x = T);dim(GENEX3total);dim(patients3tps2);dim(GENEX3)
GENEX3total[1:5,1:5]
rownames(GENEX3total)<- GENEX3total[,1]
GENEX3total<- as.matrix(GENEX3total[,c(-1)])
dim(GENEX3total)

GENEX0total<-merge(patients3tps2,GENEX0, by="mask_id", all.x = T);dim(GENEX0total);dim(patients3tps2);dim(GENEX0)
GENEX0total[1:5,1:5]
...
dim(NPLSDAFullarrayGEMARCH136$VIP2D)
...
#save(NPLSDAarraytwooutofthreeGeneList136, file= "NPLSDAarraytwooutofthreeGeneList136.RData")
load(file="NPLSDAarraytwooutofthreeGeneList136.RData")
