...
### load specimen info table and select females only
specimen_info <- read.csv(paste(getwd(), "/specimen_descriptions.csv", sep=""))
specimen_info_females_logical <- specimen_info$Sex == "female"
specimen_info_females <- specimen_info[specimen_info_females_logical,]
...
### subset landmarks to females only
landmarks_rep1 <- landmarks_rep1[,,specimen_info_females_logical]
landmarks_rep2 <- landmarks_rep2[,,specimen_info_females_logical]
...
plot(PCA_rep1, main = "PCA_rep1", pch = specimen_info_females$Sex, col = specimen_info_females$color)
dev.off()
pdf(file = paste(getwd(), '/results/PCA_rep2.pdf', sep = ''))
plot(PCA_rep2, main = "PCA_rep2", pch = specimen_info_females$Sex, col = specimen_info_females$color)
dev.off()
pdf(file = paste(getwd(), '/results/PCA_means.pdf', sep = ''))
plot(PCA_means, main = "PCA_means", pch = specimen_info_females$Sex, col = specimen_info_females$color)
dev.off()

### plot species separately
P_calypso <- specimen_info_females$Species == "Phrynus_calypso"
...
P_exsul <- specimen_info_females$Species == "Phrynus_exsul"
...
P_goesii <- specimen_info_females$Species == "Phrynus_goesii"
...
P_sp.nov <- specimen_info_females$Species == "Phrynus_sp.nov"
...
P_longipes <- specimen_info_females$Species == "Phrynus_longipes"
...
P_pinarensis <- specimen_info_females$Species == "Phrynus_pinarensis"
...
P_tessellatus <- specimen_info_females$Species == "Phrynus_tessellatus"
