...
## Function, get_cons_and_target_musse_null_Ki1
### almost same as get_cons_and_target_null but Ki=1 for 5 par estimation
...
free.i<-c(tcal(snum,1,2),tcal(snum,2,3),tcal(snum,3,2)) #for 5 par q0201 was deleted.
free.i<-c(1,snum+1,((snum*2)+free.i))
cons_and_tar<-get_cons_and_target_musse_null_Ki1_ec(chrmax) #for 5 par
...
#MLE
print(paste("Start MLE of otop CM",chrmax, "with Ki=1..."))
timepoint<-c(proc.time()[[3]])
fit.musse.otop<-find.mle(mussefunc,p)
timepoint<-c(timepoint,proc.time()[[3]])

## Save fit.musse.otop
filename<-paste("fit_musse_otop_M1_CM",chrmax,"_unisample_XXXXXX.Robj",sep="")
save(fit.musse.otop,file=filename)

## Result of the 5 para Estimation

coef_res<-c(coef(fit.musse.otop)[c(5,4,3,1,2)]) #2020 definition
