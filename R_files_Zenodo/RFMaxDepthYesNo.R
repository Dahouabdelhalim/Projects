...
maxdepthcount <- sum(Z_complete$maxdepthmeasure)/length(Z_complete$maxdepthmeasure)
...
maxdepth<-randomForest(formula=as.factor(maxdepthmeasure) ~ .,
                data = train, ntree=500)

print(maxdepth)

test$maxdepthpred <- predict(maxdepth,test)
test$maxdepthpred <- as.factor(test$maxdepthpred)
test$maxdepthmeasure <- as.factor(test$maxdepthmeasure)
confusionMatrix(test$maxdepthpred,test$maxdepthmeasure)

pred <- predict(object=maxdepth,newdata=test)
library(Metrics)
auc(actual=test$maxdepthmeasure,predicted=pred)
...
for (j in 1:500)
{
  ...
  cm <- confusionMatrix(flips,test$maxdepthmeasure)
