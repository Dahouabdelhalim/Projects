...
system.time(
  possum_AEF_pop_data <- foreach (i = 1:25, .combine = rbind) %do% {
    set.seed(333)
    Poss3 <- simulation(landscape = poss_landscape,
    ...
    pop_data <- return_pop_data(Poss3)
    rm(Poss3)
    gc()
    pop_data
  }
