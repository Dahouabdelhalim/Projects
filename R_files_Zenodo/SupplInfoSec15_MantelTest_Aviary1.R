#Basic information
##### Load the data
BasicInfo <- read.csv("Event_All_Count.csv",stringsAsFactors=FALSE, sep=",")

length(unique(BasicInfo$EventID))#266 #number of feeding visits in total
length(unique(BasicInfo$EventID[which(BasicInfo$AdultsSex=="f")])) #154 feeding visits
length(unique(BasicInfo$EventID[which(BasicInfo$AdultsSex=="m")])) #112 feeding visits
table(BasicInfo$AdultsSex)# f:292, m:199
table(BasicInfo$Category)# Unrelated: 491 (feeding events)
length(unique(BasicInfo$AdultID[which(BasicInfo$AdultsSex=="f" & BasicInfo$Aviary=="1" )]))+length(unique(BasicInfo$AdultID[which(BasicInfo$AdultsSex=="f" & BasicInfo$Aviary=="2" )]))+length(unique(BasicInfo$AdultID[which(BasicInfo$AdultsSex=="f" & BasicInfo$Aviary=="3" )]))+length(unique(BasicInfo$AdultID[which(BasicInfo$AdultsSex=="f" & BasicInfo$Aviary=="4" )]))#15
length(unique(BasicInfo$AdultID[which(BasicInfo$AdultsSex=="m" & BasicInfo$Aviary=="1" )]))+length(unique(BasicInfo$AdultID[which(BasicInfo$AdultsSex=="m" & BasicInfo$Aviary=="2" )]))+length(unique(BasicInfo$AdultID[which(BasicInfo$AdultsSex=="m" & BasicInfo$Aviary=="3" )]))+length(unique(BasicInfo$AdultID[which(BasicInfo$AdultsSex=="m" & BasicInfo$Aviary=="4" )]))#21
...
Event_All_Count_A1<-Event_All_Count[which(Event_All_Count$Aviary=="1"),]
...
# Load the data
load("2018_A1_prebreedingMtx.RData") #prebreedmtx
...
for(i in 1:nrow(A1_individuals)){
  ...
  AdultsinA1<-dataParentsID[dataParentsID$aviary=="1",]
...
for(i in 1:length(AM_Combinations)){
  ...
  for(j in 1:length(All_Events)){
    ...
    #RecipientMom<-AllInfo$NoNQPeriod_MotherOFRecipients[which(AllInfo$Event==temp_All_Events)]
...
dataChickID_A1<-dataChickID[which(dataChickID$fosteraviary=="1"),]
...
A1AllMales<-dataParentsID$qr_code[which(dataParentsID$sex=="m"&dataParentsID$aviary=="1")]
A1AllFemales<-dataParentsID$qr_code[which(dataParentsID$sex=="f"&dataParentsID$aviary=="1")]
...
### plot 
pdf("A1ProvNet_MotherRec_network.pdf")
...
maleAdults<-dataParentsID$qr_code[which(dataParentsID$sex=="m"&dataParentsID$aviary=="1")]
...
femaleAdults<-dataParentsID$qr_code[which(dataParentsID$sex=="f"&dataParentsID$aviary=="1")]
...
pdf("Aviary1_PrebreedingSocialNetwork_RecipientMother.pdf")
...
### plot 
pdf("A1ProvNet_FatherRec_network.pdf")
...
pdf("Aviary1_PrebreedingSocialNetwork_RecipientFather.pdf")
coords <- layout_with_fr(net)
plot(net,layout=coords,edge.width=(200*E(net)$weight^1.2))
dev.off()
...
pdf("Paper_Aviary1_ProvisioningNetwork.pdf")
...
pdf("Paper_Aviary1_PrebreedingSocialNetwork.pdf")
plot(net,layout=coords, edge.width=(200*E(net)$weight^1.2))
dev.off()
