setwd("H:/BigLake/Paper/LASSOLimitedData0513")

alldata <- read.csv("LagosDec2019Data0514.csv",na.strings=c("NA"),header=TRUE)
library(tidyverse)
library(glmnet)
library(mpath)
...
QWq <- alldata %>% select(maxdepth,avgsecchi0514,nsecchigreater10514)
...
write.csv(predbestlamcoef,file="predbestlamcoefwoutSDSecchi.csv")
...
write.csv(predbestlamcoef,file="predbestlamcoefs1woutSDSecchi.csv")
...
write.csv(predbestlamcoef,file="predbestlamcoefs2woutSDSecchi.csv")

teststat <- c(cvm,cvm.s1,cvm.s2,bestlam,bestlam.s1,bestlam.s2)
write.csv(teststat,file="teststat.csv")
