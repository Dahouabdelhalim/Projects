...
semtypeExclusions <- " ('medd', 'genf', 'imft', 'chvs', 'orga', 'clna', 'bdsu', 'nnon', 'nusq', 'chem', 'diap', 'chvf', 'sbst', 'bodm', 'resd', 'drdd') "

drugSemtypes  <- " ('phsu', 'orch', 'hops', 'food', 'irda', 'clnd', 'vita', 'eico', 'opco', 'lipd', 'horm', 'antb', 'elii', 'carb')  "

sort(drugSemtypes)
# carbohydrate sequence, clinical drugs, food, hazardous or poisonous substances, hormones, indicator, reagent, diagnostic aid, lipids, organic chemicals, pharmaceutical substances, therapeutic or preventive procedure

geneSemtypes <- "  ('aapp', 'gngm', 'bacs', 'inch', 'nsba') " # bacs is frustrating

# amino acid, peptide, or proteins, genes, genome, 

########
# bacs - biologically active substance - very tricky!!! inside the body or outside????
########

phenotypeSemtypes <- " ('dsyn', 'fndg', 'patf', 'sosy', 'mobd', 'strd', 'inpo', 'lbtr', 'virs', 'neop', 'bact', 'invt', 'orgm', 'cgab', 'rich', 'topp', 'dora', 'acab', 'inbe', 'eehu')  "

# acquired abnormality, bacterium, congenital abnormality, daily or recreational activity, disease syndrome, environmental effect on humans, finding, individual behavior, 
# injury or poisoning, lab or test result, mental or behavioral dysfunction, neoplastic process, organism, pathological function, sign or symptom, therapeutic or preventive procedure, virus 
...
getConfounders <- function(predicates) {
  sqlQuery <- paste(" WITH ZX AS (SELECT cp.subject_cui AS covar, COUNT(distinct cp.pmid) AS scnt, cp.subject_name --, string_agg(cp.subject_semtype) as subject_semtype
                        FROM causalPredications cp, citations c
                        WHERE cp.pmid = c.pmid
                        AND cp.subject_semtype NOT IN ", semtypeExclusions, "
                        AND cp.subject_semtype IN ", drugSemtypes, "
                     --   AND cp.subject_semtype IN ", geneSemtypes, "
                        ...
                        ZY AS (SELECT cp.subject_cui AS covar , COUNT(DISTINCT cp.pmid) AS scnt, cp.subject_name --, string_agg(cp.subject_semtype) as subject_semtype
                        FROM causalPredications cp, citations c
                        WHERE cp.pmid = c.pmid
                        AND cp.subject_semtype NOT IN ", semtypeExclusions, "
                        AND cp.subject_semtype IN ", drugSemtypes, "
  ...
  print(sqlQuery)
  confounders <- dbGetQuery(con, sqlQuery)
...
paste0(sort(print(confounders[,4])), collapse = ", ")
#########
#
#########
