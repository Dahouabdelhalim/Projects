...
system.time(
  possum_AF_AEF_pop_data <- foreach (i = 1:25, .combine = rbind) %do% {
    set.seed(333)
    Poss4 <- simulation(landscape = poss_landscape,
    ...
    pop_data <- return_pop_data(Poss4)
    rm(Poss4)
    gc()
    pop_data
  }
...
save(possum_AF_AEF_pop_data, file = "possum_AF_AEF_pop_data.RData")

# Plot possum population trajectory of each life-stage
plot(Poss)

# Plot total possum population trajectory
plot(Poss4, stage = 0, ylim = c(0,50000))

# Plot possum abundance by timestep
plot(Poss4[1], type = "raster", stage = 0, timesteps = 126)
