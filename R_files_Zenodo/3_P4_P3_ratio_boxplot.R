...
pdf(file = paste(getwd(), '/P4_P3_ratio_boxplot.pdf', sep = ''))
ggplot(pedipalp_measurements, aes(x=Species, y=P4_P3_ratio)) + 
  geom_boxplot()+
  geom_dotplot(binaxis='y', stackdir='center', stackratio=1, dotsize=0.5) +
  stat_summary(fun=mean, geom="point", shape=3, size=5, color="red")
dev.off()

pdf(file = paste(getwd(), '/P4_P3_ratio_males_boxplot.pdf', sep = ''))
ggplot(test_subset_group_males, aes(x=Species, y=P4_P3_ratio)) + 
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(width = 0.2, size=2.5) + 
  scale_y_continuous(name="P4_P3_ratio",breaks=seq(0.5, 0.8, 0.1), limits=c(0.5,0.8)) +
  stat_summary(fun=mean, geom="point", shape=3, size=5, color="red") +
  theme(aspect.ratio=1)
dev.off()

pdf(file = paste(getwd(), '/P4_P3_ratio_females_boxplot.pdf', sep = ''))
ggplot(test_subset_group_females, aes(x=Species, y=P4_P3_ratio)) + 
  geom_boxplot(outlier.shape = NA)+
  geom_jitter(width = 0.2, size=2.5) + 
  scale_y_continuous(name="P4_P3_ratio",breaks=seq(0.5, 0.8, 0.1), limits=c(0.5,0.8)) +
  stat_summary(fun=mean, geom="point", shape=3, size=5, color="red") +
...
pdf(file = paste(getwd(), '/P4_P3_ratio_boxplots_both_Sexes.pdf', sep = ''))
ggplot(pedipalp_measurements, aes(x=Species, y=P4_P3_ratio, fill=Sex)) + 
  geom_boxplot()+
  scale_y_continuous(name="P4_P3_ratio",breaks=seq(0.5, 0.8, 0.1), limits=c(0.5,0.8)) +
  stat_summary(fun=mean, geom="point", shape=3, size=5, color="red") +
  theme(aspect.ratio=1)
