###### Parameter ######
## Give chromosome number limit  and special states having different speciation and extinction rates
...
## Phenotype are already filtered and arranged as otop.state.
## load the otop.state from the file

load("otop_state_YK2021.Robj")
keep<-which(names.fish %in% names(otop.state))
phy.o.otop<-keep.tip(phy.fish,keep)
phy.o.otop$tip.label<-names(otop.state)
phy.o.otop$tip.state<-otop.state

## Remove the tip with out of range in PCM

snum<-(chrmax+1)*(chrmax+2)/2-1
phy.o.otop<-keep.tip(phy.o.otop,which(otop.state<=snum))
phy.o.otop$tip.state<-phy.o.otop$tip.state[otop.state<=snum]

###### Sampling.f preparation ######
### M2 model needs different two values of sampling.f

## Preparation of karyotype and taxon information

load("val_chrinfo_YK2021.Robj")
otoplist<-c("CYPRINIFORMES","CHARACIFORMES","SILURIFORMES","GYMNOTIFORMES")

## Determine sampling.f
### sampling.f is used for the correction of species richness by sampling species for phylogeny
### The calculation for "sfsp" and "sfother" here is a coventional way to make sampling.f in our method.
...
### Then in M2 model, the two values are determined.

tipstate<-factor(phy.o.otop$tip.state,levels=1:snum)
sfsp<-sum(tipstate %in% sp_state)/sum(val_chrinfo$Stat %in% sp_state)
sfother<-sum(!(tipstate %in% sp_state))/sum(!(val_chrinfo$Stat %in% sp_state))
sampling.f<-rep(sfother,snum)
sampling.f[sp_state]<-rep(sfsp,length(sp_state))

###### MLE wih MuSSE M2 model ######

## Make musse object with sampling.f
mussefunc<-make.musse(phy.o.otop,phy.o.otop$tip.state,snum,sampling.f=sampling.f,strict=F)

## Determine free.i for 4 parameters of karyotype evolution
free.i<-c(tcal(snum,1,2),tcal(snum,2,1),tcal(snum,2,3),tcal(snum,3,2))

## Determine free.i for 4 parameters of two speciation and two extinction rates
free.i<-c(1,sp_state[1],snum+1,snum+sp_state[1],((snum*2)+free.i))

## Get constraint formulae and target.i for MuSSE M2 model
cons_and_tar<-get_cons_and_target_musse_testSE_ec(chrmax,sp_state)

## Make constraints
mussefunc<-constrain.kt(mussefunc,cons_and_tar[[1]],free.i,cons_and_tar[[2]])

## Get priors
int.p<-starting.point.musse(phy.o.otop,k=snum)
p<-int.p[free.i] #prior
names(p)<-c()

## Start MLE for MuSSE M2 model
print(paste("Start MLE of M2 otop CM",chrmax, "..."))
timepoint<-c(proc.time()[[3]])
fit.musse.otop<-find.mle(mussefunc,p)
timepoint<-c(timepoint,proc.time()[[3]])

## Save the resultant R object fit.musse.otop 
filename<-paste("fit_musse_otop_testSE_CM",chrmax,"_samplingf_XXXXXX.Robj",sep="")
save(fit.musse.otop,file=filename)

## Text output of the results
coef_res<-c(coef(fit.musse.otop)[c(8,7,5,6,2,4,1,3)]) #2020 definition
names(coef_res)<-c("k1","k2","k3","k4","lambdaT","muT","lambdaC","muC")
coef_res #coef
fit.musse.otop[2] #lnLik
paste("Chromosome limit,",chrmax, ": Process time,", round(timepoint[2]-timepoint[1],3))

###### ASR wih MuSSE M2 model ######
## Ansestral State Reconstruction

print(paste("Start ASR of M2 otop CM",chrmax, "..."))
timepoint<-c(proc.time()[[3]])
st.nodes.otop<-asr.marginal(mussefunc,coef(fit.musse.otop))
timepoint<-c(timepoint,proc.time()[[3]])

## Save the resultant R objecct, st.nodes.neot
filename<-paste("st_nodes_otop_M2_CM",chrmax,"_unisample_XXXXXX.Robj",sep="")
save(st.nodes.otop,file=filename)
