...
require(phytools)
## read data
X<-read.csv("rachis.csv",header=TRUE)
X
rachis <- X$rachis
names(rachis) <- X$Species
class(rachis)

rachis <- as.factor(rachis)
class(rachis)
## read tree
...
fitER<-ace(rachis,tree,model="ER",type="discrete")
fitER
...
# 1. Open pdf file
pdf("rachis_ER_1.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
plotTree(tree,type="fan",fsize=0.7,ftype="i",lwd=1)
nodelabels(node=1:tree$Nnode+Ntip(tree),
           pie=fitER$lik.anc,piecol=colr,cex=0.4)
tiplabels(pie=to.matrix(rachis[tree$tip.label],
                        levels(rachis)),piecol=colr,cex=0.3)
...
mtreeER<-make.simmap(tree,rachis,model="ER")

# 1. Open pdf file
pdf("rachis_ER_2.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
...
mtreesER<-make.simmap(tree,rachis,model="ER",nsim=1000)

# 1. Open pdf file
pdf("rachis_ER_3.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
...
# 1. Open pdf file
pdf("rachis_ER_4.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
par(mfrow=c(1,1))
...
# 1. Open pdf file
pdf("rachis_ER_5.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
...
# 1. Open pdf file
pdf("rachis_ER_6.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
objER<-densityMap(mtreesER,states=levels(rachis)[2:1],plot=FALSE)
...
# 1. Open pdf file
pdf("rachis_ER_7.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
dotTree(tree,rachis,colors=colr,fsize=0.7,ftype="i",
        legend=FALSE)
...
# 1. Open pdf file
pdf("rachis_ER_8.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
par(mar=c(4,4,3,3))
