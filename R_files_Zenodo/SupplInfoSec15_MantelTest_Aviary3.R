...
Event_All_Count_A1<-Event_All_Count[which(Event_All_Count$Aviary=="3"),]
...
# Load the data
load("2018_A3_prebreedingMtx.RData") #prebreedmtx
...
for(i in 1:nrow(A1_individuals)){
  ...
  AdultsinA1<-dataParentsID[dataParentsID$aviary=="3",]
...
dataChickID_A1<-dataChickID[which(dataChickID$fosteraviary=="3"),]
...
A1AllMales<-dataParentsID$qr_code[which(dataParentsID$sex=="m"&dataParentsID$aviary=="3")]
A1AllFemales<-dataParentsID$qr_code[which(dataParentsID$sex=="f"&dataParentsID$aviary=="3")]
...
### plot 
pdf("A3ProvNet_MotherRec_network.pdf")
...
maleAdults<-dataParentsID$qr_code[which(dataParentsID$sex=="m"&dataParentsID$aviary=="3")]
...
femaleAdults<-dataParentsID$qr_code[which(dataParentsID$sex=="f"&dataParentsID$aviary=="3")]
...
pdf("Aviary3_PrebreedingSocialNetwork_RecipientMother.pdf")
...
### plot 
pdf("A3ProvNet_FatherRec_network.pdf")
...
pdf("Aviary3_PrebreedingSocialNetwork_RecipientFather.pdf")
coords <- layout_with_fr(net)
plot(net,layout=coords,edge.width=(200*E(net)$weight^1.2))
dev.off()
...
library(vegan)
mantel(prov, soc, na.rm = T) #r=-0.4784, Significance 0.901
...
pdf("Paper_Aviary3_ProvisioningNetwork.pdf")
...
pdf("Paper_Aviary3_PrebreedingSocialNetwork.pdf")
#coords <- layout_with_fr(net)
plot(net,layout=coords, edge.width=(200*E(net)$weight^1.2))
dev.off()
