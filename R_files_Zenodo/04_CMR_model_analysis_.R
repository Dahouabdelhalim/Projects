...
load(file="Logvol_encounter_history_Big5.Rdata")
...
for (i in 1:length(classes)) {
	...
	bodysize.bias <-function() {
   	   	...
   	   	#extinction models 
       	Phi.ME <- list(formula=~time + logvol + mass.ext + logvol * mass.ext)
       	Phi.BE <- list(formula=~time + logvol + mass.ext)
   
   		#sampling model 
       	p.logvol <- list(formula=~time + logvol)
   
       	#origination models
       	Gamma.ME <- list(formula=~time + logvol + rec.mass + logvol * rec.mass)
		...
		save(bodysize.models, file=paste(classes[i],"CMR_bodysize_model_results_big5","Rdata", sep="."))
...
write.table(coefficients.classes, file="CMR_coeffs_bodysize_big5.txt", sep="\\t")

AIC.class <- na.omit(AIC.class)
AIC.class

write.table(AIC.class, file="AIC_table_bodysize_big5.txt", sep="\\t")
