...
### LINEAR MODEL Site.Richness.Total ----
...
### LINEAR MODEL Sown.Richness.Total ----
...
### LINEAR MODEL Nonsown.Richness.Total ----
...
### LINEAR MODEL Mean.Nonsown.Cover ----
...
p1a <- effect_plot(nonsownrich.mod, pred = Weather.PC2, 
            interval = TRUE, plot.points = TRUE,
            x.label="PC2: Planting Year Precipitation",
...
p1b <- effect_plot(nonsowncover.mod, pred = Weather.PC2, 
            interval = TRUE, plot.points = TRUE,
            x.label="PC2: Planting Year Precipitation",
...
p1c <- effect_plot(sownrich.mod, pred = Weather.PC2, 
            interval = TRUE, plot.points = TRUE,
            x.label="PC2: Planting Year Precipitation",
...
p1d <- effect_plot(bc.mod, pred = Weather.PC2, 
            interval = TRUE, plot.points = TRUE,
            x.label="PC2: Planting Year Precipitation",
...
labels2<- c(Sown.Richness.Total="Sown richness",
            ...
            Mix.Veg.BCdissim="Bray-Curtis dissimilarity")
var.label<-c("Weather.PC1"="PC1: Planting-year temperature",
             "Weather.PC2"="PC2: Planting-year precipitation",
             "Weather.PC3"="PC3: Planting-year mild droughts",
...
### FIGURE S1: Histograms of Variables ----
...
### FIGURE S2: Weather vs. Climate ####
# Get climate data
load("climate.PY.comparison.RData")
head(climate.PY.comparison)

# High temp
high.mod <- lm(PY.high ~ climate.high, data=climate.PY.comparison)
effect_plot(high.mod, pred = climate.high, 
            interval = TRUE, 
            plot.points = TRUE,
            x.label="All Years",
            y.label="Planting Year Only")+
    theme_bw(base_size = 11, base_family = "") +
    theme(plot.title=element_text(size=15,face="bold"),
        axis.title=element_text(size=14),
        axis.text = element_text(size=12)) +
    ggtitle(expression(paste("a. Planting year versus all years, mean daily high temperature")))
summary(high.mod)
# adj R2=0.4522

# Low temp
low.mod <- lm(PY.low ~ climate.low, data=climate.PY.comparison)
effect_plot(low.mod, pred = climate.low, 
            interval = TRUE, 
            plot.points = TRUE,
            x.label="All Years",
            y.label="Planting Year Only")+
    ...
    theme(plot.title=element_text(size=15,face="bold"),
        axis.title=element_text(size=14),
        axis.text = element_text(size=12)) +
    ggtitle(expression(paste("b. Planting year versus all years, mean daily low temperature")))
summary(low.mod)
# adj R2=0.3435

# Precip
precip.mod <- lm(PY.precip ~ climate.precip, data=climate.PY.comparison)
effect_plot(precip.mod, pred = climate.precip, 
            interval = TRUE, 
            plot.points = TRUE,
            x.label="All Years",
            y.label="Planting Year Only")+
    ...
    theme(plot.title=element_text(size=15,face="bold"),
        axis.title=element_text(size=14),
        axis.text = element_text(size=12)) +
    ggtitle(expression(paste("c. Planting year versus all years, mean daily precipitation, mm")))
summary(precip.mod)
# adj R2=0.2449
...
### FIGURE S6: Pairwise Weather Scatterplots ----
...
### FIGURE S7: Pairwise Predictor Scatterplots ----
# Scatter plots and pairwise correlations (r) between non weather variables.

# Pull out predictors:
names(data)
names(data[,c(28:32)])
pred.only <- data[complete.cases(data[,]),c(28:32)] 
head(pred.only)
names(pred.only) <- c("Mix Richness", "Site Age", "Years Since Burn", "Seeding Density", "Soil Productivity")

# Plot:
pairs(pred.only, lower.panel = my_line, upper.panel = panel.cor)
