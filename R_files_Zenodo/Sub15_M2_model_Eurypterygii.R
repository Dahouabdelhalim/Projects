...
#MLE
print(paste("Start MLE MuSSE of neot M2 CM",chrmax, "..."))
timepoint<-c(proc.time()[[3]])
fit.musse.neot<-find.mle(mussefunc,p)
timepoint<-c(timepoint,proc.time()[[3]])

## Save fit.musse.neot
filename<-paste("fit_musse_neot_M2_CM",chrmax,"_XXXXXX.Robj",sep="")
save(fit.musse.neot,file=filename)

## Result of the 8 para Estimation

coef_res<-c(coef(fit.musse.neot)[c(8,7,5,6,2,4,1,3)]) #2020 definition
...
print(paste("Start ASR of M2 neot CM",chrmax, "..."))
timepoint<-c(proc.time()[[3]])
st.nodes.neot<-asr.marginal(mussefunc,coef(fit.musse.neot))
timepoint<-c(timepoint,proc.time()[[3]])

## Save the resultant R objecct, st.nodes.neot
filename<-paste("st_nodes_neot_M2_CM",chrmax,"_unisample_XXXXXX.Robj",sep="")
