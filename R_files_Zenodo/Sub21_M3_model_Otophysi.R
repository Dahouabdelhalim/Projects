...
## Function, get_cons_and_target_musse_testSE_ec
...
get_cons_and_target_musse_testSE_Ki1_ec<-function(chrmax,sp_state){
  ...
  target.i<-1:(snum*2)
  target.i<-target.i[-c(1,sp_state[1],(snum+1),(snum+sp_state[1]))]
...
## Function, get_cons_and_target_musse_ec
### almost same as get_cons_and_target but lambda and mu (speciation rate and extinction rate, respectively) 
### This function is specific to the test to know one given state have different lambda and mu from the other states.
### sp_state is the state number (calculated with scal)
...
free.i<-c(tcal(snum,1,2),tcal(snum,2,3),tcal(snum,3,2))
free.i<-c(1,sp_state[1],snum+1,snum+sp_state[1],((snum*2)+free.i))
cons_and_tar<-get_cons_and_target_musse_testSE_Ki1_ec(chrmax,sp_state)
...
#MLE
print(paste("Start MLE MuSSE of otop CM",chrmax, "testSE Ki=1..."))
timepoint<-c(proc.time()[[3]])
fit.musse.otop<-find.mle(mussefunc,p)
timepoint<-c(timepoint,proc.time()[[3]])

## Save fit.musse.otop
filename<-paste("fit_musse_otop_M3_CM",chrmax,"_samplingf_XXXXXX.Robj",sep="")
save(fit.musse.otop,file=filename)

## Result of the 7 para Estimation

coef_res<-c(coef(fit.musse.otop)[c(7,6,5,2,4,1,3)]) #2020 definition
