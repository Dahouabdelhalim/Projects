...
data<-read.snp(fileName)

#use K means clustering in adegenet to identify haplotype clusters for the X chromosome locus
grp <- find.clusters(data, max.n.clust=6,n.pca = 2,n.clust = 3)
K3_grouping<-as.data.frame(grp$grp)
outfile_name<-paste (fileName,".k3_clustering.csv",sep="")
write.table(K3_grouping, file = outfile_name, sep = '\\t', row.names = FALSE)

#implement PCA for the genlight object, retaining the first 2 principal components
...
#make and save new data frame containing the coordinates of each individual on each PC axis
