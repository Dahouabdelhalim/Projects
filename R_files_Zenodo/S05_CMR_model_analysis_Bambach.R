...
load(file="Logvol_encounter_history_Bambach.Rdata")
...
classes <- as.character(sort(unique(nd4$class)))

for (i in 1:length(classes)) {
	nd7 <- subset(nd4, class==classes[i])
	nd7.prad <- process.data(nd7, model="Pradsen")
    print(classes[i])
	...
	bodysize.bias <-function() {
   	   	...
   	   	#extinction models 
       	Phi.ME <- list(formula=~time + logvol * bamb.ext)
       	...
       	#origination models
       	Gamma.ME <- list(formula=~time + logvol * rec.bamb)
   		...
   		#run all the models
   		design.parameters=list(Phi=list(formula=~bamb.ext), Gamma=list(formula=~rec.bamb))
   		results <- mark.wrapper(model.list, data=nd7.prad, design.parameters=design.parameters, output=FALSE, silent=FALSE)
   		return(results)
		}

		bodysize.models <- bodysize.bias() #results saved to object 'bodysize.models'
		save(bodysize.models, file=paste(classes[i],"CMR_bodysize_model_bambach_results","Rdata", sep="."))
    	...
    	MRI1[1,1] <- classes[i]
    	MRI1[1,2:3] <- bodysize.models$Phi.ME.p.logvol.Gamma.ME$results$beta["Phi:logvol:bamb.ext",1:2]
    	MRI1[1,4:5] <- bodysize.models$Phi.ME.p.logvol.Gamma.ME$results$beta["Gamma:logvol:rec.bamb",1:2]
    	MRI1[1,6:7] <- bodysize.models$Phi.ME.p.logvol.Gamma.ME$results$beta["p:logvol",1:2]
    	MRI1[1,8] <- "mass.extinction/recovery-combined"
    	
    	MRI2[1,1] <- classes[i]
    	MRI2[1,2:3] <- bodysize.models$Phi.ME.p.logvol.Gamma.BE$results$beta["Phi:logvol:bamb.ext",1:2]
    	MRI2[1,4:5] <- bodysize.models$Phi.BE.p.logvol.Gamma.ME$results$beta["Gamma:logvol:rec.bamb",1:2]
...
write.table(coefficients.classes, file="CMR_coeffs_bodysize_bambach.txt", sep="\\t")

AIC.class <- na.omit(AIC.class)
AIC.class

write.table(AIC.class, file="AIC_table_bodysize_bambach.txt", sep="\\t")
