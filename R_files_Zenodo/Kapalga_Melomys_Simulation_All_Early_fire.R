...
system.time(
  melomys_AEF_pop_data <- foreach (i = 1:25, .combine = rbind) %do% {
    set.seed(333)
    Melomys3 <- simulation(landscape = melomys_landscape,
    ...
    pop_data <- return_pop_data(Melomys3)
    rm(Melomys3)
    gc()
    pop_data
  }
...
save(melomys_AEF_pop_data, file = "melomys_AEF_pop_data.RData")

# Plot possum population trajectory of each life-stage
plot(Melomys)

# Plot total possum population trajectory
plot(Melomys3, stage = 0, ylim = c(0,15000))
