...
deltaLs=0.3   #rate of small urchin consumption by lobsters
deltaLm=0.1 #rate of med urchin consumption by lobsters
deltaLl=0.005  #rate of large urchin consumption by lobsters
...
##############
###Sigma=0.0
plot00bi<-subset(kelpdiff00>50) # was 50  #determine where each is bistable
plot00bi<-matrix(as.integer(plot00bi), dim(plot00bi))
plot00bi[plot00bi==1]<-2  #look at plot00bi for where bistability occurs

plot00kelp<-subset(sig00kelp[,,1]>200) #pull out only those in the kelp forest state   #was 100 originally
plot00kelp<-matrix(as.integer(plot00kelp), dim(plot00kelp)) #turn T/F into numbers

plot00kelp[24,1]<-2       #from plot00bi, manually input where bistability occurs
plot00kelp[25,1]<-2
plot00kelp[26,2]<-2
plot00kelp[27,2]<-2
plot00kelp[28,3]<-2
...
plot00kelp[30,4:6]<-2
plot00kelp[31,5:8]<-2
plot00kelp[32,5:11]<-2
plot00kelp[33,6:15]<-2
plot00kelp[34,7:23]<-2
plot00kelp[35,9:41]<-2
plot00kelp[36,10:41]<-2
plot00kelp[37,12:41]<-2
plot00kelp[38,14:41]<-2
plot00kelp[39,17:41]<-2
plot00kelp[40,22:41]<-2
...
############
#Sigma=0.5
plot05bi<-subset(kelpdiff05>50) #was 100 Check where is bistability
...
plot05kelp[30,4:6]<-2
plot05kelp[31,5:8]<-2
plot05kelp[32,6:11]<-2
plot05kelp[33,6:17]<-2
plot05kelp[34,8:26]<-2
plot05kelp[35,9:41]<-2
plot05kelp[36,11:41]<-2
plot05kelp[37,13:41]<-2
plot05kelp[38,18:41]<-2
...
plot095kelp[25,1]<-2
plot095kelp[26,2]<-2
plot095kelp[27,2:3]<-2
plot095kelp[28,3:4]<-2
plot095kelp[29,3:5]<-2
plot095kelp[30,4:7]<-2
plot095kelp[31,5:11]<-2
plot095kelp[32,7:20]<-2
plot095kelp[33,12:41]<-2
...
tiff("ParameterSpaceXSize.tif", width=6, height=4, units="in", res=600, pointsize=5)
par(mfrow=c(3,2), mar=c(5,5,4,4), xaxs="i", yaxs="i", cex.axis=1.5, cex.lab=1.5)
image(plot00kelp, col=colors, las=1, xlab=expression("Lobster fishing mortality"~ (yr^{-1})), ylab=expression("Urchin fishing mortality" ~ (yr^{-1})), bty="O")
box(which="plot", lty="solid")
image(plot05kelp, col=colors, las=1, xlab=expression("Lobster fishing mortality"~ (yr^{-1})), ylab=expression("Urchin fishing mortality" ~ (yr^{-1})), bty="O")
box(which="plot", lty="solid")
image(plot095kelp, col=colors, las=1, xlab=expression("Lobster fishing mortality"~ (yr^{-1})), ylab=expression("Urchin fishing mortality" ~ (yr^{-1})), bty="O")
box(which="plot", lty="solid")
dev.off()

#plot of all 3
pdf("ParameterSpace_xtraSizeStruc.pdf")
par(mfrow=c(3,2))
