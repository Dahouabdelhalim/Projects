#R code for the multilevel selection analyses on individual strength and network density in the manuscript "Multilevel selection on social network traits: resource and sex effects on levels of selection in experimental populations of forked fungus beetles"
#This script measures multilevel selection gradients on individual strength and population density (connectedness) in males and females. This script first runs models with the observed dataset. This script then performs a node-level permutation to create 2000 randomizations of the observed dataset. This script then generates a null distribution of F-values from models that use the randomized datasets. After generating a null distribution of F-values, this script then compares the F-values from the model that uses observed data and calculates p-values.
...
#upload data
ObsSNBeetleset <- read.csv("ObsStrengthDensityBeetleset.csv")
...
##shuffle elytra, number of scans seen, treatment, period, strength, and density. Shuffle within each sex because sexes will be analyzed separately. Shuffle without replacement. 
...
for(i in 1:permutations) {
  PermSNShuffled[[i]] <- ObsSNBeetleset %>% group_by(Survey_Sex) %>% mutate(Elytra=sample(Elytra)) %>% mutate(ScansSeen=sample(ScansSeen)) %>% mutate(Treatment=sample(Treatment)) %>% mutate(Period=sample(Period)) %>% mutate(alpha=sample(alpha)) %>% mutate(StandardizedDensity=sample(StandardizedDensity)) 
}
...
obs.guards.strength.density.selection.global<-glmmTMB(GlobalStandardizedGuards~StandardizedAlpha*Treatment+StandardizedDensity*Treatment+StandardizedElytra+StandardizedScansSeen+Period+(1|Condo/ID), data=ObsSNBeetlesetM)
...
obs.lays.strength.density.selection.global.0inflation<-glmmTMB(GlobalStandardizedLays~StandardizedAlpha*Treatment+StandardizedDensity*Treatment+StandardizedElytra+StandardizedScansSeen+Period+(1|Condo/ID), ziformula=~1, data=ObsSNBeetlesetF)
summary(obs.lays.strength.density.selection.global.0inflation)
Anova(obs.lays.strength.density.selection.global.0inflation, type=3)
plot(allEffects(obs.lays.strength.density.selection.global.0inflation))

###test assumptions with DHARMa
obs.lays.strength.density.selection.global.0inflation.resid<-simulateResiduals(obs.lays.strength.density.selection.global.0inflation, n=250)
plot(obs.lays.strength.density.selection.global.0inflation.resid)
testDispersion(obs.lays.strength.density.selection.global.0inflation.resid)
testZeroInflation(obs.lays.strength.density.selection.global.0inflation.resid)

###extract F-values from Anova
obs.mls.females.Anova<-Anova(obs.lays.strength.density.selection.global.0inflation, type=3)
...
###calculate marginal means
emmeans(obs.lays.strength.density.selection.global.0inflation, "Period")
emmeans(obs.lays.strength.density.selection.global.0inflation, "Treatment")
emtrends(obs.lays.strength.density.selection.global.0inflation, pairwise~Treatment, var="StandardizedAlpha")
emtrends(obs.lays.strength.density.selection.global.0inflation, pairwise~Treatment, var="StandardizedDensity")
...
perm.mls.females = function(PermSNFemales){
  perm.mls.females.mod<-glmmTMB(GlobalStandardizedLays~StandardizedAlpha*Treatment+StandardizedDensity*Treatment+StandardizedElytra+StandardizedScansSeen+Period+(1|Condo/ID), ziformula=~1, data=PermSNFemales)
