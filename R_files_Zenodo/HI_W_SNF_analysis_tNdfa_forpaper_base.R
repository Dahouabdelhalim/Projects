# NSF FX N fixation analysis for the Waiakea 15N data: total N fixation

# This script calculates N fixation estimates (total Ndfa; %Ndfa is in another
# file) for the trees at Waiakea from the Menge, Funk, Perakis, Wolf NSF
...
bmcols <- read.csv("HI_W_FX_AllBiomassComponents.csv")
...
tissuechem <- read.csv("HI_W_FX_TissueCN_FilledData.csv")
...
tNdfa_kg_y_u_GLSE_89 <- 
	c(
	dat[dat$Species=="GLSE" & !is.na(dat$Nfix_g_N_kg_biomass_yr_u_04) & 
		dat$Use_growth_04 == 1,]$Nfix_g_N_kg_biomass_yr_u_04,
	dat[dat$Species=="GLSE" & !is.na(dat$Nfix_g_N_kg_biomass_yr_u_06) & 
...
summary(lme_tNdfa_kg_y_base_GLSE <- lme(tNdfa_kg_y_u_GLSE_89 ~ 0 + Treatment_GLSE_89,
	random=~1 | Tree_GLSE_89))
print(em_lme_tNdfa_kg_y_base_GLSE <- emmeans(lme_tNdfa_kg_y_base_GLSE, list(pairwise ~ Treatment_GLSE_89), adjust = "tukey"))
# emmeans a a a a, all > 0 (based on 95% CIs)
tab <- as.data.frame(summary(em_lme_tNdfa_kg_y_base_GLSE)$emmean)
print(round(tNdfa_kg_y_base_GLSE_LN <- c(tab[2,2],tab[2,2]-tab[2,3],tab[2,2]+tab[2,3]),2))
print(round(tNdfa_kg_y_base_GLSE_MN <- c(tab[3,2],tab[3,2]-tab[3,3],tab[3,2]+tab[3,3]),2))
print(round(tNdfa_kg_y_base_GLSE_HN <- c(tab[1,2],tab[1,2]-tab[1,3],tab[1,2]+tab[1,3]),2))
print(round(tNdfa_kg_y_base_GLSE_PHN <- c(tab[4,2],tab[4,2]-tab[4,3],tab[4,2]+tab[4,3]),2))

# Get dataset for tNdfa_y and for treatment and for tree
tNdfa_y_u_GLSE_89 <- 
	c(
	dat[dat$Species=="GLSE" & !is.na(dat$Nfix_g_N_yr_u_04) & 
		dat$Use_growth_04 == 1,]$Nfix_g_N_yr_u_04,
	dat[dat$Species=="GLSE" & !is.na(dat$Nfix_g_N_yr_u_06) & 
		dat$Use_growth_06 == 1,]$Nfix_g_N_yr_u_06
	)
summary(lme_tNdfa_y_base_GLSE <- lme(tNdfa_y_u_GLSE_89 ~ 0 + Treatment_GLSE_89,
	random=~1 | Tree_GLSE_89))
print(em_lme_tNdfa_y_base_GLSE <- emmeans(lme_tNdfa_y_base_GLSE, list(pairwise ~ Treatment_GLSE_89), adjust = "tukey"))
# emmeans a a a a, none > 0 (based on 95% CIs)
tab <- as.data.frame(summary(em_lme_tNdfa_y_base_GLSE)$emmean)
print(round(tNdfa_y_base_GLSE_LN <- c(tab[2,2],tab[2,2]-tab[2,3],tab[2,2]+tab[2,3]),1))
print(round(tNdfa_y_base_GLSE_MN <- c(tab[3,2],tab[3,2]-tab[3,3],tab[3,2]+tab[3,3]),1))
print(round(tNdfa_y_base_GLSE_HN <- c(tab[1,2],tab[1,2]-tab[1,3],tab[1,2]+tab[1,3]),1))
print(round(tNdfa_y_base_GLSE_PHN <- c(tab[4,2],tab[4,2]-tab[4,3],tab[4,2]+tab[4,3]),1))
...
# 2018-2019
# Get dataset for tNdfa_kg_y and for treatment and for tree
tNdfa_kg_y_u_CAEQ_89 <- 
	c(
	dat[dat$Species=="CAEQ" & !is.na(dat$Nfix_g_N_kg_biomass_yr_u_04) & 
		dat$Use_growth_04 == 1,]$Nfix_g_N_kg_biomass_yr_u_04,
	dat[dat$Species=="CAEQ" & !is.na(dat$Nfix_g_N_kg_biomass_yr_u_06) & 
		dat$Use_growth_06 == 1,]$Nfix_g_N_kg_biomass_yr_u_06
	)
Treatment_CAEQ_89 <- 
	c(
...
summary(lme_tNdfa_kg_y_base_CAEQ <- lme(tNdfa_kg_y_u_CAEQ_89 ~ 0 + Treatment_CAEQ_89,
	random=~1 | Tree_CAEQ_89))
print(em_lme_tNdfa_kg_y_base_CAEQ <- emmeans(lme_tNdfa_kg_y_base_CAEQ, list(pairwise ~ Treatment_CAEQ_89), adjust = "tukey"))
# emmeans a a a a, all > 0
tab <- as.data.frame(summary(em_lme_tNdfa_kg_y_base_CAEQ)$emmean)
print(round(tNdfa_kg_y_base_CAEQ_LN <- c(tab[2,2],tab[2,2]-tab[2,3],tab[2,2]+tab[2,3]),2))
print(round(tNdfa_kg_y_base_CAEQ_MN <- c(tab[3,2],tab[3,2]-tab[3,3],tab[3,2]+tab[3,3]),2))
print(round(tNdfa_kg_y_base_CAEQ_HN <- c(tab[1,2],tab[1,2]-tab[1,3],tab[1,2]+tab[1,3]),2))
print(round(tNdfa_kg_y_base_CAEQ_PHN <- c(tab[4,2],tab[4,2]-tab[4,3],tab[4,2]+tab[4,3]),2))

# Get dataset for tNdfa_y and for treatment and for tree
tNdfa_y_u_CAEQ_89 <- 
	c(
	dat[dat$Species=="CAEQ" & !is.na(dat$Nfix_g_N_yr_u_04) & 
		dat$Use_growth_04 == 1,]$Nfix_g_N_yr_u_04,
	dat[dat$Species=="CAEQ" & !is.na(dat$Nfix_g_N_yr_u_06) & 
		dat$Use_growth_06 == 1,]$Nfix_g_N_yr_u_06
	)
summary(lme_tNdfa_y_base_CAEQ <- lme(tNdfa_y_u_CAEQ_89 ~ 0 + Treatment_CAEQ_89,
	random=~1 | Tree_CAEQ_89))
print(em_lme_tNdfa_y_base_CAEQ <- emmeans(lme_tNdfa_y_base_CAEQ, list(pairwise ~ Treatment_CAEQ_89), adjust = "tukey"))
# emmeans a a a a, all > 0
tab <- as.data.frame(summary(em_lme_tNdfa_y_base_CAEQ)$emmean)
print(round(tNdfa_y_base_CAEQ_LN <- c(tab[2,2],tab[2,2]-tab[2,3],tab[2,2]+tab[2,3]),1))
print(round(tNdfa_y_base_CAEQ_MN <- c(tab[3,2],tab[3,2]-tab[3,3],tab[3,2]+tab[3,3]),1))
print(round(tNdfa_y_base_CAEQ_HN <- c(tab[1,2],tab[1,2]-tab[1,3],tab[1,2]+tab[1,3]),1))
print(round(tNdfa_y_base_CAEQ_PHN <- c(tab[4,2],tab[4,2]-tab[4,3],tab[4,2]+tab[4,3]),1))

# Uncomment this to write out a spreadsheet with the N fixation data
#write.csv(dat[,c(2,5,8,9,49,59,60,77,87,88,101,109,111,132,133,142:154)],
#	"HI_W_FX_SNF.csv")
