...
pd <- population_dynamics(change = growth(melomys_trans_mat,
                                          transition_function = modified_transition_custom(fire_stack_early = "Early_fire",
                                                                                           ...
                                                                                           early_surv_mult = 0.58,
                                                                                           late_surv_mult = 0.65,
                                                                                           early_tran_mult = 0.58,
                                                                                           late_tran_mult = 0.65,
                                                                                           early_recr_mult = 1.90,
                                                                                           late_recr_mult = 1.26),
...
system.time(
  melomys_pop_data <- foreach (i = 1:25, .combine = rbind) %do% {
    set.seed(333)
    Melomys <- simulation(landscape = melomys_landscape,
    ...
    pop_data <- return_pop_data(Melomys)
    rm(Melomys)
    gc()
    pop_data
  }
