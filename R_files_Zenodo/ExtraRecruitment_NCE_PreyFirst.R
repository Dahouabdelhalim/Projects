...
#Time-delayed NCEs, first close the lobster fishery
...
tnce = 10
ncetimes<-1:tnce
out3=as.data.frame(lsoda(y=newerstate, times=ncetimes, func=threestage, parms=parms))  

#now set up NCEs
ncestate<-c(A=out3$A[10],Us=out3$Us[10],Um=out3$Um[10],Ul=out3$Ul[10], L=out3$L[10])
...
parms= c(r=r, Ka=Ka, Kul=Kul, phiU=phiU, phiL=phiL, sigma=sigma,deltaUs=deltaUs, deltaUm=deltaUm, 
         ...
         deltaLs=deltaLs, deltaLm=deltaLm,deltaLl=deltaLl, MUl=MUl, MUm=MUm,
         MUs=MUs, FU=FU, taus=taus, taum=taum, taul=taul, ML=ML, FL=FL)
out4=as.data.frame(lsoda(y=ncestate, times=times, func=threestage, parms=parms))
...
maxfished<-max(y$FishedDens)  # 15.425  #15.58 with delayed NCE
fishedvol<-(maxfished-y[410,8])/y[410,8] #0.111   #with delayed NCE: 0.322
maxcomm<-max(y$CommDens) #27.32
commvol<-(maxcomm - y[410,7])/y[410,7] #0.418   #with delayed NCE: 0.672
