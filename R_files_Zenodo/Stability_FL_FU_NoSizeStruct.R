...
####Set attack rate params so that there is no size structured predation####
###----------parameters----------------------###
...
deltaLs=0.133   #rate of small urchin consumption by lobsters
deltaLm=0.133 #rate of med urchin consumption by lobsters
deltaLl=0.133  #rate of large urchin consumption by lobsters
...
##############
###Sigma=0.0
plot00bi<-subset(kelpdiff00>40) #determine where each is bistable
plot00bi<-matrix(as.integer(plot00bi), dim(plot00bi))
plot00bi[plot00bi==1]<-2  #look at plot00bi for where bistability occurs

plot00kelp<-subset(sig00kelp[,,1]>175) #pull out only those in the kelp forest state
plot00kelp<-matrix(as.integer(plot00kelp), dim(plot00kelp)) #turn T/F into numbers

plot00kelp[20,1:41]<-2       #from plot00bi, manually input where bistability occurs

#plot
colors2<-c("white",  "darkgrey", "black")  #set up color vector for black=kelp, white=barren
sig0plot<-image(plot00kelp, col=colors2, las=1, xlab="Lobster fishing mortality", ylab="Urchin fishing mortality", bty="O")

############
#Sigma=0.5
plot05bi<-subset(kelpdiff05>30) # Check where is bistability
plot05bi<-matrix(as.integer(plot05bi), dim(plot05bi))
plot05bi[plot05bi==1]<-2

plot05kelp<-subset(sig05kelp[,,1]>180)
plot05kelp<-matrix(as.integer(plot05kelp), dim(plot05kelp))

plot05kelp[20,1:41]<-2       #from plot00bi, manually input where bistability occurs

#plot
sig05plot<-image(plot05kelp, col=colors2, las=1, xlab="Lobster fishing mortality", ylab="Urchin fishing mortality", bty="O")

##########
#######Sigma=0.95
plot095bi<-subset(kelpdiff095>40) #check where is it bistable
plot095bi<-matrix(as.integer(plot095bi), dim(plot095bi))
plot095bi[plot095bi==1]<-2

plot095kelp<-subset(sig095kelp[,,1]>190)
plot095kelp<-matrix(as.integer(plot095kelp), dim(plot095kelp))

#all bistability remains within the kelp forest state, ie Kelp biomass > 100kg

#plot
colors3<-c("white", "black", "darkgrey")
sig095plot<-image(plot095kelp, col=colors3, las=1, xlab="Lobster fishing mortality", ylab="Urchin fishing mortality", bty="O")

#hi resolution plot of all 3
tiff("ParameterSpaceNoSize.tif", width=6, height=4, units="in", res=600, pointsize=5)
...
box(which="plot", lty="solid")
image(plot095kelp, col=colors3, las=1, xlab="Lobster fishing mortality", ylab="Urchin fishing mortality", bty="O")
box(which="plot", lty="solid")
dev.off()


#plot of all 3
pdf("ParameterSpace_NoSizeStruc.pdf")
par(mfrow=c(3,2))
image(plot00kelp, col=colors2, las=1, xlab="Lobster fishing mortality", ylab="Urchin fishing mortality", bty="O")
box(which="plot", lty="solid")
image(plot05kelp, col=colors2, las=1, xlab="Lobster fishing mortality", ylab="Urchin fishing mortality", bty="O")
box(which="plot", lty="solid")
image(plot095kelp, col=colors3, las=1, xlab="Lobster fishing mortality", ylab="Urchin fishing mortality", bty="O")
box(which="plot", lty="solid")
dev.off()
...
###################wireframe 3d plot#######################
###Sigma = 0.0
df<-expand.grid(list(x=FLseq,y=FUseq))  #put FLseq and sigmaseq into a single matrix
#head(df,10)  #check what df looks like
#tail(df,10) #same^
df$z=as.vector(kelpdiff00)  #add a z column to the df matrix, changing N to a vector #change which matrix from array N to get other state variables

#plot as wireframe with rainbow colors, scale bar
#pdf("BiStability3d00.pdf")
wireframe(z ~ x*y, df,
          aspect = c(61/87, 0.4),  #Aspect ratio of graph
          scales = list(arrows=F,cex=0.5,tick.number=10, z=list(arrows=T)),
          xlab="Lobster F", ylab="Urchin F", zlab="Equil Diff", 
          main="sigma=0.0",
          drape=T, col.regions = rev(rainbow(1000, start=0, end=0.7)), #to make black and white: par.settings=standard.theme(color=F),
          screen=list(z=0,x=-60) #screen=list(z=-110,x=-60)  # #Orientation of graph
)
#dev.off()

##3d wireframe for sigma=0.5
df$z=as.vector(kelpdiff05)  #add a z column to the df matrix, changing N to a vector #change which matrix from array N to get other state variables
#pdf("BiStability3d05.pdf")
wireframe(z ~ x*y, df,
          aspect = c(61/87, 0.4),  #Aspect ratio of graph
          scales = list(arrows=F,cex=0.5,tick.number=10, z=list(arrows=T)),
          xlab="Lobster F", ylab="Urchin F", zlab="Equil Diff", 
          main="sigma=0.5",
          drape=T, col.regions = rev(rainbow(1000, start=0, end=0.7)), #to make black and white: par.settings=standard.theme(color=F),
          screen=list(z=20,x=-60) #screen=list(z=-110,x=-60)  # #Orientation of graph
)
#dev.off()

##3d wireframe for sigma=0.95
df$z=as.vector(kelpdiff095)  #add a z column to the df matrix, changing N to a vector #change which matrix from array N to get other state variables
#pdf("BiStability3d05.pdf")
wireframe(z ~ x*y, df,
          aspect = c(61/87, 0.4),  #Aspect ratio of graph
          scales = list(arrows=F,cex=0.5,tick.number=10, z=list(arrows=T)),
