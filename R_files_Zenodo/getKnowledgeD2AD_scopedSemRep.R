...
# count variants of anemia in subject position
sql <- " select distinct subject_name, subject_cui, subject_semtype, count(distinct PMID) as cnt 
          from causalpredications where lower(subject_name) LIKE '%anemia%' 
          GROUP BY subject_name, subject_cui, subject_semtype ORDER BY cnt DESC;"
...
sort(setdiff(intersect(colliders[,4], mediators[,4]), confounders[,4]))
