# R script to run the metabolic network model for smaller G. barretti sponges (Bart et al. 2020; doi: doi: 10.1002/lno.11652)
# Date: June 2020
...
#read the input file
file    <- "MN_Gbarretti_smaller_fit.input" 						# LIM inputfile for smaller sponges
  readLIM     <- Read(file)            							# this reads the LIM file
...
for (i in 1:1000000){ 
  #create ranges
   ...
   Cfix_nit1<-c(Cfix_nit1,runif(1,min=0.04,max=0.11))     	# literature range
    ...
    Oxyupt<-c(Oxyupt, runif(1, min = 3.1, max = 19.3))  	# measured Oxyupt (mean +/- sd)
    DOCupt<-c(DOCupt, runif(1, min = 12, max = 39))  		# measured DOCupt (mean +/- sd)
    BacCupt<-c(BacCupt, runif(1, min = 0, max = 0.31)) 		# measured Bacupt (mean +/- sd)
    NH4upt<-c(NH4upt, runif(1, min = -0.54, max = 0.41)) 	# measured NH4upt (mean +/- sd)
    NO2upt<-c(NO2upt, runif(1, min = -0.069, max = 0.023)) 	# measured NO2upt (mean +/- sd)
    NO3rel<-c(NO3rel, runif(1, min = -1.74, max = -0.16)) 	# measured NO3rel (mean +/- sd)
...
##analyse reaction coefficients from uncertainty analysis
