...
ArrivalDate <- function(Slice) {
  ...
  mtext(paste(Slice$Common_Name, " - ", Slice$Year), cex = 0.75)
...
# Provide and write summaries by species, grid cell, and year
eBird_Dat_summary <- eBird_Dat %>% 
  select(Common_Name, GridID, Year) %>% 
  distinct()
write_csv(eBird_Dat_summary, "Routine 6/Routine 6 - merged files/eBird_Daily_Occupancy_all_grids_summary.csv")
# How many grid cells?
eBird_Dat_summary %>% 
  select(GridID) %>% 
  distinct() %>% 
...
# How many unique block combinations?
eBird_Dat_summary %>% 
  distinct() %>% 
  nrow()
...
# -----------------------------------------------------
# Go through all species one by one
# -----------------------------------------------------
# Replace "[1]" with each number in sequence to model each species
eBird_Dat_Spp <- eBird_Dat %>% 
  filter(Common_Name == spp_names[1])
...
pdffile <- paste("Routine 7/Processing by species/", speciesname, "_ebird_MAD_Jd_", Jd_min, "_", Jd_max_write, "_w_enddate_", enddateadd, "_Occ_Thresh_", Occupancy_Threshold, "_2002-2019_", datestub, ".pdf", sep = "")
...
# Start function
ArrivDateSum <- plyr::ddply(eBird_dat_enddate,
                               c("GridID",
                                 # "Common_Name", 
                                 "Year"),
...
csvfile <- paste("Routine 7/Processing by species/", speciesname, "_ebird_MAD_Jd_", Jd_min, "_", Jd_max_write, "_w_enddate_", enddateadd, "_Occ_Thresh_", Occupancy_Threshold, "_2002-2019_", datestub, ".csv", sep = "")
