##### This file is to calculate genetic variance-covariances to determine Robertson-Price identities of leaf traits (from the rosette dataset), both for reduced and full models. #####
...
colnames(traits);
traits2017 <- cbind(traits[ , c(1:14)], traits[ , c("LAR_rescaled_2017", "LWC_Rosette_Predicted_nonNeg_2017", "SLA_Rosette_Predicted_log_2017", "w_Reproduction_2017")]);
traits2018 <- cbind(traits[ , c(1:14)], traits[ , c("LAR_rescaled_2018", "LWC_Rosette_Predicted_nonNeg_2018", "SLA_Rosette_Predicted_log_2018", "w_Reproduction_excludeDead_2018")]);
...
write.csv(traits2017, "Results/Gmat_leaf/Formatted_input_data_2017.csv", row.names = F);
write.csv(traits2018, "Results/Gmat_leaf/Formatted_input_data_2018.csv", row.names = F);
...
getGMat <- function(variables, variable_distributions, modelName, data = traits, pedigree, snow, n_iterations = 2500, n_burnin = 500) {

	# reformat the data;
	data_sub <- cbind(data[ , c(1:14)], data[ , variables]);
	...
	write.csv(data_sub, file = paste("Results/Gmat_leaf/", modelName, "_", snow, ".csv", sep = ""), quote = F, row.names = F);
	...
	modelGMat <- MCMCglmm(as.formula(model_formula), random = ~us(trait):animal + us(trait):Garden_Block, rcov = ~us(trait):units, pedigree = target_ped, data = data_sub, prior = priorblock_internal, family = variable_distributions, nitt = n_iterations, thin = round((n_iterations - n_burnin)/2000), burnin = n_burnin);
...
# test the function;
#z <- getGMat(variables = c("LAR_rescaled_2017", "w_Reproduction_2017"), variable_distributions = c("poisson", "ordinal"), modelName = "2017_test", snow = "c", pedigree = ped, data = traits2017);
...
allVarCombos <- list(c("LAR_rescaled_2017", "LWC_Rosette_Predicted_nonNeg_2017", "SLA_Rosette_Predicted_log_2017", "w_Reproduction_2017"),
	c("LAR_rescaled_2017", "w_Reproduction_2017"),
	c("LWC_Rosette_Predicted_nonNeg_2017", "w_Reproduction_2017"),
	c("SLA_Rosette_Predicted_log_2017", "w_Reproduction_2017"),
	c("LAR_rescaled_2018", "LWC_Rosette_Predicted_nonNeg_2018", "SLA_Rosette_Predicted_log_2018", "w_Reproduction_excludeDead_2018"),
	c("LAR_rescaled_2018", "w_Reproduction_excludeDead_2018"),
	c("LWC_Rosette_Predicted_nonNeg_2018", "w_Reproduction_excludeDead_2018"),
	c("SLA_Rosette_Predicted_log_2018", "w_Reproduction_excludeDead_2018"));

allVarDistributions <- list(c("poisson", "gaussian", "gaussian", "ordinal"),
	c("poisson", "ordinal"),
	c("gaussian", "ordinal"),
	c("gaussian", "ordinal"),
	c("poisson", "gaussian", "gaussian", "ordinal"),
	c("poisson", "ordinal"),
	c("gaussian", "ordinal"),
	c("gaussian", "ordinal"));

allModelNames <- list("FullModel_2017", "LAR_rescaled_2017", "LWC_Rosette_Predicted_nonNeg_2017", "SLA_Rosette_Predicted_log_2017",
	"FullModel_2018", "LAR_rescaled_2018", "LWC_Rosette_Predicted_nonNeg_2018", "SLA_Rosette_Predicted_log_2018");
...
for (i in 1:length(allVarCombos)) {
	...
	for (j in 1:length(allSnowTreatments)) {
		 ...
		 tempRes <- getGMat(variables =allVarCombos[[i]], variable_distributions = allVarDistributions[[i]], modelName = allModelNames[[i]], data = temp_traitsdf, pedigree = ped, snow = allSnowTreatments[j], n_iterations = 2000000, n_burnin = 600000);
		outname <- paste("Results/Gmat_leaf/res_", allModelNames[[i]], "_", allSnowTreatments[j], ".Rdata", sep = "");
