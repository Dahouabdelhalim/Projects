...
ca_dispersal <- cellular_automata_dispersal(max_cells = c(50,50),
                                            dispersal_proportion = density_dependence_dispersing(maximum_proportions = c(1,0.25)))
...
pd <- population_dynamics(change = growth(bandicoot_trans_mat,
                                          transition_function = modified_transition_custom(fire_stack_early = "Early_fire",
                                                                                           ...
                                                                                           early_surv_mult = 0.86,
                                                                                           late_surv_mult = 0.6,
                                                                                           early_tran_mult = 0.86,
                                                                                           late_tran_mult = 0.6,
                                                                                           early_recr_mult = 1.0,
                                                                                           late_recr_mult = 0.21),
...
system.time(
  bandicoot_AEF_pop_data <- foreach (i = 1:25, .combine = rbind) %do% {
    set.seed(333)
    Coot3 <- simulation(landscape = coot_landscape,
    ...
    pop_data <- return_pop_data(Coot3)
    rm(Coot3)
    gc()
    pop_data
  }
...
save(bandicoot_AEF_pop_data, file = "bandicoot_AEF_pop_data.RData")

# Plot bandicoot population trajectory of each life-stage
plot(Coot)

# Plot total bandicoot population trajectory
plot(Coot3, stage = 0, ylim = c(0,35000))

# Plot bandicoot abundance by timestep
plot(Coot3[1], type = "raster", stage = 0, timesteps = 1:126, animate = TRUE)
