...
#MLE
print(paste("Start MLE of otop M0 CM",chrmax, "args..."))
timepoint<-c(proc.time()[[3]])
fit.musse.otop<-find.mle(mussefunc,p)
timepoint<-c(timepoint,proc.time()[[3]])

## Save fit.musse.otop
filename<-paste("fit_musse_otop_M0_CM",chrmax,"_unisample_XXXXXX.Robj",sep="")
...
print(paste("Start ASR of otop M0 CM",chrmax, "args..."))
timepoint<-c(proc.time()[[3]])
st.nodes.otop<-asr.marginal(mussefunc,coef(fit.musse.otop))
timepoint<-c(timepoint,proc.time()[[3]])

## Save st.nodes.otop
save(st.nodes.otop,file="st_nodes_otop_M0_XXXXXX.Robj")
