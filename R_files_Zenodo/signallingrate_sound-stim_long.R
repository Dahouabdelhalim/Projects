...
res <- lme(log.long~time.in.rec+stimulus+sex*(z.age+I(z.age^2))+z.temp+I(z.temp^2), random=~1|id, data=xdata)#, family='poisson'
...
## Colinearity
library(car)
res.lm <- lm(log.long~z.age+sex+z.temp+stimulus+time.in.rec, data=xdata)
vif(res.lm)# Ok, <4
...
res <- lme(log.long~time.in.rec+stimulus+sex*(z.age+I(z.age^2))+z.temp+I(z.temp^2), random=~1|id, data=xdata)
