...
setwd("~/Dropbox/Rosvall_Postdoc/Heat Stress/WGCNA/Heat Stress Gonad/Gonad heat vs control")
...
gonad = read.csv("ZF_gonad_normalizedCounts_PE_nocold.csv"); # This dataset is of normalized counts from PE reads
...
dim(countMatrixFiltered) # 11839    15
...
dim(Filtered) # Left with 8879 genes out of 17051
write.csv(Filtered,file="ZF_gonad_normCounts_logreduced25.csv")
...
########################################### Read in trait data ##########################################
traitData = read.csv("ZF_heat_traits_WGCNA_nocold.csv");
dim(traitData) # 15 31
names(traitData)
...
datExpr<-read.csv("ZF_gonad_normCounts_logreduced25.csv")
dim(datExpr) # 8879    16
...
save(MEs, moduleLabels, moduleColors, geneTree, file = "ZF_gonad_hotvscontrol_normcounts_nocold_modules.RData")
...
write.csv(geneInfo, file = "ZF_gonad_hot_genename_pant.csv")
...
chooseTopHubInEachModule(datExpr0,colorh = "brown") # can't get this to work... out put is 8168 for both
