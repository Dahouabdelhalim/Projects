...
# make aic table -------------------------------
header <- "Paller: varying parasite density"
aic_tab <- aic_table(model_fits, d)
save_aic_table(aic_tab, header, filename = "tables/paller1_table.md")
(best_models <- rownames(subset(aic_tab, dAICc < 2)))

# make parameter table ----------------------------------------------------
make_par_table(model_fits, name = "tables/parameter_estimates/paller1.csv")
