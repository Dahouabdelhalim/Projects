...
require(phytools)
## read data
X<-read.csv("bending.csv",header=TRUE)
X
bending <- X$bending
names(bending) <- X$Species
class(bending)

bending <- as.factor(bending)
class(bending)
## read tree
...
fitER<-ace(bending,tree,model="ER",type="discrete")
fitER
...
# 1. Open pdf file
pdf("bending_ER_1.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
plotTree(tree,type="fan",fsize=0.7,ftype="i",lwd=1)
nodelabels(node=1:tree$Nnode+Ntip(tree),
           pie=fitER$lik.anc,piecol=colr,cex=0.4)
tiplabels(pie=to.matrix(bending[tree$tip.label],
                        levels(bending)),piecol=colr,cex=0.3)
...
mtreeER<-make.simmap(tree,bending,model="ER")

# 1. Open pdf file
pdf("bending_ER_2.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
...
mtreesER<-make.simmap(tree,bending,model="ER",nsim=1000)

# 1. Open pdf file
pdf("bending_ER_3.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
...
# 1. Open pdf file
pdf("bending_ER_4.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
par(mfrow=c(1,1))
...
# 1. Open pdf file
pdf("bending_ER_5.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
...
# 1. Open pdf file
pdf("bending_ER_6.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
objER<-densityMap(mtreesER,states=levels(bending)[2:1],plot=FALSE)
...
# 1. Open pdf file
pdf("bending_ER_7.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
dotTree(tree,bending,colors=colr,fsize=0.7,ftype="i",
        legend=FALSE)
...
# 1. Open pdf file
pdf("bending_ER_8.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
par(mar=c(4,4,3,3))
