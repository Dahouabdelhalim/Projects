...
align.params <- list(bwa.max.edit.dist = 0.04, bwa.max.gap.opens = 2, 
...
mpileup.params <- list(mp.disable.prob.realign = T, mp.mismatch.qual.downgrade = 0, 
  ...
  mp.gap.extension.error.prob = 40, mp.homopolymer.error.coef = 100, mp.no.indels = T, 
...
fasta.params <- list(fasta.min.cov = 3, fasta.min.freq = 0.6, fasta.min.freq.cov = 10)
