#R code for the multilevel selection analyses on local and global clustering coefficient in the manuscript "Multilevel selection on social network traits: resource and sex effects on levels of selection in experimental populations of forked fungus beetles"
#This script measures multilevel selection gradients on local and global clustering coefficient in males and females. This script first runs models with the observed dataset. This script then performs a node-level permutation to create 2000 randomizations of the observed dataset. This script then generates a null distribution of F-values from models that use the randomized datasets. After generating a null distribution of F-values, this script then compares the F-values from the model that uses observed data and calculates p-values.
...
#upload data
ObsSNBeetleset <- read.csv("ObsLocalGlobalCCBeetleset.csv")

#remove beetles without local cc values
ObsSNBeetleset %>%
  filter(!is.na(am)) -> ObsSNBeetleset
...
for(i in 1:permutations) {
  PermSNShuffled[[i]] <- ObsSNBeetleset %>% group_by(Survey_Sex) %>% mutate(Elytra=sample(Elytra)) %>% mutate(ScansSeen=sample(ScansSeen)) %>% mutate(Treatment=sample(Treatment)) %>% mutate(Period=sample(Period)) %>% mutate(am=sample(am)) %>% mutate(StandardizedGlobalCC=sample(StandardizedGlobalCC))
}
...
ObsSNBeetleset %>%
  ...
  mutate(StandardizedLocalCC = ((am-(mean(am)))/sd(am))) -> ObsSNBeetleset
...
PermSNShuffled<-lapply(PermSNShuffled, function(x) {mutate(x, StandardizedLocalCC = ((am-mean(am)))/sd(am))})
...
##guards~localcc+gloalcc####
obs.guards.localcc.globalcc.selection.global<-glmmTMB(GlobalStandardizedGuards~StandardizedLocalCC*Treatment+StandardizedGlobalCC*Treatment+StandardizedElytra+StandardizedScansSeen+Period+(1|Condo/ID), data=ObsSNBeetlesetM)
summary(obs.guards.localcc.globalcc.selection.global)
Anova(obs.guards.localcc.globalcc.selection.global, type=3)
plot(allEffects(obs.guards.localcc.globalcc.selection.global))

###test for assumptions with DHARMa
obs.guards.localcc.globalcc.selection.global.resid<-simulateResiduals(obs.guards.localcc.globalcc.selection.global, n=250)
plot(obs.guards.localcc.globalcc.selection.global.resid)
testDispersion(obs.guards.localcc.globalcc.selection.global.resid)
testZeroInflation(obs.guards.localcc.globalcc.selection.global.resid)

###extract F-values from Anova
obs.mls.males.Anova<-Anova(obs.guards.localcc.globalcc.selection.global, type=3)
obs.mls.males.Anova %>%
  #extract F-values
  dplyr::select("Chisq") %>%
...
emmeans(obs.guards.localcc.globalcc.selection.global, "Period")
emmeans(obs.guards.localcc.globalcc.selection.global, "Treatment")
emtrends(obs.guards.localcc.globalcc.selection.global, pairwise~Treatment, var="StandardizedLocalCC")
emtrends(obs.guards.localcc.globalcc.selection.global, pairwise~Treatment, var="StandardizedGlobalCC")

##lays~localcc+globalcc####
obs.lays.localcc.globalcc.selection.global<-glmmTMB(GlobalStandardizedLays~StandardizedLocalCC*Treatment+StandardizedGlobalCC*Treatment+StandardizedElytra+StandardizedScansSeen+Period+(1|Condo/ID), data=ObsSNBeetlesetF)
summary(obs.lays.localcc.globalcc.selection.global)
Anova(obs.lays.localcc.globalcc.selection.global, type=3)
plot(allEffects(obs.lays.localcc.globalcc.selection.global))

###test for assumptions with DHARMa
obs.lays.localcc.globalcc.selection.global.resid<-simulateResiduals(obs.lays.localcc.globalcc.selection.global, n=250)
plot(obs.lays.localcc.globalcc.selection.global.resid)
testDispersion(obs.lays.localcc.globalcc.selection.global.resid)
testZeroInflation(obs.lays.localcc.globalcc.selection.global.resid)

###extract F-values from Anova
obs.mls.females.Anova<-Anova(obs.lays.localcc.globalcc.selection.global, type=3)
...
###calculate marginal means
emmeans(obs.lays.localcc.globalcc.selection.global, "Period")
emmeans(obs.lays.localcc.globalcc.selection.global, "Treatment")
emtrends(obs.lays.localcc.globalcc.selection.global, pairwise~Treatment, var="StandardizedLocalCC")
emtrends(obs.lays.localcc.globalcc.selection.global, pairwise~Treatment, var="StandardizedGlobalCC")
...
##guards~localcc+globalcc####

###function that runs the model
perm.mls.males = function(PermSNMales){
  perm.mls.males.mod<-glmmTMB(GlobalStandardizedGuards~StandardizedLocalCC*Treatment+StandardizedGlobalCC*Treatment+StandardizedElytra+StandardizedScansSeen+Period+(1|Condo/ID), data=PermSNMales)
...
perm.mls.females = function(PermSNFemales){
  perm.mls.females.mod<-glmmTMB(GlobalStandardizedLays~StandardizedLocalCC*Treatment+StandardizedGlobalCC*Treatment+StandardizedElytra+StandardizedScansSeen+Period+(1|Condo/ID), data=PermSNFemales)
