...
# DESeq2 analysis of T. longispinosus
...
#########################################################################################
## Input results generated by RSEM
#########################################################################################
...
save(countsMatrix,file="countsMatrix_Tlongi.Rdata")

# Read in BLAST hits, TAB format.
blast <- read.table("/Directory/BLAST/All_TL_BLAST_0801.out.tab",header=TRUE,sep="\\t",quote="")

# Create a data frame containing the behavioral groups in the same order as in the countsmatrix
sample_info<-data.frame(group=c("Follower", "Leader", "Scout","Follower", "Leader", "Scout","Follower", "Leader", "Scout","Follower", "Leader", "Scout","Follower", "Leader", "Scout"), colony=c("A", "A", "A", "F", "F", "F", "J", "J", "J","O", "O","O", "U", "U", "U"))
...
save(matrix_cook,file="matrix_cook_Tlongi.Rdata")
...
write.table(table_SCvsLE,file="DESeq2_ScoutvsLeader_Tlongi.csv", sep = "\\t", row.names = FALSE)
...
write.table(table_SCvsFO,file="DESeq2_ScoutvsFollower_Tlongi.csv", sep="\\t", row.names = FALSE)
...
write.table(table_LEvsSC,file="DESeq2_LeadervsScout_Tlongi.csv", sep="\\t", row.names = FALSE)
...
write.table(table_LEvsFO,file="DESeq2_LeadervsFollower_Tlongi.csv", sep="\\t", row.names = FALSE)
...
write.table(table_FOvsSC,file="DESeq2_FollowervsScout_Tlongi.csv", sep="\\t", row.names = FALSE)
...
write.table(table_FOvsLE,file="DESeq2_FollowervsLeader_Tlongi.csv", sep="\\t", row.names = FALSE)

####################################################################################################################
## Plots
####################################################################################################################
# Read in the lists where contigs up-regulated in the same behavior are merged
scout <- read.table("DESeq2_All_Scout_Tlongi.csv",header=TRUE,sep="\\t",quote="")
leader <-read.table("DESeq2_All_Leader_Tlongi.csv",header=TRUE,sep="\\t",quote="")
follower <-read.table("DESeq2_All_Follower_Tlongi.csv",header=TRUE,sep="\\t",quote="")
...
tiff("Vennie_Tlongi.tiff",width = 1280, height = 1230)
...
ggplot(plotData1, aes(x = PC1, y = PC2, color= group.1, shape=colony)) +
  ...
  coord_fixed()+
  scale_color_manual(values=c("steelblue", "palevioletred2", "paleturquoise3"), labels=c("Scout", "Leader", "Follower"))+scale_shape_manual(values=c(0:8))+
