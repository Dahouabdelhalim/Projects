...
require(phytools)
## read data
X<-read.csv("stigma.csv",header=TRUE)
X
stigma <- X$stigma
names(stigma) <- X$Species
class(stigma)

stigma <- as.factor(stigma)
class(stigma)
## read tree
...
fitARD<-ace(stigma,tree,model="ARD",type="discrete")
fitARD
...
# 1. Open pdf file
pdf("stigma_ARD_1.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
plotTree(tree,type="fan",fsize=0.7,ftype="i",lwd=1)
nodelabels(node=1:tree$Nnode+Ntip(tree),
           pie=fitARD$lik.anc,piecol=colr,cex=0.4)
tiplabels(pie=to.matrix(stigma[tree$tip.label],
                        levels(stigma)),piecol=colr,cex=0.3)
add.simmap.legend(colors=colr,prompt=FALSE,x=0.9*par()$usr[1],
                  y=0.8*par()$usr[3],fsize=0.8)
# 3. Close the file
dev.off()

## An alternative tactic to the one outline above is to use an MCMC approach to sample character histories from their posterior probability stigma. 
...
## rather than a probability stigma for the character at nodes.
## simulate single stochastic character map using empirical Bayes method
mtreeARD<-make.simmap(tree,stigma,model="ARD")

# 1. Open pdf file
pdf("stigma_ARD_2.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
...
## we need to look at the whole stigma from a sample of stochastic maps. 
## This can be a bit overwhelming.

mtreesARD<-make.simmap(tree,stigma,model="ARD",nsim=1000)

# 1. Open pdf file
pdf("stigma_ARD_3.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
...
# 1. Open pdf file
pdf("stigma_ARD_4.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
par(mfrow=c(1,1))
...
# 1. Open pdf file
pdf("stigma_ARD_5.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
...
# 1. Open pdf file
pdf("stigma_ARD_6.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
objARD<-densityMap(mtreesARD,states=levels(stigma)[2:1],plot=FALSE)
plot(objARD,fsize=c(0.6,1))
# 3. Close the file
dev.off()

## can generate a visualizing showing the posterior stigma of changes on the tree (rather than states). This is accomplished as follows:
# 1. Open pdf file
pdf("stigma_ARD_7.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
dotTree(tree,stigma,colors=colr,fsize=0.7,ftype="i",
        legend=FALSE)
...
# 1. Open pdf file
pdf("stigma_ARD_8.pdf", width = 8.27, height = 11.69)
# 2. Create the plot
par(mar=c(4,4,3,3))
