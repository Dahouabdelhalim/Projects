...
# Load each timeseries precinct data file from the list and turn each file into a dataframe with the cummulative vote totals at that timepoint
count <- 0
...
statewide_totals <- rbind(c(timepoint, Trump, Biden, Jorgensen))

# Name the columns
colnames(statewide_totals)<- c("Timepoint", "Trump","Biden","Jorgensen")
...
for (val in 1:174) {
  ...
  statewide_totals <- rbind(statewide_totals, c(timepoint, Trump, Biden, Jorgensen))
  timepoint = timepoint+1
}

# Convert the matrix to a dataframe
statewide_totals <- as.data.frame(statewide_totals)

# Export the dataframe as a Microsoft Excel file
write_xlsx(statewide_totals,"All GA Votes.xlsx")
